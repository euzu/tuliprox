use crate::{
    app::components::{
        convert_bool_to_chip_style, menu_item::MenuItem, popup_menu::PopupMenu, AppIcon, BatchInputContentView, Chip,
...
pub fn InputTable(props: &InputTableProps) -> Html {
    ...
    let render_data_cell = {
        ...
        Callback::<(usize, usize, Rc<InputRow>), Html>::from(move |(row, col, input): (usize, usize, Rc<InputRow>)| {
            match &*input {
                InputRow::Input(dto) => match col {
                    ...
                    8 => html! { <RevealContent preview={ html!{translator.t("LABEL.SETTINGS")}}><InputOptions input={dto.clone()} /></RevealContent> },
