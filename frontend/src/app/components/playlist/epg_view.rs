...
fn compute_zoomed_scroll_left(
    ...
    let content_anchor_x = scroll_left + anchor_x;
    ((content_anchor_x * (next_pixels_per_min / current_pixels_per_min)) - anchor_x).max(0.0)
}
...
pub fn EpgView() -> Html {
    ...
    let pixels_per_min = use_state(|| DEFAULT_PIXELS_PER_MIN);
    ...
    let handle_select_source = {
        ...
        Callback::from(move |req: PlaylistEpgRequest| {
            ...
            visible_range.set((0, 20));
            pixels_per_min.set(DEFAULT_PIXELS_PER_MIN);
...

    {
        let container_ref = container_ref.clone();
        let pending_scroll_left = pending_scroll_left.clone();
        use_effect_with(*pixels_per_min, move |_| {
...

    {
        ...
        use_effect_with((epg_window, *pixels_per_min), move |(epg_window, pixels_per_min)| {
...

    {
        ...
        use_effect_with(epg_window, move |epg_window| {
            update_now_line(&container_ref, &now_line_ref, epg_window, *pixels_per_min, true);
            || ()
        });
    }
    ...
    let apply_timeline_zoom = {
        ...
        Callback::from(move |(next_pixels_per_min, anchor_x): (f64, f64)| {
            let current_pixels_per_min = *pixels_per_min;
            let next_pixels_per_min = clamp_timeline_zoom(next_pixels_per_min);
            if (next_pixels_per_min - current_pixels_per_min).abs() < f64::EPSILON {
                return;
            }
            ...
            *pending_scroll_left.borrow_mut() = Some(next_scroll_left.round() as i32);
            pixels_per_min.set(next_pixels_per_min);
        })
    };
        ...
        Callback::from(move |e: WheelEvent| {
            ...
            if let Some(timeline) = timeline_ref.cast::<HtmlElement>() {
                ...
                let next_pixels_per_min = if e.delta_y() < 0.0 {
                    *pixels_per_min * WHEEL_ZOOM_FACTOR_IN
                } else {
                    *pixels_per_min * WHEEL_ZOOM_FACTOR_OUT
                };
            ...
            <div class="tp__epg__body" ref={container_ref}>
                {
                    if epg.is_none() {
                        ...
                        let window_duration = (end_window - start_window).max(0);
                        let num_blocks = (window_duration + TIME_BLOCK_MINS - 1) / TIME_BLOCK_MINS;
                        let timeline_content_width = (num_blocks as f64 * (*pixels_per_min * TIME_BLOCK_MINS as f64)).round() as i64;
                                    ...
                                    html! {
                                      <div key={i} class="tp__epg__channel-programs" style={row_style}>
                                        { for ch.programmes.iter().map(|p| {
                                            ...
                                            let left = get_pos(p.start, start_window, *pixels_per_min);
                                            let right = get_pos(p.stop, start_window, *pixels_per_min);
