...
pub fn HdHomeRunTargetOutputView(props: &HdHomeRunTargetOutputViewProps) -> Html {
    ...
    let output_form_state: UseReducerHandle<HdHomeRunTargetOutputFormState> =
        use_reducer(|| HdHomeRunTargetOutputFormState { form: HdHomeRunTargetOutputDto::default(), modified: false });
...

    {
        ...
        use_effect_with(config_output, move |cfg| {
            if let Some(output) = cfg {
                ...
                output_form_state
                    .dispatch(HdHomeRunTargetOutputFormAction::SetAll(HdHomeRunTargetOutputDto::default()));
            }
            || ()
        });
    }
    ...
    let handle_apply = {
        ...
        Callback::from(move |_| {
            ...
            source_editor_ctx
                .on_form_change
                .emit((block_id, BlockInstance::Output(Rc::new(TargetOutputDto::HdHomeRun(output)))));
