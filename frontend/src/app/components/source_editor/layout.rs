...
pub fn layout(blocks: &mut [Block], connections: &[Connection]) {
    ...
    let target_map: HashMap<BlockId, &TargetBlock> = target_blocks.iter().map(|t| (t.id, t)).collect();

    let mut last_input_y = CANVAS_OFFSET;

    for block_id in &input_order {
        let connected_targets: Vec<&TargetBlock> =
            connections.iter().filter(|c| c.from == *block_id).filter_map(|c| target_map.get(&c.to)).copied().collect();

        let desired_y = if connected_targets.is_empty() {
            last_input_y
        } else {
            // Center Y-Position of target
            let min_y = connected_targets.iter().map(|t| t.position.1).fold(f32::INFINITY, |a, b| a.min(b));

            let max_y =
                connected_targets.iter().map(|t| t.position.1 + t.height).fold(f32::NEG_INFINITY, |a, b| a.max(b));
