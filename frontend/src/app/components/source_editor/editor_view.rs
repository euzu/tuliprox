...
use crate::app::components::{
    ...
    InputRow, PortStatus, SourceEditorContext, SourceEditorForm, SourceEditorSidebar, TextButton,
...
use shared::model::{
    ConfigInputDto, ConfigSourceDto, ConfigTargetDto, HdHomeRunTargetOutputDto, InputType,
...
fn create_instance(block_type: BlockType) -> BlockInstance {
    match block_type {
        BlockType::InputXtream => {
            BlockInstance::Input(Rc::new(ConfigInputDto::new_with_type(InputType::Xtream)))
        }
        BlockType::InputM3u => {
            BlockInstance::Input(Rc::new(ConfigInputDto::new_with_type(InputType::M3u)))
        }
        BlockType::InputLibrary => {
            BlockInstance::Input(Rc::new(ConfigInputDto::new_with_type(InputType::Library)))
        }
        BlockType::Target => {
...
pub fn SourceEditor() -> Html {
                    ...
                    for input_row in inputs {
                        match input_row.as_ref() {
                            InputRow::Input(input_config) => {
                                let block_id = if let Some(&existing_id) =
                                    added_inputs.get(&input_config.id)
                                {
                                    existing_id
                                } else {
                                    let id = current_id;
    ...
    let handle_save = {
        ...
        Callback::from(move |_| {
            let mut sources_config = config_ctx
                .config
                .as_ref()
            ...
            //find all inputs
            let input_blocks: Vec<&Block> = editor_state
                .blocks
                .iter()
            ...
            // Find all targets
            let target_blocks: Vec<&Block> = editor_state
                .blocks
                .iter()
                        ...
                        if conn.from == target_block.id {
                            if let Some(output_block) = editor_state.get_block(conn.to) {
                                if output_block.block_type.is_output() {
                                    if let BlockInstance::Output(output_dto) =
                                        &output_block.instance
                                    {
    ...
    let handle_confirm_save = {
        ...
        Callback::from(move |_| {
            ...
            spawn_local(async move {
                let result = confirm
                    .confirm(&translator.t("MESSAGES.CONFIRM_SOURCES_SAVE"))
                    .await;
