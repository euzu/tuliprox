...
use crate::{
    config_field, config_field_bool, config_field_child, config_field_custom, config_field_hide,
    config_field_optional, edit_field_bool, edit_field_list, edit_field_number,
    edit_field_number_f64, edit_field_number_u64, edit_field_number_usize, edit_field_text,
    edit_field_text_option, generate_form_reducer,
};
use shared::model::{
    CacheConfigDto, GeoIpConfigDto, RateLimitConfigDto, ResourceRetryConfigDto,
    ReverseProxyConfigDto, ReverseProxyDisabledHeaderConfigDto, StreamBufferConfigDto,
    StreamConfigDto,
};
...
pub fn ReverseProxyConfigView() -> Html {
    ...
    let stream_buffer_state: UseReducerHandle<StreamBufferConfigFormState> =
        use_reducer(|| StreamBufferConfigFormState {
            form: StreamBufferConfigDto::default(),
            modified: false,
        });

    let failover_patterns_state: UseReducerHandle<FailoverPatternsFormState> =
        use_reducer(|| FailoverPatternsFormState {
            form: FailoverPatternsDto::default(),
            modified: false,
        });
        ...
        use_effect_with(
            ...
            move |(
                ...
                resource_retry_form.failover_redirect_patterns =
                    if failover_patterns.form.is_empty() {
                        None
                    } else {
        ...
        use_effect_with(
            ...
            move |(cfg, _mode)| {
                if let Some(rp) = cfg {
                    ...
                    failover_patterns_state.dispatch(FailoverPatternsFormAction::SetAll(
                        FailoverPatternsDto {
                            patterns: rp
                                .resource_retry
                                .as_ref()
                    ...
                    stream_buffer_state.dispatch(StreamBufferConfigFormAction::SetAll(
                        StreamBufferConfigDto::default(),
                    ));
                    failover_patterns_state.dispatch(FailoverPatternsFormAction::SetAll(
                        FailoverPatternsDto::default(),
                    ));
                }
                || ()
            },
