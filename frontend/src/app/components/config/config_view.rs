        ...
        components::{
            config::{
                config_page::{
                    ...
                    LABEL_RECORDING_CONFIG, LABEL_REVERSE_PROXY_CONFIG, LABEL_SCHEDULES_CONFIG, LABEL_WEB_UI_CONFIG,
                },
...
fn config_form_to_config_page(form: &ConfigForm) -> ConfigPage {
    match form {
        ...
        ConfigForm::Recording(_, _) => ConfigPage::Recording,
...
pub fn ConfigView() -> Html {
    ...
    let tabs = {
        ...
        use_memo((form_state_value, edit_value, translate.clone()), move |(forms, editing, translate)| {
            ...
            let tab_configs = vec![
                ...
                (ConfigPage::Recording, LABEL_RECORDING_CONFIG, html! { <RecordingConfigView/> }, "Record"),
                (
    ...
    let handle_save_config = {
        ...
        Callback::from(move |_| {
            ...
            spawn_local(async move {
                let mut ok = true;
                let main_refresh_required = modified_main_dto.is_some();
                let api_proxy_refresh_required = modified_api_proxy.is_some();
                let sources_refresh_required = modified_sources.is_some();
                ...
                if ok {
                    ...
                    let has_main_refresh = !main_refresh_required || app_config.is_some();
                    let has_api_proxy_refresh = !api_proxy_refresh_required || api_proxy_config.is_some();
                    let has_sources_refresh = !sources_refresh_required || app_config.is_some();
                    ...
                    if has_main_refresh && has_api_proxy_refresh && has_sources_refresh {
                        let set_edit_mode = set_edit_mode.clone();
                        set_timeout(
                            move || {
                                set_edit_mode.set(false);
