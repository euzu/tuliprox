...
use crate::{
    config_field, config_field_bool, config_field_child, config_field_optional, edit_field_bool,
    edit_field_list, edit_field_number, edit_field_number_u64, edit_field_text,
...
pub fn LibraryConfigView() -> Html {
    ...
    let form_state: UseReducerHandle<LibraryConfigFormState> =
        use_reducer(|| LibraryConfigFormState {
            form: LibraryConfigDto::default(),
            modified: false,
        });

    let playlist_state: UseReducerHandle<LibraryPlaylistConfigFormState> =
        use_reducer(|| LibraryPlaylistConfigFormState {
            form: LibraryPlaylistConfigDto::default(),
            modified: false,
        });

    let metadata_state: UseReducerHandle<LibraryMetadataConfigFormState> =
        use_reducer(|| LibraryMetadataConfigFormState {
            form: LibraryMetadataConfigDto::default(),
            modified: false,
        });

    let metadata_read_state: UseReducerHandle<LibraryMetadataReadConfigFormState> =
        use_reducer(|| LibraryMetadataReadConfigFormState {
            form: LibraryMetadataReadConfigDto::default(),
            modified: false,
        });

    let tmdb_state: UseReducerHandle<LibraryTmdbConfigFormState> =
        use_reducer(|| LibraryTmdbConfigFormState {
            form: LibraryTmdbConfigDto::default(),
            modified: false,
        });

    {
        ...
        let library_cfg = config_ctx
            .config
            .as_ref()
            .and_then(|c| c.config.library.clone());
        use_effect_with(
            (library_cfg, config_view_ctx.edit_mode.clone()),
            move |(library_cfg, _mode)| {
                if let Some(library) = library_cfg {
                    form_state.dispatch(LibraryConfigFormAction::SetAll(library.clone()));
                    playlist_state.dispatch(LibraryPlaylistConfigFormAction::SetAll(
                        library.playlist.clone(),
                    ));
                    metadata_state.dispatch(LibraryMetadataConfigFormAction::SetAll(
                        library.metadata.clone(),
                    ));
                    metadata_read_state.dispatch(LibraryMetadataReadConfigFormAction::SetAll(
                        library.metadata.read_existing.clone(),
                    ));
                    tmdb_state.dispatch(LibraryTmdbConfigFormAction::SetAll(
                        library.metadata.tmdb.clone(),
                    ));
                } else {
                    form_state
                        .dispatch(LibraryConfigFormAction::SetAll(LibraryConfigDto::default()));
                    playlist_state.dispatch(LibraryPlaylistConfigFormAction::SetAll(
                        LibraryPlaylistConfigDto::default(),
                    ));
                    metadata_state.dispatch(LibraryMetadataConfigFormAction::SetAll(
                        LibraryMetadataConfigDto::default(),
                    ));
                    metadata_read_state.dispatch(LibraryMetadataReadConfigFormAction::SetAll(
                        LibraryMetadataReadConfigDto::default(),
                    ));
                    tmdb_state.dispatch(LibraryTmdbConfigFormAction::SetAll(
                        LibraryTmdbConfigDto::default(),
                    ));
                }
                || ()
            },
