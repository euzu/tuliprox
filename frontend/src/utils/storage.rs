...
pub fn set_session_storage_item(key: &str, value: &str) {
    if let Some(storage) = window().and_then(|w| w.session_storage().ok()).flatten() {
        if let Err(err) = storage.set_item(key, value) {
            error!("failed to write to sessionStorage: {err:?}");
        }
    }
}

pub fn get_session_storage_item(key: &str) -> Option<String> {
    window().and_then(|w| w.session_storage().ok()).flatten().and_then(|storage| storage.get_item(key).ok().flatten())
}

pub fn remove_session_storage_item(key: &str) {
    if let Some(storage) = window().and_then(|w| w.session_storage().ok()).flatten() {
