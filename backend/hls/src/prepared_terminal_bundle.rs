...
use tuliprox_mpegts::transport_stream_buffer::{
...
pub struct HlsPreparedTerminalBundleKey {
...
#[derive(Clone)]
pub struct HlsPreparedTerminalSegment {
    pub index: u16,
...
#[derive(Clone)]
pub struct HlsPreparedTerminalBundle {
...
impl std::fmt::Debug for HlsPreparedTerminalBundle {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let byte_len = self.segments.iter().fold(0_usize, |total, segment| total.saturating_add(segment.bytes.len()));
        formatter
...
impl HlsPreparedTerminalBundle {
    pub fn matches_key_and_shape(&self, key: HlsPreparedTerminalBundleKey) -> bool {
        self.key == key
            ...
            && self.segments.iter().enumerate().all(|(position, segment)| {
                usize::from(segment.index) == position
                    ...
                    && terminal_timestamp_offset_ticks_90khz(segment.index, self.source_asset_duration_ticks_90khz)
...
#[derive(Clone)]
pub struct HlsAnchoredTerminalSegment {
    pub index: u16,
...
#[derive(Clone)]
pub struct HlsAnchoredTerminalBundle {
        ...
        self.prepared_key == key
            ...
            && self.segments.iter().enumerate().all(|(position, segment)| {
                usize::from(segment.index) == position
                    ...
                    && terminal_timestamp_offset_ticks_90khz(segment.index, source_asset_duration_ticks_90khz)
                        .map(|offset| add_timestamp_offsets_90khz(offset, self.splice_anchor.timestamp_delta_ticks))
...
pub enum HlsPreparedTerminalBundleIncompatibility {
    EmptySegmentSet,
    ZeroTargetDuration,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum HlsPreparedTerminalBundleBuildError {
    Incompatible(HlsPreparedTerminalBundleIncompatibility),
    AssetIdentityMismatch,
    TimestampOffsetOverflow { index: u16, source_asset_duration_ticks_90khz: u64 },
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum HlsPreparedTerminalBundleFailure {
...
#[derive(Debug, Clone)]
#[must_use]
pub enum HlsPreparedTerminalBundleState {
...
impl PartialEq for HlsPreparedTerminalBundleState {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (Self::Ready { .. } | Self::Preparing { .. } | Self::Failed { .. } | Self::Incompatible { .. }, _) => false,
        }
    }
}
...
#[derive(Debug, Clone)]
#[must_use]
pub enum HlsPreparedTerminalBundleCompletion {
    Ready { bundle: Arc<HlsPreparedTerminalBundle> },
    Failed { key: HlsPreparedTerminalBundleKey, reason: HlsPreparedTerminalBundleFailure },
    Incompatible { key: HlsPreparedTerminalBundleKey, reason: HlsPreparedTerminalBundleIncompatibility },
...
#[derive(Clone)]
pub struct HlsPreparedTerminalBundleCompletionTicket {
    key: HlsPreparedTerminalBundleKey,
    generation: u64,
    completion: Arc<HlsPreparedTerminalBundleFlightCompletion>,
}
...
#[cfg(any(test, feature = "test-support"))]
pub struct HlsPreparedTerminalBundleCompletionPublisher {
    completion: Arc<HlsPreparedTerminalBundleFlightCompletion>,
}
...
#[cfg(any(test, feature = "test-support"))]
pub fn prepared_terminal_bundle_completion_channel_for_test(
    key: HlsPreparedTerminalBundleKey,
) -> (HlsPreparedTerminalBundleCompletionTicket, HlsPreparedTerminalBundleCompletionPublisher) {
...
#[derive(Debug, Clone)]
#[must_use]
pub enum HlsPreparedTerminalBundleObservation {
...
pub struct HlsPreparedTerminalBundleCache {
...
impl HlsPreparedTerminalBundleCache {
    ...
    fn spawn_preparation(
        ...
        drop(runtime.spawn(async move {
            ...
            let entry = match result {
                ...
                Ok(Ok(bundle)) if bundle.key != key => {
                    HlsPreparedTerminalBundleCacheEntry::Failed(HlsPreparedTerminalBundleFailure::Build(
    ...
    pub fn state(&self, key: HlsPreparedTerminalBundleKey) -> Option<HlsPreparedTerminalBundleState> {
        let mut inner = self.lock_inner();
        if let Some(entry) = inner.entries.get(&key) {
    ...
    #[cfg(any(test, feature = "test-support"))]
    pub fn install_controlled_flight_for_test(
        &self,
        key: HlsPreparedTerminalBundleKey,
        ...
        inner.flights.insert(
            key,
            HlsPreparedTerminalBundleFlight { generation, completion: Arc::clone(&completion), estimated_bytes: 0 },
        );
        Some(HlsPreparedTerminalBundleCompletionPublisher { completion })
    }

    pub fn observe_exact(&self, key: HlsPreparedTerminalBundleKey) -> HlsPreparedTerminalBundleObservation {
        let mut inner = self.lock_inner();
...
fn estimated_bundle_bytes(asset: &HlsTerminalMediaAsset, segment_count: u16) -> Option<u64> {
    u64::try_from(asset.renderer().as_bytes().len()).ok()?.checked_mul(u64::from(segment_count))
}

fn cache_entry_bytes(entry: &HlsPreparedTerminalBundleCacheEntry) -> u64 {
    match entry {
        HlsPreparedTerminalBundleCacheEntry::Ready(bundle) => bundle
            .segments
            .iter()
            .fold(0_u64, |total, segment| total.saturating_add(u64::try_from(segment.bytes.len()).unwrap_or(u64::MAX))),
...
pub fn prepared_terminal_bundle_key(
...
pub const fn terminal_asset_fits_target_duration(asset_duration_ms: u64, target_duration_ms: u64) -> bool {
...
pub fn build_prepared_terminal_bundle(
    asset: &HlsTerminalMediaAsset,
    key: HlsPreparedTerminalBundleKey,
) -> Result<Arc<HlsPreparedTerminalBundle>, HlsPreparedTerminalBundleBuildError> {
...
pub fn anchor_prepared_terminal_bundle(
    asset: &HlsTerminalMediaAsset,
    ...
    for relative in prepared.segments.iter() {
        ...
        let bytes = renderer
            ...
            .map_err(HlsPreparedTerminalBundleBuildError::FiniteSegmentRender)?;
        let total_timestamp_offset_ticks_90khz =
            add_timestamp_offsets_90khz(relative.timestamp_offset_ticks_90khz, anchor.timestamp_delta_ticks);
        segments.push(HlsAnchoredTerminalSegment { index: relative.index, total_timestamp_offset_ticks_90khz, bytes });
    }
...
mod tests {
    use super::*;
    use crate::snapshot_terminal_media_asset;
    use std::sync::{
    ...
    fn hls_prepared_terminal_bundle_contains_twelve_small_segments_with_exact_asset_offsets() {
        ...
        assert_eq!(bundle.segments[1].timestamp_offset_ticks_90khz, bundle.source_asset_duration_ticks_90khz);
        let last = bundle.segments.last().expect("last prepared terminal segment");
        assert_eq!(last.timestamp_offset_ticks_90khz, u64::from(last.index) * bundle.source_asset_duration_ticks_90khz);
    ...
    fn target_duration_twelve_seconds_does_not_create_terminal_timestamp_gap() {
        ...
        let live_profile = tuliprox_mpegts::transport_stream_buffer::HlsTsTimestampProfile {
        ...
        let anchor = tuliprox_mpegts::transport_stream_buffer::HlsTsSpliceAnchor::between(live_profile, asset_profile)
            .expect("splice anchor");
        let key = prepared_terminal_bundle_key(&asset, TARGET_DURATION_MS, SEGMENT_COUNT);
        let prepared = build_prepared_terminal_bundle(&asset, key).expect("relative terminal bundle");
        let anchored = anchor_prepared_terminal_bundle(&asset, &prepared, anchor).expect("anchored terminal bundle");
        ...
        let actual_stride =
            segment_one.first_clock_90khz.wrapping_add(1_u64 << 33).wrapping_sub(segment_zero.first_clock_90khz)
                % (1_u64 << 33);
    ...
    fn hls_prepared_terminal_bundle_key_separates_asset_revision_fingerprint_and_target_duration() {
        ...
        let different_fingerprint =
            HlsPreparedTerminalBundleKey { asset: HlsTerminalAssetIdentity { fingerprint, ..key.asset }, ..key };
    ...
    async fn hls_prepared_terminal_bundle_limits_in_flight_owner_tasks() {
        ...
        for key in keys {
            assert!(matches!(cache.wait_for_completion(key).await, Some(HlsPreparedTerminalBundleState::Ready { .. })));
        }
    }
    ...
    async fn hls_prepared_terminal_bundle_rejects_a_single_bundle_above_the_byte_budget() {
        ...
        assert_eq!(
            ...
            HlsPreparedTerminalBundleState::Failed {
                key,
                reason: HlsPreparedTerminalBundleFailure::ByteCapacityExceeded { required_bytes, capacity_bytes },
            }
        );
        assert_eq!(cache.in_flight_count(), 0);
