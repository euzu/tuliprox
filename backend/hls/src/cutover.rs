...
pub enum HlsTerminalCutoverCapability {
    NotEvaluated,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct HlsTerminalCutoverInput {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum HlsTerminalCutoverDecision {
    NotRequired,
    RetrySupersededSnapshot,
    EvaluateTerminalCapability { preparation: HlsTerminalMediaPreparationDisposition },
    CommitTerminalTail,
...
pub fn evaluate_terminal_cutover(input: &HlsTerminalCutoverInput) -> HlsTerminalCutoverDecision {
...
fn terminal_decision_for_preparation(input: &HlsTerminalCutoverInput) -> HlsTerminalCutoverDecision {
    ...
    terminal_decision(input.terminal, preparation, input.terminal_preparation, input.required_terminal_media_key)
}
...
fn terminal_decision(
    ...
    match capability {
        ...
        HlsTerminalCutoverCapability::TailCompatible { .. } => {
            HlsTerminalCutoverDecision::CommitTerminalUnavailable(HlsTerminalTailCompatibility::AssetRevisionMismatch)
        }
