...
mod tests {
    use super::{
        ...
        adopt_repair_output, detect_video_codec, parse_ffmpeg_warnings, parse_probe,
        ready_segment_repair_prewarm_candidates, repair_object_metadata_key, select_repair_remux_streams, sha256_file,
    ...
    async fn on_demand_selection_joins_the_prewarm_candidate_identity() {
        ...
        assert_eq!(
            manager.windows.read().await.windows[&HlsAccessLeaseId("lease-a".to_string())].remaining_segments,
            0
        );
    }
    ...
    async fn prewarmed_ready_object_reuses_metadata_on_later_demand() {
        ...
        manager
            .spawn_ready_cache_prewarm_inner(Arc::clone(&cache), vec![(cache_key.clone(), context.clone())], None)
            .await;
