...
pub enum HlsInitialStripOutcome {
    Applied { mode: &'static str, configured: u64, effective: usize, visible_segments: usize },
    Skipped { reason: HlsInitialStripSkipReason, visible_segments: usize },
}
...
mod tests {
    ...
    fn pending_strip_segments_keeps_visible_head_window() {
        ...
        assert!(matches!(
            view.outcome,
            HlsInitialStripOutcome::Applied { mode: "segments", configured: 3, effective: 3, visible_segments: 3 }
        ));
    }
    ...
    fn pending_strip_seconds_counts_tail_extinf_durations() {
        ...
        assert!(matches!(
            view.outcome,
            HlsInitialStripOutcome::Applied { mode: "seconds", configured: 30, effective: 3, visible_segments: 3 }
        ));
    }
