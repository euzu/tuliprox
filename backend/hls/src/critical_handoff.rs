use super::{
    ...
    manifest_acceptance::{HlsManifestAcceptanceGeneration, HlsTerminalAlternativeCompatibility},
...
use tuliprox_core::model::{AppConfig, CustomStreamResponse};
...
const fn critical_handoff_lock_contention_decision(attempt: usize) -> HlsCriticalHandoffLockContentionDecision {
    ...
    AccessFuture: Future<Output = HlsCriticalHandoffStateAccess<Result<T, HlsManifestCommitError>>>,
{
    ...
    Err(staging_error(HlsManifestRejectLogReason::CriticalHandoffLockContentionExhausted))
}
...
pub(super) fn critical_handoff_snapshot_is_current(
    ...
    select_critical_handoff_lease(session, &active, acceptance_generation, generation_is_current, now_ms)
