...
impl HlsMapWorkerPool {
    ...
    pub fn with_global_semaphore_and_access_leases(
        ...
        Self::with_global_semaphore_access_leases_and_availability(policy, global_semaphore, access_leases, None)
    }

    pub fn with_global_semaphore_access_leases_and_availability(
        policy: SegmentFetchPolicy,
        global_semaphore: Arc<Semaphore>,
        access_leases: Arc<RwLock<HlsAccessLeaseStore>>,
    ...
    async fn fetch_one_map(
        ...
        let (generation_valid, evidence_changed_for) = {
            ...
            (generation_valid, generation_valid.then(|| session.proxy_session_id.clone()))
        };
        if fetch_succeeded && !generation_valid {
        ...
        if let (Some(coordinator), Some(proxy_session_id)) =
            (self.availability_reevaluations.as_ref(), evidence_changed_for.as_ref())
        {
...
mod tests {
    ...
    use crate::{
        HlsAccessLease, HlsAccessLeaseId, HlsAccessLeaseStore, HlsPlaybackFamilyKey, HlsSegmentCache, HlsSessionKey,
        HlsSessionStore, MapCacheStatus, ProxySessionId, SegmentFetchPolicy,
    };
    ...
    fn normal_manifest(body: &str, base_url: &str) -> tuliprox_parser::hls::origin_manifest::ParsedOriginManifest {
