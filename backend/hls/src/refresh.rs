use super::{
    ...
    manifest_fetch::{
        ...
        next_committed_origin_highwater, retry_hls_origin_manifest_recovery_chain,
        score_hls_manifest_candidate_for_selection_log, FetchedOriginManifest, HlsManifestAcceptanceRejectReason,
        HlsManifestCommitAcceptanceMode, HlsManifestCommitError, HlsManifestFetchSelection, HlsManifestOriginQuality,
        HlsManifestOriginRelation, HlsManifestRecoveryUnavailableReason, HlsManifestRejectLogReason,
    ...
    terminal_tail::{
        ...
        terminal_media_asset_identity, HlsTerminalBaseEvidence, HlsTerminalMediaAsset, HLS_TERMINAL_TAIL_SEGMENT_COUNT,
    },
    timeline::{effective_origin_host_id, HlsOriginHandoffPreview, HlsOriginHandoffPreviewError},
    CacheAccessState, HlsAccessLeaseId, HlsFreshManifestRequiredReason, HlsLogIdentity, HlsManifestAcceptanceDirective,
    HlsManifestRenderer, HlsMapWorkerPool, HlsOriginIoContext, HlsOriginWorkClass, HlsProxyManager,
    HlsRecoveryTriggerDiagnostic, HlsRecoveryTriggerSource, HlsSegmentCache, HlsSegmentRepairManager,
    HlsSegmentWorkerPool, HlsSessionHandle, HlsSessionMode, HlsTrackEvidenceResolution, MapCacheStatus, MapEntry,
    MapFetchContext, RenderedManifestStoreOutcome, RenderedManifestStoreRejectReason, SegmentCacheKey,
    SegmentCacheStatus, SegmentEntry, SegmentFetchContext, StagedCacheObject, TransientPassthroughReason,
...
mod tests {
    use super::{
        super::{
            ...
            manifest_fetch::{
                ...
                LiveHlsOriginEntry, ManifestRecoverySelectionLogPhase, OriginManifestFetchError,
                OriginManifestStatusClass, RetryPolicy,
            },
            manifest_origin_binding::HlsManifestOriginBinding,
            ...
            recovery_timing::{
                ...
                HlsRecoveryMapWorkload, HlsRecoveryMediumReadiness, HlsRecoveryObjectReadiness,
                HlsRecoverySegmentWorkload, HlsRecoveryTimingPolicy, HlsRecoveryWorkload, HlsRecoveryWorkloadEnvelope,
        ...
        apply_manifest_fetch_failure_signal, build_manifest_refresh_timing,
        cancel_superseded_terminal_work_after_media_progress, classify_manifest_fetch_failure, commit_fetched_manifest,
        compute_origin_refresh_interval_ms, current_time_millis, fetch_and_commit_manifest_with_policy,
        fetched_effective_manifest_host, format_millis_as_seconds, format_optional_millis_as_seconds,
        key_resource_extension, manifest_fetch_context, manifest_hard_fetch_error, manifest_progress_from_highwater,
        manifest_recovery_trigger, mark_origin_refresh_started, mark_origin_refresh_started_with_outcome,
        record_committed_manifest_media_progress, record_committed_manifest_success, refresh_and_commit,
    ...
    use crate::{
        ...
        maybe_trigger_origin_refresh, CacheAccessState, GarbageCollectionPolicy, HlsAccessLease, HlsAccessLeaseId,
        HlsAccessLeasePendingDeadline, HlsAccessLeaseStore, HlsAccessLeaseTiming, HlsBoundAccountAcquireErrorKind,
        HlsFreshManifestRequiredReason, HlsGarbageCollector, HlsLeaseManifestSegment, HlsLeaseManifestSnapshot,
        HlsLeasePlaybackMode, HlsLeaseReserveAvailabilityBasis, HlsLeaseReserveSnapshot,
        HlsManifestAcceptanceDirective, HlsManifestAcceptanceExhaustionReason, HlsManifestDeliveryMode,
        HlsManifestSourceRenderMarker, HlsMapWorkerPool, HlsMediaContainer, HlsOriginAccountBinding,
        HlsOriginIoContext, HlsPlaybackFamilyKey, HlsPreparedTerminalBundleState, HlsProxyManager, HlsSegmentCache,
        HlsSegmentRepairManager, HlsSegmentWorkerPool, HlsSession, HlsSessionKey, HlsSessionMode, HlsSessionStore,
        HlsTerminalTailCompatibility, MapCacheStatus, OriginSegmentKey, ProxySessionId, RenderedManifest,
        RenderedManifestStoreOutcome, SegmentCacheKey, SegmentCacheStatus, SegmentEntry, SegmentFetchPolicy,
        SegmentFetchPriority, TimelineMapError, TransientObjectFetchDecision, TransientPassthroughReason,
        TransientResourceFile, TransientResourceId, HLS_PROVISIONING_ORIGIN_EPOCH,
    };
    ...
    async fn hls_manifest_acceptance_directive_stale_guard_prevents_refresh_start() {
        ...
        session.write().await.origin_control.progress_generation =
            owner_key.origin_progress_generation.saturating_add(1);
    ...
    async fn assert_recovery_supersedes_terminal_preparation(
        ...
        assert_eq!(
            hls_proxy.commit_access_lease_terminal_if_generation_matches(HlsTerminalCommitRequest {
                ...
                asset_revision_guard: super::super::terminal_commit::HlsTerminalAssetRevisionGuard::matching_for_test(
                    None
                ),
            }),
    ...
    async fn recovery_unavailable_after_valid_response_preserves_response_evidence() {
        ...
        let mut request = test_origin_refresh_request(Arc::clone(&session));
        request.origin_entry = LiveHlsOriginEntry::parse(&format!("{}/live/new/index.m3u8", origin.base_url))
            .expect("candidate entry URL");
        ...
        assert_eq!(evidence_origin.requests.lock().await.len(), 1);
        {
            ...
            assert!(session
                .origin_control
                .last_origin_response_at_ms
                .is_some_and(|recorded_at_ms| recorded_at_ms > 50));
    ...
    async fn assert_committed_content_anchor_is_gc_pinned_until_read_pin_drop() {
        ...
        let access = {
            ...
            let expected_identity = super::HlsMediaResourceIdentity::from_url("http://origin.example/live/1.ts", None);
            session
                .segments
                .values()
                .rev()
                .find_map(|entry| {
                    let fetch_ref = entry.origin_fetch_ref.as_ref()?;
                    super::HlsMediaResourceIdentity::from_url(&fetch_ref.resolved_origin_url, fetch_ref.byte_range)
    ...
    fn candidate_handoff_preview(
        ...
        body: &str,
        now_ms: u64,
    ) -> (super::ParsedOriginManifest, Vec<super::TransientResourceRef>, super::HlsOriginHandoffPreview) {
        ...
        let preview = session
            .preview_origin_handoff_manifest(&manifest, super::effective_origin_host_id("candidate.example"), 0)
    ...
    fn hls_recovery_timing_candidate_preview_detects_generation_local_ready_aes128_key() {
        ...
        let workload =
            super::handoff_preview_recovery_workload(&session, actual_recovery_segment, None, &key_resources, 101);
    ...
    async fn staged_cross_host_aes_candidate_with_generation_local_ready_key_commits() {
        ...
        };
        {
            ...
            let fetch =
                session.transient.begin_object_fetch(&proxy_session_id, &key_resource, extension, now_ms, 300_000);
            let TransientObjectFetchDecision::Fetch(token) = fetch else {
