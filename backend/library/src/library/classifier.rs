use crate::{
    library::{scanner::ScannedMediaFile, SeriesKey},
...
impl MediaClassifier {
    ...
    pub fn classify(
        ...
        episode_counters: &mut HashMap<(String, u32), u32>,
    ) -> MediaClassification {
    ...
    fn classify_as_recording(file: &ScannedMediaFile, ptt_metadata: &PttMetadata) -> MediaClassification {
        MediaClassification::Recording { file_name: file.file_name.clone(), display_title: ptt_metadata.title.clone() }
    }

    fn make_series(episode: u32, season: u32, ptt_metadata: PttMetadata) -> MediaClassification {
        MediaClassification::Series {
            key: SeriesKey { title: ptt_metadata.title.clone(), year: ptt_metadata.year, tmdb_id: ptt_metadata.tmdb },
            episode,
            season,
    ...
    fn make_series_normalized(
        ...
        MediaClassification::Series {
            key: SeriesKey { title: normalized_title.to_string(), year: None, tmdb_id: None },
            episode,
            season,
            metadata: ptt_metadata,
        }
...
mod tests {
    ...
    fn test_force_series_without_pattern_auto_increments() {
        let mut counters = HashMap::new();
        let file1 =
            create_test_file("Breaking.Bad.1999.1080p.BluRay.mkv", "/tv/Breaking.Bad", LibraryContentType::Series);
