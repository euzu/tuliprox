use crate::library::{
    tmdb::{TmdbCredits, TmdbNetwork, TmdbSeriesInfoEpisodeDetails},
    ScannedMediaFile,
};
...
impl MetadataCacheEntry {
    ...
    pub fn new(file_path: String, file_size: u64, file_modified: i64, metadata: MediaMetadata) -> Self {
        Self {
    ...
    pub fn is_file_modified(&self, file: &ScannedMediaFile, season_num: u32, episode_num: u32) -> bool {
        match &self.metadata {
            MediaMetadata::Movie(_) => {
                self.file_size != file.size_bytes
                    || self.file_modified != file.modified_timestamp
            ...
            MediaMetadata::Series(series) => {
                ...
                for episode in episodes {
                    if episode.season == season_num
                        ...
                        && episode.file_path == file.file_path
                    {
                        return episode.file_size != file.size_bytes
                            || episode.file_modified != file.modified_timestamp;
                    }
                }
