...
use tuliprox_core::model::TmdbConfig;
use url::Url;
            ...
            let response = match timeout(request_timeout, self.client.get(url).send()).await {
                ...
                Ok(Err(err)) => {
                    if attempt < MAX_RETRIES {
                        warn!("TMDB request failed for {safe_url}: {err}, retrying {attempt}/{MAX_RETRIES}");
                ...
                Err(_) => {
                    if attempt < MAX_RETRIES {
                        warn!("TMDB request timed out for {safe_url} (attempt {attempt}/{MAX_RETRIES}), retrying");
                        sleep(Duration::from_millis(500 * u64::from(attempt))).await;
                        continue;
                    }
                    return Err(format!("TMDB API request timed out after {MAX_RETRIES} attempts for {safe_url}"));
                }
            };
