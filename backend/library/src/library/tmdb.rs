use crate::library::{
    metadata::{Actor, MetadataSource, MovieMetadata, SeriesMetadata},
    EpisodeMetadata, SeasonMetadata, VideoClipMetadata,
};
...
fn resolve_tmdb_image_url(path: &str) -> Option<String> {
    ...
    if normalized.starts_with('/') {
        return Some(format!("{TMDB_IMAGE_BASE_URL}{normalized}"));
    }
    Some(format!("{TMDB_IMAGE_BASE_URL}/{normalized}"))
}
...
fn crew_names(credits: Option<&TmdbCredits>, jobs: &[&str]) -> Option<Vec<String>> {
    credits
        ...
        .map(|crew| {
            crew.iter()
                .filter(|crew| crew.job.as_ref().is_some_and(|j| jobs.contains(&j.as_str())))
...
impl TmdbVideos {
    pub fn get_youtube(&self) -> Option<Vec<TmdbVideo>> {
        self.results.as_ref().map(|r| r.iter().filter(|v| v.site.eq_ignore_ascii_case("youtube")).cloned().collect())
    }
}
...
impl TmdbMovieDetails {
    pub fn to_meta_data(&self) -> MovieMetadata {
        MovieMetadata {
            ...
            genres: self
                .genres
                .as_ref()
                .map(|list| list.iter().map(|g| g.name.clone()).collect())
                .and_then(some_if_nonempty),
                ...
                .map(|cast| {
                    cast.iter()
                        ...
                        .map(|actor| Actor {
                            ...
                            thumb: actor.profile_path.as_ref().and_then(|p| resolve_tmdb_image_url(p)),
                        })
                        .collect::<Vec<_>>()
                })
                .and_then(some_if_nonempty),
            studios: self
                ...
                .and_then(some_if_nonempty),
            poster: self.poster_path.as_ref().and_then(|p| resolve_tmdb_image_url(p)),
            fanart: self.backdrop_path.as_ref().and_then(|p| resolve_tmdb_image_url(p)),
            ...
            videos: self
                ...
                .map(|list| list.iter().map(TmdbVideo::to_meta_data).collect::<Vec<_>>())
                .filter(|v| !v.is_empty()),
            technical: None,
        }
    }
...
impl TmdbSeriesInfoDetails {
    pub fn to_meta_data(&self) -> SeriesMetadata {
        SeriesMetadata {
            title: self.name.clone(),
            original_title: Some(self.original_name.clone()),
            year: self.first_air_date.split('-').next().and_then(|y| y.parse().ok()),
            plot: Some(self.overview.clone()),
            mpaa: None,
            ...
            episodes: self.seasons.as_ref().map(|season_list| {
                season_list
                    .iter()
                    .flat_map(|season| {
                        season
...
mod tests {
    ...
    fn test_resolve_tmdb_url_with_absolute_http() {
        ...
        assert_eq!(resolve_tmdb_image_url(input), Some(input.to_string()));
    }
    ...
    fn test_resolve_tmdb_url_with_protocol_relative() {
        ...
        assert_eq!(resolve_tmdb_image_url(input), Some(String::from("https://cdn.example.org/image.jpg")));
    }

    #[test]
    fn test_resolve_tmdb_url_with_leading_slash() {
        let input = "/abc/def.jpg";
        assert_eq!(resolve_tmdb_image_url(input), Some(format!("{TMDB_IMAGE_BASE_URL}{input}")));
    }
    ...
    #[test]
    fn test_resolve_tmdb_url_with_blank_path() {
        let input = "   ";
