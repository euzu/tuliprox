use crate::{
    library::{
        metadata::{MediaMetadata, MetadataSource, MovieMetadata, SeriesMetadata},
        scanner::ScannedMediaFile,
        tmdb_client::TmdbClient,
        MediaGroup, MetadataStorage,
    },
    ptt::{ptt_parse_title, PttMetadata},
};
use log::{debug, error, warn};
use shared::{defaults::TMDB_API_KEY, utils::clean_playlist_title};
use tuliprox_core::model::{LibraryConfig, MetadataUpdateConfig};
...
impl MetadataResolver {
        ...
        let tmdb_client = match metadata_update_config.filter(|config| config.tmdb.enabled) {
            Some(config) => {
                if let Some(tmdb_storage) = storage {
                    let api_key =
                        config.tmdb.api_key.as_ref().map_or_else(|| TMDB_API_KEY.to_string(), ToString::to_string);
    ...
    pub async fn resolve_from_title(
        &self,
        title: &str,
        known_tmdb_id: Option<u32>,
        is_movie: bool,
        resolve_from_tmdb: bool,
        ...
        if metadata.title.is_empty() {
            metadata.title = if cleaned_title.trim().is_empty() { title.to_string() } else { cleaned_title };
        }
    ...
    async fn resolve_from_tmdb_file(
        &self,
        movie: bool,
        metadata: &PttMetadata,
        tmdb: &TmdbClient,
        ...
        if movie {
            ...
            let (series_year, tmdb_id) = if metadata.year.is_some() {
                ...
                file.path.parent().and_then(|p| p.file_name()).and_then(|n| n.to_str()).map_or((None, None), |s| {
    ...
    async fn resolve_from_tmdb_title(
        &self,
        movie: bool,
        metadata: &PttMetadata,
        tmdb: &TmdbClient,
    ...
    fn resolve_from_filename(movie: bool, metadata: &PttMetadata, file: Option<&ScannedMediaFile>) -> MediaMetadata {
        ...
        let title = if metadata.title.is_empty() || metadata.title.chars().all(|c| c.is_ascii_digit()) {
            file.map_or_else(
                ...
                |f| f.path.file_stem().and_then(|s| s.to_str()).unwrap_or(&f.file_name).to_string(),
            )
        } else {
            metadata.title.clone()
        };
...
mod tests {
    ...
    fn create_test_library_config(fallback_filename: bool) -> LibraryConfig {
        LibraryConfig {
            ...
            metadata: LibraryMetadataConfig {
                read_existing: LibraryMetadataReadConfig { kodi: true, jellyfin: true, plex: true },
                fallback_to_filename: fallback_filename,
                formats: vec![],
            },
            playlist: LibraryPlaylistConfig { movie_category: "Movies".intern(), series_category: "Series".intern() },
    ...
    fn create_test_metadata_update_config(tmdb_enabled: bool) -> MetadataUpdateConfig {
        MetadataUpdateConfig {
            tmdb: TmdbConfig {
                ...
                api_key: if tmdb_enabled { Some("test_key".to_string()) } else { None },
                rate_limit_ms: 250,
    ...
    fn create_test_file(name: &str) -> ScannedMediaFile {
        ScannedMediaFile {
            ...
            content_type: shared::model::LibraryContentType::Auto,
        }
    }
