use crate::ptt::{
    constants::{BRACKETS, PTT_CONSTANTS},
    models::PttMetadata,
};
use fancy_regex::{Match, Regex as FancyRegex};
...
create_bitset!(u8, HandlerOptionFlags, SkipIfAlreadyFound, SkipFromTitle, SkipIfFirst, Remove);
...
impl PttParser {
        ...
        S: Fn(&mut PttMetadata, T) + Send + Sync + 'static,
    {
        ...
        let handler = Box::new(move |context: &mut ParseContext| -> Option<MatchInfo> {
            if options.flags.contains(HandlerOptionFlags::SkipIfAlreadyFound)
            ...
            if let Some(m) = match_result {
                ...
                let current_skip_from_title =
                    before_title_matched || options.flags.contains(HandlerOptionFlags::SkipFromTitle);
                ...
                if options.flags.contains(HandlerOptionFlags::SkipIfFirst) {
                    ...
                    if other_matches_exist {
                        ...
                        let is_before_all = context.matched.values().all(|m| current_start < m.match_index);
                        if is_before_all {
                            return None;
                        }
                    }
    ...
    pub fn parse(&self, raw_title: &str, _translate_languages: bool) -> PttMetadata {
        let mut context =
            ParseContext { title: raw_title.to_string(), result: PttMetadata::default(), matched: HashMap::new() };
...
pub fn clean_title(raw_title: &str) -> String {
    ...
    title = PTT_CONSTANTS.not_allowed_symbols_at_start_and_end.replace_all(&title, "").to_string();
    title = PTT_CONSTANTS.russian_cast_regex_fancy.replace_all(&title, "").to_string();
    ...
    title = PTT_CONSTANTS.not_only_non_english_regex.replace_all(&title, "$1").to_string();
    title = PTT_CONSTANTS.remaining_not_allowed_symbols_at_start_and_end.replace_all(&title, "").to_string();
    ...
    title = PTT_CONSTANTS.parantheses_without_content.replace_all(&title, "").to_string();
    title = PTT_CONSTANTS.special_char_spacing.replace_all(&title, "").to_string();
