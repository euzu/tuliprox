    ...
    spawn_blocking(move || {
        ...
        match xtream_cluster {
             XtreamCluster::Live => {
                  ...
                  deserializer.deserialize_any(visitor).map_err(|e| TuliproxError::new(TuliproxErrorKind::Notify, format!("JSON parse error: {e}")))?;
             }
             XtreamCluster::Video => {
        ...
        Ok(())
    }).await.map_err(|e| TuliproxError::new(TuliproxErrorKind::Notify, format!("Streaming parse failed: {e}")))??;
