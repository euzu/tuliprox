...
use crate::processing::processor::xtream::playlist_resolve_livetv; // Import added
...
async fn process_playlist_for_target(ctx: &PlaylistProcessingContext,
    ...
    for provider_fpl in playlists.iter_mut() {
            ...
            if let Some(opts) = provider_fpl.input.options.as_ref() {
                 if opts.analyze_stream {
                     ...
                     if ctx.config.config.load().video.as_ref().is_some_and(|v| v.ffprobe_enabled) {
                         ...
                         // We need input type to decide what ID to use
        ...
        // Pass the metadata manager if available
        playlist_resolve_series(&ctx.config, &ctx.client, target, errors, &pipe, provider_fpl, &mut processed_fpl, ctx.provider_manager.as_ref(), ctx.metadata_manager.as_ref()).await;
        playlist_resolve_vod(&ctx.config, &ctx.client, target, errors, provider_fpl, &mut processed_fpl, ctx.provider_manager.as_ref(), ctx.metadata_manager.as_ref()).await;
        // Resolve LiveTV (probe check)
        playlist_resolve_livetv(&ctx.config, &ctx.client, target, errors, &mut processed_fpl, ctx.provider_manager.as_ref(), ctx.metadata_manager.as_ref()).await;
