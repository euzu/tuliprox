...
pub trait PlaylistSource: Send + Sync {
    ...
    fn get_groups(&mut self) -> Vec<PlaylistGroup>;
...
impl PlaylistSource for EmptyPlaylistSource {
    ...
    fn get_groups(&mut self) -> Vec<PlaylistGroup> { vec![] }
...
impl XtreamDiskPlaylistSource {
    ...
    async fn reload(&mut self) {
        if self.live.is_none() {
            ...
            self.live = load_bplustree_query::<XtreamPlaylistItem>(&self.app_config, &live_path).await
                .map(|(query, guard)| (query, Arc::new(guard)));
        }
        if self.vod.is_none() {
            let vod_path = xtream_get_file_path(&self.storage_path, XtreamCluster::Video);
            self.vod = load_bplustree_query::<XtreamPlaylistItem>(&self.app_config, &vod_path).await
                .map(|(query, guard)| (query, Arc::new(guard)));
        }

        if self.series.is_none() {
            let series_path = xtream_get_file_path(&self.storage_path, XtreamCluster::Series);
            self.series = load_bplustree_query::<XtreamPlaylistItem>(&self.app_config, &series_path).await
                .map(|(query, guard)| (query, Arc::new(guard)));
        }
    }
}
...
pub struct M3uDiskPlaylistSource {
    app_config: Arc<AppConfig>,
    file_path: PathBuf,
    playlist: Option<BPlusTreeQuery<u32, M3uPlaylistItem>>,
    guard: Option<Arc<FileReadGuard>>,
}

impl M3uDiskPlaylistSource {
    pub(crate) async fn new(app_config: &Arc<AppConfig>, file_path: &Path) -> Self {
        let mut source = M3uDiskPlaylistSource {
            app_config: Arc::clone(app_config),
    ...
    async fn reload(&mut self) {
        self.guard = None;
        self.playlist = load_bplustree_query::<M3uPlaylistItem>(&self.app_config, &self.file_path).await
            .map(|(query, guard)| {
...
impl PlaylistSource for M3uDiskPlaylistSource {
    fn is_memory(&self) -> bool { false }
    fn get_channel_count(&mut self) -> usize { self.playlist.as_mut().map_or(0usize, |t: &mut BPlusTreeQuery<u32, M3uPlaylistItem>| t.len().unwrap_or(0usize)) }
    fn get_group_count(&mut self) -> usize {
        let mut groups = HashSet::new();
    ...
    fn update_playlist<'a>(&'a mut self, _plg: &'a PlaylistGroup) -> BoxFuture<'a, ()> {
        Box::pin(async move {
            ...
            // self.playlist = None;
            // {
            //     let _lock = self.app_config.file_locks.write_lock(&self.file_path).await;
            //     if let Ok(mut tree) = BPlusTreeUpdate::<u32, M3uPlaylistItem>::try_new(&self.file_path) {
            //         let m3u_items: Vec<M3uPlaylistItem> = plg.channels.iter().map(M3uPlaylistItem::from).collect();
            //         let batch: Vec<(&u32, &M3uPlaylistItem)> = m3u_items.iter().map(|item| (&item.virtual_id, item)).collect();
            //         let _ = tree.upsert_batch(&batch);
            //     }
            // }
            // self.reload().await;
            ...
            for (_, item) in q.iter() {
                groups_map.entry(item.group.clone())
                    .or_insert_with(|| PlaylistGroup {
                        ...
                        title: item.group.clone(),
                        channels: vec![],
                        xtream_cluster: XtreamCluster::Live, // Default for M3U
                    })
                    .channels.push(PlaylistItem::from(&item));
            }
            groups_map.into_values().collect()
...
async fn load_bplustree_query<P>(app_config: &Arc<AppConfig>, file_path: &Path) -> Option<(BPlusTreeQuery<u32, P>, FileReadGuard)>
where
    P: Serialize + for<'de> Deserialize<'de> + Clone + Send + 'static,
{
    if file_path.exists() {
        let guard = app_config.file_locks.read_lock(file_path).await;
        match BPlusTreeQuery::<u32, P>::try_new(file_path) {
