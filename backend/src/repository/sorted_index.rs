    ...
    SortKey: for<'de> Deserialize<'de>,
{
    ...
    fn read_value_packed(&mut self, block_offset: u64, value_index: u16) -> io::Result<V> {
        ...
        // Try cache first
        let block_buffer = if let Some(data) = self.block_cache.shift_remove(&block_offset) {
            // Found in cache - move to MRU by re-inserting
            self.block_cache.insert(block_offset, data.clone());
            data
        } else {
            // Miss - read from disk
