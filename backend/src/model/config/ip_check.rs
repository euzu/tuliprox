...
impl From<&IpCheckConfigDto> for IpCheckConfig {
    fn from(dto: &IpCheckConfigDto) -> Self {
        Self {
            ...
            pattern_ipv4: dto.pattern_ipv4.as_ref().and_then(|s| shared::model::REGEX_CACHE.get_or_compile(s).ok()),
            pattern_ipv6:  dto.pattern_ipv6.as_ref().and_then(|s| shared::model::REGEX_CACHE.get_or_compile(s).ok()),
        }
    }
}
