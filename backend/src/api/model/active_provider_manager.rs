...
impl ActiveProviderManager {
    ...
    async fn register_allocation(&self, allocation: ProviderAllocation, addr: &SocketAddr, priority: u8) -> Option<ProviderHandle> {
    ...
    async fn try_preempt_connection(&self, input_name: &Arc<str>, new_priority: u8) -> Option<ProviderAllocation> {
        let mut victim: Option<(ClientConnectionId, AllocationId, u8, Instant)> = None;
                         ...
                         if let Some(conns_map) = connections.single.get(addr) {
                             if let Some(info) = conns_map.get(alloc_id) {
                                 if info.priority < new_priority {
                                     ...
                                     // Rules: Lowest priority, then oldest
                                     let is_better_victim = match victim {
                                         None => true,
                                         Some((_, _, v_prio, v_created)) => {
                                             if info.priority < v_prio {
                                                 true
                                             } else {
                                     ...
                                     if is_better_victim {
                                         victim = Some((*addr, *alloc_id, info.priority, info.created_at));
                                     }
                                 }
                             }
                         }
        ...
        if let Some((addr, alloc_id, v_prio, _)) = victim {
            debug_if_enabled!("Preempting connection from {} (prio={}) for higher priority request (prio={})", sanitize_sensitive_info(&addr.to_string()), v_prio, new_priority);
            
            // Kill it
            let cancel_token = {
                let mut connections = self.connections.write().await;
                if let Some(map) = connections.single.get_mut(&addr) {
                     if let Some(info) = map.get(&alloc_id) {
                         Some(info.cancel_token.clone())
                     } else { None }
                } else { None }
            };
            
            if let Some(token) = cancel_token {
                token.cancel(); // Notify the stream to stop
                
                // We also manually release it here to free the slot immediately for the new request.
                // The stream loop will also call release, but `release_handle` handles idempotency.
                let handle = ProviderHandle {
                     client_id: addr,
                     allocation_id: alloc_id,
                     allocation: ProviderAllocation::Exhausted, // Placeholder, not used for release lookup
                     cancel_token: None
                };
    ...
    pub async fn force_exact_acquire_connection(&self, provider_name: &Arc<str>, addr: &SocketAddr) -> Option<ProviderHandle> {
        // Force acquire always uses max priority (255) effectively, but here we just pass 255 to be safe, 
        // though `force` param in inner overrides checks anyway.
        self.acquire_connection_inner(provider_name, addr, true, None, 255).await
    }

    // Returns the next available provider connection
    pub async fn acquire_connection(&self, input_name: &Arc<str>, addr: &SocketAddr, priority: u8) -> Option<ProviderHandle> {
