...
impl MetadataUpdateManager {
    ...
    async fn process_input_queue(&self, app_state: &Arc<AppState>, input_name: Arc<str>) {
        ...
        loop {
            ...
            // IMPORTANT: use acquire_connection_for_probe with to strictly enforce max_connections limits for background tasks.
            if let Some(handle) = app_state.active_provider.acquire_connection_for_probe(&input_name).await {
    ...
    async fn execute_task(
        ...
        if !input_base.enabled { return Err(shared::error::info_err!("Input {input_name} is disabled")); }

        let input_to_use = allocated_config
