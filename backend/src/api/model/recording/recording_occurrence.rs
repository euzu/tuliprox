...
use chrono_tz::Tz;
use shared::model::recording_rule::{RuleBody, RuleSource, RuleVisibility, RecordingRule};
...
pub fn matches_weekly(rule_body: &RuleBody, candidate_start_utc: i64) -> bool {
    ...
    let Ok(tz) = timezone.parse::<Tz>() else { return false; };
...
mod tests {
    ...
    use shared::model::recording_rule::RuleSource;
    use shared::model::UserId;
