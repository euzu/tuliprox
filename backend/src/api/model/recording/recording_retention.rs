...
use shared::model::recording::RecordingOwner;
...
impl ChannelKey {
    ...
    pub fn from_metadata(channel_id: Option<&str>, channel_name: Option<&str>) -> Self {
        let stable = channel_id
            .map(str::trim)
            .filter(|s| !s.is_empty())
            .map(str::to_string);
        let name_fallback = channel_name
...
fn group_for<V: QuotaRecordingTaskView>(task: &V) -> Option<(RetentionGroupKey, i64)> {
    ...
    if !matches!(task.state(), crate::api::model::download::DownloadState::Completed) {
        return None;
    }
...
pub fn compute_candidates<V: QuotaRecordingTaskView>(
    ...
    for task in tasks {
        ...
        groups
            .entry(key)
            .or_default()
            .push((task.uuid().to_string(), completed_at));
    }
