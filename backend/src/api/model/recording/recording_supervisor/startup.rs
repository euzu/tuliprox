...
async fn reconcile_rule_drift(app_state: &Arc<AppState>) -> usize {
    ...
    let tasks = super::super::recording_rule_scheduler::reconcilable_tasks(app_state.as_ref()).await;
    let now = now_ts();
    ...
    let mut applied = finalize_cancelled_occurrences_in_queue(&actions, &app_state.downloads).await;
