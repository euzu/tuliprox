...
pub fn build_demand_segments(
    candidate: &DemandPoint,
    higher_priority: &[DemandPoint],
) -> Vec<DemandSegment> {
...
pub fn preview_conflict(
    ...
    let higher: Vec<DemandPoint> = others
        .iter()
        .filter(|d| d.priority >= candidate.priority)
        .cloned()
        .collect();
...
mod tests {
    ...
    fn empty_demand_means_no_known_conflict() {
        ...
        let sev = classify(&candidate, &segments, EffectiveCapacity { background_slots: 1, reserved_interactive_slots: 0 });
