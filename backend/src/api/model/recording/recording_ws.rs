...
mod tests {
    ...
    fn denial_distinguishes_a_stale_token_from_a_missing_permission() {
        ...
        assert_eq!(
            recording_view_denial(&no_read_claims()),
            Some(RecordingViewDenial::NotPermitted)
        );
        assert_eq!(
            recording_view_denial(&stale_claims()),
            Some(RecordingViewDenial::TokenRefreshRequired)
        );
    }
    ...
    fn denial_codes_match_the_rest_error_codes() {
        assert_eq!(
            RecordingViewDenial::TokenRefreshRequired.code(),
            "recording_token_refresh_required"
        );
