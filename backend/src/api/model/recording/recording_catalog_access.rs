...
use shared::model::recording::{
    RecordingMetadata, RecordingOwner, RecordingVisibility,
};
use shared::model::{Claims, FileDownloadDto, RecordingTaskDto, UserId, CURRENT_PERMISSION_SCHEMA_VERSION};

use crate::api::model::download::DownloadQueue;
use crate::auth::{
    authorize, RecordingAction, RecordingDecision, RecordingSubject, TerminalState,
};
use shared::model::recording_catalog::{CatalogKey, RecordingCatalogEntry};
...
pub fn authorize_catalog_entry(
    ...
    let recording_meta = RecordingMetadata {
        owner: entry
            .owner_id
            .clone()
            .map_or(RecordingOwner::LegacyAdmin, RecordingOwner::User),
        visibility: entry
            .visibility
            .unwrap_or(RecordingVisibility::Private),
        source: None,
        program_start: None,
        program_end: None,
    ...
    let subject = RecordingSubject::new(
        Some(&recording_meta),
...
pub fn catalog_entry_view(
    ...
    if !can_view_owner_metadata {
        ...
        view = RecordingTaskDto::from_metadata(&recording_meta_stub(
            None,
...
pub fn task_view_for(recording: &crate::api::model::FileDownload) -> FileDownloadDto {
...
mod tests {
    ...
    fn validate_open_rejects_parent_traversal() {
        assert!(matches!(
            validate_open("../escape.ts", true),
            Err(CatalogAccessError::InvalidPath)
        ));
    }
    ...
    fn validate_open_rejects_absolute() {
        assert!(matches!(
            validate_open("/abs/path.ts", true),
            Err(CatalogAccessError::InvalidPath)
        ));
    }
