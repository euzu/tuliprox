...
pub(crate) use super::prepared_terminal_bundle::HlsPreparedTerminalBundleKey as HlsTerminalMediaPreparationKey;
...
pub(crate) const fn bounded_manifest_request_eta_ms(operation_timeout_ms: u64) -> u64 {
...
macro_rules! duration_millis_newtype {
    ($name:ident) => {
        ...
        impl $name {
            pub(crate) const fn from_millis(milliseconds: u64) -> Self { Self(milliseconds) }
        }
    };
}
...
macro_rules! instant_millis_newtype {
    ($name:ident) => {
        ...
        impl $name {
            pub(crate) const fn from_millis_since_epoch(milliseconds: u64) -> Self { Self(milliseconds) }
        }
    };
}
...
impl HlsOperationTimeoutMs {
    #[cfg(test)]
    pub(crate) const fn as_millis(self) -> u64 { self.0 }
}
...
impl HlsAcceptanceDeadlineMs {
    pub(crate) const fn as_millis_since_epoch(self) -> u64 { self.0 }
}
...
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, PartialOrd, Ord)]
pub(crate) struct HlsTerminalCommitAcquisitionBudgetMs(u64);

impl HlsTerminalCommitAcquisitionBudgetMs {
    pub(crate) fn from_retry_policy() -> Self {
        Self(super::terminal_commit::terminal_commit_retry_schedule_budget_ms())
    }

    pub(crate) fn fail_closed_handoff_from_retry_policy() -> Self {
...
pub(crate) enum HlsTerminalCommitWindow {
    NotDue,
    AcquisitionOpen,
    CutoverDue,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsRecoveryBurstWorkload {
    FullBurstPending,
    FullBurstCompleted,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsRecoverySegmentWorkload {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsRecoveryMapWorkload {
    NotRequired,
    Ready,
    Fetch,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsRecoveryObjectReadiness {
    Ready,
    Fetch,
    Staged,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsRecoveryEncryptionReadiness {
    Clear,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct HlsRecoveryMediumReadiness {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct HlsRecoveryWorkload {
...
impl HlsRecoveryWorkload {
    #[cfg(test)]
    pub(crate) const fn clear_fetch() -> Self {
        Self {
    ...
    pub(crate) const fn is_no_greater_than(self, ceiling: Self) -> bool {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct HlsRecoveryWorkloadEnvelope {
    ceiling: HlsRecoveryWorkload,
}

impl HlsRecoveryWorkloadEnvelope {
    pub(crate) const fn acceptance_policy() -> Self {
        Self {
            ceiling: HlsRecoveryWorkload {
                burst: HlsRecoveryBurstWorkload::FullBurstPending,
    ...
    pub(crate) const fn from_timing_ceiling(ceiling: HlsRecoveryWorkload) -> Self { Self { ceiling } }

    pub(crate) const fn ceiling(self) -> HlsRecoveryWorkload { self.ceiling }

    pub(crate) const fn contains(self, workload: HlsRecoveryWorkload) -> bool {
        workload.is_no_greater_than(self.ceiling)
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsTerminalMediaPreparationState {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsTerminalMediaPreparationDisposition {
    Ready,
    Preparing,
    Incompatible,
    Failed,
...
impl HlsTerminalMediaPreparationState {
    #[cfg(test)]
    pub(crate) fn is_ready_for(self, required: HlsTerminalMediaPreparationKey) -> bool {
    ...
    pub(crate) fn authorizes_prepared_key(
        self,
        required: Option<HlsTerminalMediaPreparationKey>,
...
pub(crate) struct HlsObservedRecoveryLatency {
    pub p95: Option<HlsRecoveryEtaMs>,
    pub p99: Option<HlsRecoveryEtaMs>,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct HlsRecoveryTimingPolicy {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct HlsAcceptanceEpisodeTimingSeed {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct HlsAcceptanceEpisodeTiming {
    pub manifest_operation_timeout: HlsOperationTimeoutMs,
    pub media_operation_timeout: HlsOperationTimeoutMs,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct HlsAcceptanceEpisodeTimingInput {
...
impl HlsAcceptanceEpisodeTiming {
    pub(crate) fn from_input(input: &HlsAcceptanceEpisodeTimingInput) -> Self {
    ...
    pub(crate) fn remaining_eta(self, workload: HlsRecoveryWorkload) -> HlsRecoveryEtaMs {
    ...
    pub(crate) fn estimated_completion_at(
        self,
        now_ms: u64,
...
pub(crate) fn manifest_recovery_burst_max_stagger_ms(plan: HlsManifestRecoveryBurstPlan) -> u64 {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct HlsLeaseCutoverTiming {
...
impl HlsLeaseCutoverTiming {
    ...
    pub(crate) const fn with_estimated_recovery_completion_at(
        self,
        estimated_recovery_completion_at: Option<HlsEstimatedRecoveryCompletionAtMs>,
    ) -> Self {
    ...
    pub(crate) fn terminal_commit_window(
        self,
        ...
        let trigger_reserve_ms = self
            .transition_margin
            .as_millis()
            .saturating_add(acquisition_budget.as_millis());
    ...
    #[cfg(test)]
    pub(crate) const fn recovery_may_delay_cutover(self) -> bool {
...
mod tests {
    ...
    fn hls_recovery_timing_candidate_medium_distinguishes_ready_key_from_key_fetch() {
        ...
        let staged_media = HlsRecoveryWorkload::from_recovery_medium(HlsRecoveryMediumReadiness {
            segment: HlsRecoveryObjectReadiness::Staged,
            map: None,
            encryption: HlsRecoveryEncryptionReadiness::Aes128 {
                key: HlsRecoveryObjectReadiness::Ready,
            },
        });
        assert_eq!(
            staged_media.segment,
            HlsRecoverySegmentWorkload::SegmentStagedWithDependenciesReady
        );
    }
    ...
    fn hls_recovery_timing_observations_are_frozen_without_reusing_operation_timeout() {
        ...
        assert_eq!(
            frozen.remaining_eta(frozen.initial_workload),
            frozen.initial_recovery_eta
        );
    ...
    fn hls_terminal_commit_acquisition_window_is_bounded_by_the_retry_policy() {
        ...
        assert_eq!(
            budget.as_millis(),
            super::super::terminal_commit::terminal_commit_retry_schedule_budget_ms()
        );
        ...
        assert_eq!(
            at_trigger.terminal_commit_window(true, false, budget),
            HlsTerminalCommitWindow::AcquisitionOpen
        );
        assert_eq!(
            before_trigger.terminal_commit_window(true, false, budget),
            HlsTerminalCommitWindow::NotDue
        );
        assert_eq!(
            at_margin.terminal_commit_window(true, false, budget),
            HlsTerminalCommitWindow::CutoverDue
        );
        assert_eq!(
            at_trigger.terminal_commit_window(false, false, budget),
            HlsTerminalCommitWindow::NotDue
        );
        assert_eq!(
            at_trigger.terminal_commit_window(true, true, budget),
            HlsTerminalCommitWindow::NotDue
        );
        assert_eq!(
    ...
    fn hls_prepared_terminal_bundle_timing_state_requires_exact_canonical_key() {
        ...
        let other_revision = HlsTerminalMediaPreparationKey {
            asset: HlsTerminalAssetIdentity { revision: 8, ..KEY.asset },
            ..KEY
        };
        let mut fingerprint = KEY.asset.fingerprint;
        fingerprint[0] ^= 0xff;
        let other_fingerprint = HlsTerminalMediaPreparationKey {
        ...
        assert!(!HlsTerminalMediaPreparationState::Incompatible { key: Some(KEY) }
            .authorizes_prepared_key(Some(KEY), KEY));
    }
}
