use super::{
    ...
    runtime_custom_tail::{
        current_hls_runtime_custom_tail_identity, snapshot_hls_runtime_custom_tail_asset,
        HlsRuntimeCustomTailAsset, HlsRuntimeCustomTailAssetIdentity,
...
#[cfg(test)]
use super::prepared_terminal_bundle::build_prepared_terminal_bundle;
use crate::api::model::{is_custom_video_stream_enabled, AppState, HlsTsSpliceAnchor};
use log::{debug, warn};
use std::{future::Future, sync::Arc, time::Duration};

pub(crate) const HLS_PLAYBACK_RATE_GUARD_MILLI: u16 = 1_050;
...
fn evaluate_lease_terminal_cutover(
    ...
    let pressure_policy =
        hls_recovery_pressure_policy(&app_state.hls_proxy, app_state.hls_proxy.origin_manifest_timeout_ms());
...
async fn commit_terminal_cutover(
    context: &HlsLeaseTerminalDecisionContext<'_>,
) -> HlsDetailedTerminalResolution {
    let Some(preparation) = context
        .app_state
        .hls_proxy
        .prepare_access_lease_terminal_tail(context.preparation_request())
        .await
    else {
        return HlsDetailedTerminalResolution::with_deadline(
            HlsTerminalResolution::Reevaluate,
...
pub(crate) struct HlsManifestAcceptanceDirective {
    pub trigger: HlsManifestAcceptanceTrigger,
    pub timing_seed: Option<HlsAcceptanceEpisodeTimingSeed>,
    pub(crate) recovery_pressure_guard: Option<HlsRecoveryPressureGuard>,
    pub(crate) recovery_diagnostic: Option<HlsRecoveryTriggerDiagnostic>,
}
...
pub(crate) enum HlsManifestAcceptanceEvaluationOutcome {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsTerminalFailedClosedReason {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[must_use]
pub(crate) enum HlsTerminalResolution {
    LiveAllowed,
    Committed,
    Reevaluate,
...
pub(crate) fn hls_recovery_timing_policy(
...
fn terminal_media_timing_seed(
    ...
    let responses = app_state.app_config.custom_stream_response.load_full();
    ...
    let preparation = match app_state.hls_proxy.start_prepared_terminal_bundle(
        asset,
        target_duration_ms,
        HLS_TERMINAL_TAIL_SEGMENT_COUNT,
    ) {
...
pub(crate) async fn hls_startup_admission_allows_snapshot(
    app_state: &Arc<AppState>,
...
fn acceptance_directive_from_evidence(
    ...
    let (required_terminal_media_key, terminal_media_preparation) =
        terminal_media_timing_seed(app_state, timing_seed.target_duration_ms);
...
pub(crate) async fn hls_manifest_acceptance_directive_for_session(
    app_state: &Arc<AppState>,
    session: &HlsSessionHandle,
    proxy_session_id: &ProxySessionId,
    ...
    let policy = hls_recovery_pressure_policy(&app_state.hls_proxy, app_state.hls_proxy.origin_manifest_timeout_ms());
    let state_access = retry_availability_state_access(|| {
        app_state.hls_proxy.with_critical_handoff_state(session, |leases, session| {
    ...
    HlsManifestAcceptanceEvaluationOutcome::Evaluated(acceptance_directive_from_evidence(
        app_state, evidence.0, evidence.1,
    ))
}
...
async fn hls_manifest_acceptance_directive_for_reevaluation(
    ...
    if !app_state.hls_proxy.availability_reevaluation_session_is_current(session, owner_key).await {
...
async fn handoff_to_current_availability_reevaluation(
    ...
    let Some(current_session) = app_state
        .hls_proxy
        .sessions()
        .get_by_proxy_session_id(&stale_owner_key.proxy_session_id)
        .await
    else {
        return HlsAvailabilityWorkerHandoff::NoCurrentSession;
    };
    let Some(current_owner_key) = app_state
        .hls_proxy
        .availability_reevaluation_owner_key(&current_session, &stale_owner_key.proxy_session_id)
        .await
    else {
        return HlsAvailabilityWorkerHandoff::NoCurrentSession;
    };
    match ownership.handoff_to(stale_owner_key, current_owner_key.clone()) {
        HlsAvailabilityOwnerHandoffDecision::Continue { owner_key } => HlsAvailabilityWorkerHandoff::Continue {
            session: current_session,
            owner_key,
        },
        HlsAvailabilityOwnerHandoffDecision::AlreadyCurrent => HlsAvailabilityWorkerHandoff::Continue {
            session: current_session,
...
impl HlsAvailabilityAttemptSchedule {
    fn wake_at_ms(
        ...
        match self {
            Self::Backoff => {
                let retry_at_ms =
                    now_ms.saturating_add(availability_reevaluation_backoff_ms(attempts_completed));
...
const fn availability_refresh_trigger_decision(
    ...
    match outcome {
        ...
        HlsOriginRefreshTriggerOutcome::DebouncedUntil { retry_at_ms } => {
            HlsAvailabilityRefreshTriggerDecision::Wait(HlsAvailabilityAttemptSchedule::DebouncedUntil {
                retry_at_ms,
            })
        }
...
fn finish_availability_reevaluation_cycle(
    ...
    match ownership.finish_cycle(owner_key, reason) {
        ...
        | HlsAvailabilityReevaluationFinishDecision::Superseded => HlsAvailabilityWorkerDecision::Stop,
    }
}
...
async fn handoff_availability_reevaluation_worker(
    ...
    match handoff_to_current_availability_reevaluation(app_state, owner_key, ownership).await {
        HlsAvailabilityWorkerHandoff::Continue {
            session: current_session,
            owner_key: current_owner_key,
        } => {
            *session = current_session;
...
async fn handle_evaluated_availability_directive(
    ...
    if !app_state.hls_proxy.availability_reevaluation_session_is_current(session, owner_key).await {
        return handoff_availability_reevaluation_worker(
            app_state,
            session,
            owner_key,
            ownership,
    ...
    match trigger_decision {
        ...
        HlsAvailabilityRefreshTriggerDecision::Handoff => {
            handoff_availability_reevaluation_worker(app_state, session, owner_key, ownership, refresh_request).await
        }
    }
}
...
async fn handle_failed_closed_terminal_reevaluation(
    ...
    let fallback =
        evaluate_owner_failure_fallback(app_state, session, &owner_key.proxy_session_id, now_ms).await;
    ...
    HlsTerminalReevaluationDecision::Continue {
        terminal_evaluation_pending: true,
    }
}
...
async fn evaluate_post_refresh_terminal_attempt(
    ...
    if !matches!(mode, HlsAvailabilityReevaluationMode::PostRefresh(_)) {
        return HlsTerminalReevaluationDecision::Continue {
            terminal_evaluation_pending: false,
        };
    }
    match evaluate_active_terminal_leases_for_reevaluation(
        app_state,
        session,
        &owner_key.proxy_session_id,
        now_ms,
    ...
    .await
    {
        ...
        HlsPostRefreshTerminalEvaluation::FailedClosed(reason) => {
            handle_failed_closed_terminal_reevaluation(
                app_state, session, owner_key, ownership, cycle, reason, now_ms,
            )
            .await
        }
...
async fn evaluate_availability_refresh_attempt(
    ...
    let decision = match hls_manifest_acceptance_directive_for_reevaluation(app_state, session, owner_key).await {
...
async fn run_hls_availability_reevaluation_attempt(
    ...
    let terminal_evaluation_pending =
        match evaluate_post_refresh_terminal_attempt(app_state, session, owner_key, ownership, mode, cycle, attempt_now_ms)
            .await
        {
            HlsTerminalReevaluationDecision::Continue {
                terminal_evaluation_pending,
    ...
    let Some(retry_at_ms) =
        schedule.wake_at_ms(current_time_millis(), cycle.attempts_completed, cycle.deadline_ms)
    else {
        warn!("HLS availability reevaluation cycle stopped: reason=deadline_elapsed");
        return HlsAvailabilityCycleDecision::FinishCycle;
    };
...
pub(crate) fn register_hls_availability_reevaluation(
    app_state: Arc<AppState>,
    session: HlsSessionHandle,
    owner_key: HlsAvailabilityReevaluationOwnerKey,
    refresh_request: OriginRefreshRequest,
...
fn register_hls_availability_reevaluation_with_mode(
    ...
    coordinator.register(registration_key, mode, move |ownership| {
        run_hls_availability_reevaluation(app_state, session, owner_key, ownership, refresh_request)
    })
}
...
pub(crate) async fn commit_terminal_tail_if_lease_reserve_requires_cutover(
    app_state: &Arc<AppState>,
    session: &HlsSessionHandle,
    proxy_session_id: &ProxySessionId,
    lease: &HlsAccessLease,
...
pub(super) async fn commit_terminal_tail_if_lease_reserve_requires_cutover_detailed(
    ...
    .await;
    if state.capacity_recovery_blocks_ready_timeline
        && matches!(purpose, HlsTerminalDecisionPurpose::OrdinaryCutover)
    {
        return HlsDetailedTerminalResolution::resolved(HlsTerminalResolution::LiveAllowed);
    }
    let evaluation = evaluate_lease_terminal_cutover(app_state, lease, manifest, &state, now_ms);
    {
...
async fn commit_prepared_terminal_decision(
    ...
    let asset = match snapshot_hls_runtime_custom_tail_asset(app_state, reason) {
        Ok(asset) => asset,
        Err(compatibility) => {
...
pub(crate) async fn commit_prepared_runtime_custom_tail(
    app_state: &Arc<AppState>,
    session: &HlsSessionHandle,
    proxy_session_id: &ProxySessionId,
    lease_id: &HlsAccessLeaseId,
    ...
    let bundle_state = app_state.hls_proxy.prepared_terminal_bundle_state(bundle_key).unwrap_or_else(|| {
        app_state.hls_proxy.start_prepared_terminal_bundle(
    ...
    let prepared_bundle = match bundle_state {
        ...
        HlsPreparedTerminalBundleState::Preparing { .. } => {
            return register_terminal_pending_owner(
                context,
                media_asset,
                expected_asset,
                bundle_key,
    ...
    let outcome =
        commit_ready_terminal_bundle(context, media_asset, expected_asset, bundle_key, prepared_bundle).await;
...
async fn prepare_ready_terminal_splice(
    ...
    let base_evidence = prepare_terminal_base_evidence(
        context.session,
        context.app_state.hls_proxy.segment_cache(),
    ...
    match plan {
        Ok(plan) => {
            if preparation.required_terminal_media_key == Some(plan.media_preparation_key()) {
                let commit_now_ms = context.app_state.hls_proxy.terminal_commit_now_ms();
    ...
    match reason {
        HlsPreparedTerminalBundleFailure::Build(build_error) => match build_error {
            ...
            | HlsPreparedTerminalBundleBuildError::FiniteSegmentRender(
                crate::api::model::HlsFiniteTsRenderError::InvalidAsset
                | crate::api::model::HlsFiniteTsRenderError::PreparedLayoutMismatch,
            )
...
async fn register_terminal_pending_owner(
    ...
    let ticket = match context.app_state.hls_proxy.observe_prepared_terminal_bundle(bundle_key) {
    ...
    let coordinator = context.app_state.hls_proxy.terminal_pending();
    let task_app_state = Arc::clone(context.app_state);
    ...
    let asset_guard =
        terminal_asset_revision_guard(context.app_state, expected_asset.reason, Some(expected_asset));
    ...
    terminal_resolution_for_pending_registration(
        context,
        expected_asset,
...
fn terminal_pending_owner_key(
    ...
    let session_incarnation = context.app_state.hls_proxy.sessions().session_incarnation(context.session)?;
...
fn register_ready_terminal_pending_owner(
    ...
    let registration = coordinator.register(owner_key, &asset_guard, move |ownership| async move {
        ...
        let now_ms = task_app_state.hls_proxy.terminal_commit_now_ms();
        ...
        let outcome =
            commit_ready_terminal_bundle(task_context, asset, expected_asset, bundle_key, bundle).await;
        if ownership.is_current() {
...
async fn terminal_resolution_after_settled_bundle_observation(
    ...
    match state {
        Some(HlsPreparedTerminalBundleState::Ready { bundle }) if bundle.matches_key_and_shape(bundle_key) => {
            let outcome = commit_ready_terminal_bundle(
                context,
                asset,
                expected_asset,
                bundle_key,
...
async fn run_terminal_pending_owner(
    ...
    let fallback_wait_ms = fallback_commit_at_ms.saturating_sub(app_state.hls_proxy.terminal_commit_now_ms());
    ...
    let now_ms = app_state.hls_proxy.terminal_commit_now_ms();
    let context = HlsTerminalCommitContext {
        app_state: &app_state,
        session: &session,
...
fn commit_terminal_unavailable(
    ...
    let manager = &context.app_state.hls_proxy;
    ...
    loop {
        ...
        expected_asset = configured_terminal_asset_identity(context.app_state, context.preparation.trigger.reason());
...
fn commit_prepared_terminal_unavailable_after_owner_failure(
    ...
    let expected_asset =
        configured_terminal_asset_identity(app_state, HlsRuntimeCustomTailReason::ChannelUnavailable);
    let outcome = app_state
        ...
        .commit_access_lease_terminal_if_generation_matches(HlsTerminalCommitRequest {
            ...
            proxy_session_id,
            preparation,
            now_ms: app_state.hls_proxy.terminal_commit_now_ms().max(now_ms),
...
fn configured_terminal_asset_identity(
    ...
    current_hls_runtime_custom_tail_identity(app_state, reason)
}
...
fn terminal_asset_revision_guard(
    ...
    let custom_stream_response = Arc::clone(&app_state.app_config.custom_stream_response);
    let custom_video_stream_enabled = Arc::clone(&app_state.app_config.config);
    HlsTerminalAssetRevisionGuard::for_optional_runtime_tail(
        reason,
        expected,
        move || {
            ...
            .as_ref()
                .and_then(|responses| match reason {
                    ...
                    HlsRuntimeCustomTailReason::UserConnectionsExhausted => {
                        responses.user_connections_exhausted.as_ref()
                    }
                    ...
                    HlsRuntimeCustomTailReason::SessionOrLeaseExpired => {
                        responses.hls_session_or_lease_expired.as_ref()
                    }
                })
...
mod tests {
    use super::{
        super::{
            ...
            runtime_custom_tail::{
                commit_hls_runtime_custom_tail, snapshot_hls_runtime_custom_tail_asset,
                HlsRuntimeCustomTailOutcome, HlsRuntimeCustomTailRequest,
            },
    ...
    use crate::{
        api::model::{create_test_app_state, HlsTsSpliceAnchor, TransportStreamBuffer},
        model::{Config, CustomStreamResponse},
    ...
    fn availability_refresh_waits_for_completion_instead_of_polling_in_flight() {
        for outcome in [
            ...
            assert_eq!(
                ...
                HlsAvailabilityRefreshTriggerDecision::Wait(
                    HlsAvailabilityAttemptSchedule::RefreshCompletion
                )
            );
        }
        assert_eq!(
            HlsAvailabilityAttemptSchedule::RefreshCompletion.wake_at_ms(100, 1, 2_100),
            Some(2_100)
        );
    }
    ...
    fn availability_refresh_waits_until_concrete_debounce_boundary() {
        ...
        assert_eq!(
            HlsAvailabilityAttemptSchedule::DebouncedUntil { retry_at_ms: 3_000 }
                .wake_at_ms(100, 1, 2_100),
            Some(2_100)
        );
    }
    ...
    fn runtime_custom_responses_with_low_priority(low_priority_bytes: &[u8]) -> Arc<CustomStreamResponse> {
        Arc::new(CustomStreamResponse {
            ...
            provider_connections_exhausted: Some(TransportStreamBuffer::new(
                PROVIDER_EXHAUSTED_ASSET_BYTES.to_vec(),
            )),
            ...
            hls_session_or_lease_expired: Some(TransportStreamBuffer::new(
                SESSION_EXPIRED_ASSET_BYTES.to_vec(),
            )),
    ...
    async fn disabled_custom_responses_do_not_seed_terminal_media_preparation() {
        ...
        app_state.app_config.custom_stream_response.store(Some(runtime_custom_responses()));

        assert_eq!(
            terminal_media_timing_seed(&app_state, 10_000),
    ...
    impl HlsTerminalPendingCommitFixture {
        ...
        fn ready_bundle(&self) -> Arc<HlsPreparedTerminalBundle> {
            build_prepared_terminal_bundle(&self.asset, self.bundle_key)
                .expect("fixture relative terminal bundle")
        }

        fn register_owner(&self, ticket: HlsPreparedTerminalBundleCompletionTicket) -> oneshot::Receiver<()> {
            let coordinator = self.app_state.hls_proxy.terminal_pending();
            ...
            let asset_guard = terminal_asset_revision_guard(
                &self.app_state,
                expected_asset.reason,
                Some(expected_asset),
            );
    ...
    async fn terminal_pending_commit_fixture_with_base(
        ...
        let (session, _) = app_state
            .hls_proxy
            .get_or_create_session_with_outcome(HlsSessionKey::new(1, name), b"secret", now_ms)
            .await;
        let proxy_session_id = session.read().await.proxy_session_id.clone();
        let lease_id = HlsAccessLeaseId(name.to_string());
        install_terminal_pending_ready_base(
            &app_state,
            &session,
            &proxy_session_id,
            &asset,
        ...
        .await;
        publish_terminal_pending_lease(
            &app_state,
            &proxy_session_id,
            &lease_id,
            name,
    ...
    async fn missing_terminal_base_timestamp_profile_commits_terminal_unavailable() {
        ...
        let fixture =
            terminal_pending_commit_fixture_with_base("pending-owner-missing-timestamp", &base_bytes).await;
        let bundle = fixture.ready_bundle();
        let (ticket, publisher) = prepared_terminal_bundle_completion_channel_for_test(fixture.bundle_key);
        let completed = fixture.register_owner(ticket);
    ...
    async fn hls_terminal_commit_pending_owner_store_progress_supersession_keeps_lease_live() {
        ...
        fixture.app_state.hls_proxy.cancel_superseded_terminal_work_for_session(&fixture.proxy_session_id);
        publisher.publish(HlsPreparedTerminalBundleCompletion::Ready { bundle });
    ...
    async fn capacity_deferred_ready_boundary_keeps_affected_lease_live() {
        ...
        session.segments.get_mut(&1).expect("second segment").status = SegmentCacheStatus::CapacityDeferred {
    ...
    fn post_refresh_owner_request(fixture: &PostRefreshTerminalFixture) -> OriginRefreshRequest {
        ...
        OriginRefreshRequest {
            app_config: Arc::clone(&fixture.app_state.app_config),
            ...
            segment_cache: Arc::clone(fixture.app_state.hls_proxy.segment_cache()),
            hls_proxy: Arc::clone(&fixture.app_state.hls_proxy),
            segment_repair: Arc::clone(fixture.app_state.hls_proxy.segment_repair()),
            segment_worker_pool: Arc::clone(fixture.app_state.hls_proxy.segment_worker_pool()),
            map_worker_pool: Arc::clone(fixture.app_state.hls_proxy.map_worker_pool()),
            origin_manifest_timeout_ms: fixture.app_state.hls_proxy.origin_manifest_timeout_ms(),
            manifest_recovery_burst: fixture.app_state.hls_proxy.manifest_recovery_burst(),
            strip: fixture.app_state.hls_proxy.strip(),
            ...
            now_ms: fixture.now_ms,
            origin_io: None,
            post_refresh_runtime: Some(super::super::refresh::HlsPostRefreshRuntime {
                app_state: Arc::downgrade(&fixture.app_state),
            }),
        }
    }
    ...
    fn assert_availability_owner_registered(fixture: &PostRefreshTerminalFixture) {
        assert_eq!(fixture.app_state.hls_proxy.availability_reevaluations().owner_count(), 1);
    }
    ...
    async fn assert_post_refresh_owner_checks_refresh_gate_once(fixture: &PostRefreshTerminalFixture) {
        let refresh_skipped_before = fixture.app_state.hls_proxy.metrics().snapshot().refresh_skipped;
        ...
        .await;
        for _ in 0..256 {
            if fixture.app_state.hls_proxy.metrics().snapshot().refresh_skipped
        ...
        assert_eq!(
            fixture.app_state.hls_proxy.metrics().snapshot().refresh_skipped,
            refresh_skipped_before.saturating_add(1),
    ...
    async fn prepare_post_refresh_terminal_base(
        ...
        if prepare_terminal_bundle {
            ...
            let state = match state {
                HlsPreparedTerminalBundleState::Preparing { .. } => {
                    app_state.hls_proxy.wait_for_prepared_terminal_bundle(key).await
                }
                HlsPreparedTerminalBundleState::Ready { .. }
    ...
    async fn publish_post_refresh_terminal_lease(
        ...
        assert!(app_state
            ...
            .is_activated());
        assert!(
            app_state.hls_proxy.access_lease_response_snapshot(&lease_id, proxy_session_id, now_ms).await.is_some(),
    ...
    async fn post_refresh_terminal_fixture_with_bundle_state(
        ...
        PostRefreshTerminalFixture { app_state, session, proxy_session_id, lease_id, now_ms }
    }

    async fn prepare_runtime_custom_bundle(
        fixture: &PostRefreshTerminalFixture,
        reason: HlsRuntimeCustomTailReason,
    ) {
        let lease = fixture
        ...
        let asset = snapshot_hls_runtime_custom_tail_asset(&fixture.app_state, reason)
            .expect("configured runtime custom-tail asset");
    ...
    async fn wait_for_runtime_custom_plan(
        fixture: &PostRefreshTerminalFixture,
    ...
    fn configured_runtime_custom_buffer(
        ...
        let responses = fixture
            ...
            .custom_stream_response
            .load_full()
            .expect("runtime custom responses");
        match reason {
            HlsRuntimeCustomTailReason::ChannelUnavailable => responses.channel_unavailable.as_ref(),
            HlsRuntimeCustomTailReason::LowPriorityPreempted => responses.low_priority_preempted.as_ref(),
    ...
    fn payload_continuity_bounds(bytes: &[u8]) -> std::collections::HashMap<u16, (u8, u8, bool)> {
        ...
        for packet in bytes.as_chunks::<188>().0.iter().filter(|packet| packet[0] == 0x47) {
            ...
            payload_bounds
                .entry(pid)
                .and_modify(|entry| entry.1 = counter)
                .or_insert((counter, counter));
        }
        payload_bounds
            .into_iter()
            .map(|(pid, (first, last))| {
    ...
    async fn unsafe_live_transport_evidence_commits_unavailable_without_terminal_bytes() {
        ...
        tokio::fs::write(
            &metadata.path,
            with_internal_payload_continuity_jump(TERMINAL_ASSET_BYTES),
        )
        .await
        ...
        assert!(fixture
            ...
            .read()
            .await
            .terminal_tail_protection(&fixture.lease_id)
            .is_none());
    }
    ...
    async fn preemption_tail_uses_low_priority_asset_not_channel_unavailable_asset() {
        ...
        let low_priority = snapshot_hls_runtime_custom_tail_asset(
            &fixture.app_state,
            HlsRuntimeCustomTailReason::LowPriorityPreempted,
        )
        .expect("low-priority asset");
        let channel = snapshot_hls_runtime_custom_tail_asset(
            &fixture.app_state,
            HlsRuntimeCustomTailReason::ChannelUnavailable,
        )
    ...
    async fn preemption_tail_preserves_live_to_custom_pts_dts_pcr_and_cc() {
        ...
        let low_priority =
            configured_runtime_custom_buffer(&fixture, HlsRuntimeCustomTailReason::LowPriorityPreempted);
        ...
        assert_eq!(
            second_profile
                ...
                .wrapping_sub(first_profile.first_clock_90khz)
                % (1_u64 << 33),
            902_400
        );
        ...
        assert!(first_cc
            .iter()
            .all(|(pid, (_, _, discontinuity))| *pid == 0x1fff || *discontinuity), "{first_cc:?}");
    ...
    async fn preemption_tail_is_committed_at_safe_segment_boundary_without_waiting_for_reserve_cutover() {
        let fixture =
            post_refresh_terminal_fixture_with_progress("runtime-preemption-immediate", true, false).await;
    ...
    async fn preemption_does_not_fetch_another_origin_manifest() {
        ...
        let _ = commit_runtime_custom_reason(
            &fixture,
            HlsRuntimeCustomTailReason::LowPriorityPreempted,
            true,
        )
    ...
    async fn assert_active_policy_reason_commits(
        name: &str,
        reason: HlsRuntimeCustomTailReason,
    ) {
        ...
        assert_eq!(
            ...
            HlsRuntimeCustomTailAssetIdentity::from_asset(
                &snapshot_hls_runtime_custom_tail_asset(&fixture.app_state, reason)
    ...
    async fn different_late_reason_cannot_replace_committed_plan() {
        ...
        let (outcome, replay) = commit_runtime_custom_reason(
            &fixture,
            HlsRuntimeCustomTailReason::SessionOrLeaseExpired,
            false,
        )
    ...
    async fn wait_for_terminal_pending_owners(
        fixture: &PostRefreshTerminalFixture,
        expected: usize,
    ) {
        tokio::time::timeout(Duration::from_secs(10), async {
            loop {
                if fixture.app_state.hls_proxy.terminal_pending().owner_count() == expected {
                    return;
                }
                tokio::task::yield_now().await;
            }
    ...
    async fn asset_reload_supersedes_pending_custom_tail_but_not_committed_bytes() {
        ...
        let old_asset =
            snapshot_hls_runtime_custom_tail_asset(&fixture.app_state, reason).expect("old low-priority asset");
        ...
        *revised_bytes.last_mut().expect("non-empty low-priority asset") ^= 1;
        ...
        fixture
            ...
            .app_config
            .custom_stream_response
            .store(Some(runtime_custom_responses()));
        let replay = wait_for_runtime_custom_plan(&fixture).await;
    ...
    async fn same_reason_singleflight_has_one_media_finalizer() {
        ...
        let asset = snapshot_hls_runtime_custom_tail_asset(&fixture.app_state, reason).expect("singleflight asset");
        let target_duration_ms = fixture
            .app_state
            .hls_proxy
            .access_lease_response_snapshot(&fixture.lease_id, &fixture.proxy_session_id, fixture.now_ms)
        ...
        let (first, second) = tokio::join!(
            commit_hls_runtime_custom_tail(Arc::clone(&fixture.app_state), request()),
            commit_hls_runtime_custom_tail(Arc::clone(&fixture.app_state), request()),
        );
        ...
        assert_eq!(fixture.app_state.hls_proxy.terminal_pending().owner_count(), 1);
    ...
    async fn add_active_post_refresh_lease(
        ...
        fixture.app_state.hls_proxy.prepare_access_lease(lease).await;
        let publication = fixture
            .app_state
            .hls_proxy
            .prepare_access_lease_manifest_publication(
                &lease_id,
                &fixture.proxy_session_id,
                fixture.now_ms,
            )
        ...
        Arc::make_mut(&mut manifest.visible_segments)[0].uri = format!(
            "/hls/shared/live/{}/{}/0.ts",
            fixture.proxy_session_id.0, lease_id.0
        );
        assert!(fixture
            .app_state
            .hls_proxy
    ...
    async fn debounced_post_refresh_owner_does_not_poll_refresh_gate() {
        ...
        fixture.session.write().await.origin_refresh.next_fetch_allowed_at_ms =
            fixture.now_ms.saturating_add(10_000);
        assert_post_refresh_owner_checks_refresh_gate_once(&fixture).await;
    }
    ...
    async fn persistent_conflict_real_owner_commits_before_exclusive_deadline() {
        ...
        assert_eq!(
            fixture.app_state.hls_proxy.availability_reevaluations().owner_count(),
            1,
    ...
    async fn failed_closed_retry_capacity_retains_owner() {
        ...
        fixture.app_state.hls_proxy.set_terminal_commit_retry_capacity_for_test(0);
        register_real_post_refresh_owner(
            &fixture,
            super::super::refresh::HlsPostRefreshAvailabilityReason::HardManifestFailure,
        )
        ...
        fixture.app_state.hls_proxy.set_terminal_commit_retry_capacity_for_test(
            super::super::terminal_commit::HLS_TERMINAL_COMMIT_RETRY_CAPACITY,
        );
        fixture.app_state.hls_proxy.notify_session_evidence_changed(&fixture.proxy_session_id);
        wait_for_availability_owner_completion(&fixture).await;
        let lease = fixture
            .app_state
            .hls_proxy
    ...
    async fn failed_closed_lock_contention_retains_owner() {
        ...
        let lease_guard = fixture.app_state.hls_proxy.hold_access_lease_store_for_test().await;
        assert_eq!(
            register_hls_availability_reevaluation_with_mode(
                Arc::clone(&fixture.app_state),
                Arc::clone(&fixture.session),
    ...
    async fn assert_multi_lease_fallback_handles_pending_and_unavailable(reverse_insertion: bool) {
        ...
        if reverse_insertion {
            let mut leases = fixture.app_state.hls_proxy.hold_access_lease_store_for_test().await;
        ...
        let evaluation_now_ms = fixture.app_state.hls_proxy.terminal_commit_now_ms();
        ...
        fixture.app_state.hls_proxy.terminal_pending().cancel_session(&fixture.proxy_session_id);
    }
