use super::{
    ...
    HlsTrackEvidenceResolution, HlsTsProbeBudget, HlsTsProbeProtection, HlsTsSpliceEvidence,
    HlsTsTrackSignature, TransientObjectCacheKey, TransientPassthroughState, TransientResourceFile,
    TransientResourceKind,
};
use crate::{
    api::model::{HlsTsTimestampProfile, TransportStreamBuffer},
...
pub(crate) const HLS_TERMINAL_TAIL_SEGMENT_COUNT: u16 = 12;
...
#[derive(Clone)]
pub(crate) struct HlsTerminalKeyBinding {
...
impl HlsTerminalKeyBinding {
    ...
    pub(crate) fn content_type(&self) -> &str { &self.content_type }

    pub(crate) fn bytes(&self) -> Bytes { Bytes::copy_from_slice(self.material.as_bytes()) }

    #[cfg(test)]
    pub(crate) fn source_cache_key(&self) -> &TransientObjectCacheKey { &self.source_cache_key }

    #[cfg(test)]
    pub(crate) fn resource_id(&self) -> &TransientResourceId { &self.resource_id }
}
...
pub(crate) struct HlsTerminalTailGeneration(pub(crate) u64);

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsMediaContainer {
    MpegTs,
    FragmentedMp4,
    Unknown,
}
...
pub(crate) struct HlsMapSignature {
    pub fingerprint: [u8; 32],
    pub container: HlsMediaContainer,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct HlsEncryptionSignature {
    pub method: String,
...
#[derive(Debug, Clone)]
pub(crate) struct HlsTerminalMediaAsset {
...
impl HlsTerminalMediaAsset {
    ...
    pub(crate) fn container(&self) -> HlsMediaContainer { self.validated.container }

    pub(crate) fn track_signature(&self) -> &HlsTsTrackSignature { &self.validated.track_signature }

    pub(crate) fn duration_ms(&self) -> u64 { self.validated.duration_ms }

    pub(crate) fn duration_ticks_90khz(&self) -> u64 { self.validated.duration_ticks_90khz }

    pub(crate) fn timestamp_profile(&self) -> Option<HlsTsTimestampProfile> {
        self.validated.timestamp_profile
    }

    pub(crate) fn content_type(&self) -> &'static str { self.validated.content_type }
...
pub(crate) fn snapshot_terminal_media_asset(
...
pub(crate) fn terminal_media_asset_identity(
    buffer: &TransportStreamBuffer,
) -> Option<HlsTerminalAssetIdentity> {
    ...
    Some(terminal_asset_identity_from_fingerprint(
        buffer.finite_hls_asset_fingerprint(),
    ))
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub(crate) struct HlsTerminalAssetIdentity {
    pub revision: u64,
    pub fingerprint: [u8; 32],
}

impl HlsTerminalAssetIdentity {
    pub(crate) fn from_asset(asset: &HlsTerminalMediaAsset) -> Self { asset.identity() }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsTerminalBaseMediaState {
    Ready,
    NotReady,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsTerminalBaseProtection {
    Protectable,
    Unavailable,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct HlsTerminalBaseSegmentAvailability {
    pub proxy_seq: u64,
...
pub(crate) struct HlsTerminalBaseEvidence {
...
impl HlsTerminalBaseEvidence {
    pub(crate) fn availability(&self) -> Arc<[HlsTerminalBaseSegmentAvailability]> { Arc::clone(&self.availability) }

    pub(crate) fn track_signature(&self) -> Option<HlsTsTrackSignature> {
        self.track_resolution.as_ref().and_then(HlsTrackEvidenceResolution::signature).cloned()
    }

    pub(crate) fn track_resolution(&self) -> Option<&HlsTrackEvidenceResolution> {
        self.track_resolution.as_ref()
    }

    pub(crate) fn splice_evidence(&self) -> Option<&HlsTsSpliceEvidence> {
        self.splice_evidence.as_ref()
    }

    pub(crate) fn track_evidence_reason_code(&self) -> &'static str {
        self.track_resolution
            .as_ref()
            .map_or("base-not-ready", HlsTrackEvidenceResolution::reason_code)
    }

    pub(crate) fn track_base(&self) -> Option<&HlsTerminalBaseTrackIdentity> { self.track_base.as_ref() }

    pub(crate) fn timing(&self) -> Option<&HlsTerminalBaseTimingEvidence> { self.timing.as_ref() }

    pub(crate) fn key_bindings(&self) -> Arc<[HlsTerminalKeyBinding]> { Arc::clone(&self.key_bindings) }

    /// Makes the intended guard lifetime explicit at the endpoint commit boundary.
    #[cfg(test)]
    pub(crate) fn release(self) { drop(self.read_protection) }
    ...
    pub(crate) fn into_commit_guard(self) -> HlsTerminalCommitMediaGuard {
...
pub(crate) struct HlsTerminalCommitMediaGuard {
    read_protection: HlsTerminalBaseReadProtection,
}
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct HlsTerminalBaseTrackIdentity {
    pub proxy_seq: u64,
...
pub(crate) struct HlsTerminalBaseTimingEvidence {
...
pub(crate) async fn prepare_terminal_base_evidence(
...
pub(crate) fn pin_terminal_base_evidence(
...
pub(crate) async fn resolve_terminal_base_evidence(
    segment_cache: &HlsSegmentCache,
    manifest: &HlsLeaseManifestSnapshot,
...
pub(crate) struct HlsTerminalBaseEvidencePreparation {
...
fn snapshot_terminal_base_probes(
    ...
    let probes = manifest
        ...
        .map(|segment| {
            ...
            if let HlsTerminalBaseKeyEvidence::Aes128 {
                cache_key,
                object_access,
                resource_access,
                ..
...
const fn terminal_base_media_state(media_ready: bool, protectable: bool) -> HlsTerminalBaseMediaState {
    if media_ready && protectable { HlsTerminalBaseMediaState::Ready } else { HlsTerminalBaseMediaState::NotReady }
}

const fn terminal_base_protection(protectable: bool) -> HlsTerminalBaseProtection {
    if protectable { HlsTerminalBaseProtection::Protectable } else { HlsTerminalBaseProtection::Unavailable }
}
...
enum HlsTerminalBaseKeyEvidence {
    ...
    Unavailable { resolution: HlsTrackEvidenceResolution },
}
...
fn terminal_base_key_evidence(
    ...
    let iv = match super::hls_aes128_cbc_iv(
...
#[derive(Clone)]
pub(crate) struct HlsTerminalTailBuildInput {
...
impl HlsTerminalTailBuildInput {
    pub(crate) fn anchored_bundle_for_test(
        ...
        let anchor = crate::api::model::HlsTsSpliceAnchor::between(profile, profile)
    ...
    pub(crate) fn base_timing_for_test(
        asset: &HlsTerminalMediaAsset,
        manifest: &HlsLeaseManifestSnapshot,
    ...
    pub(crate) fn compatible_splice_evidence_for_test(
        asset: &HlsTerminalMediaAsset,
    ) -> HlsTsSpliceEvidence {
...
#[derive(Clone, Copy)]
pub(crate) struct HlsTerminalTailCompatibilityInput<'a> {
...
#[derive(Clone, Copy)]
pub(crate) enum HlsTerminalTailBoundaryEvidence<'a> {
    StructuralOnly,
    Exact {
        base: Option<&'a HlsTsSpliceEvidence>,
        terminal: Option<&'a HlsTsSpliceEvidence>,
    },
}

#[derive(Clone)]
pub(crate) struct HlsTerminalTailPlan {
    pub generation: HlsTerminalTailGeneration,
    pub created_at_ms: u64,
    pub base_manifest: HlsLeaseManifestSnapshot,
...
impl HlsTerminalTailPlan {
    pub(crate) fn media_preparation_key(&self) -> super::recovery_timing::HlsTerminalMediaPreparationKey {
        self.anchored_bundle.prepared_key
    }

    pub(crate) fn matches_route(&self, proxy_session_id: &ProxySessionId, lease_id: &HlsAccessLeaseId) -> bool {
        self.route_binding.proxy_session_id == *proxy_session_id && self.route_binding.lease_id == *lease_id
    }

    pub(crate) fn segment_content_length(&self, path: HlsTerminalSegmentPath) -> Option<u64> {
        self.prepared_segment(path).and_then(|segment| u64::try_from(segment.bytes.len()).ok())
    }

    pub(crate) fn segment_content_type(&self) -> &'static str { self.segment_content_type }

    /// Clones one immutable pre-rendered segment only for this plan generation.
    pub(crate) fn segment_bytes(&self, path: HlsTerminalSegmentPath) -> Option<Bytes> {
    ...
    pub(crate) fn terminal_key_binding(
        &self,
        proxy_session_id: &ProxySessionId,
        lease_id: &HlsAccessLeaseId,
...
pub(crate) enum HlsLeasePlaybackMode {
    #[default]
    Live,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsTerminalTailCompatibility {
    Compatible,
    MissingAsset,
...
impl HlsTerminalSpliceDiagnostic {
    fn from_compatibility(compatibility: HlsTerminalTailCompatibility) -> Option<Self> {
        ...
        match compatibility {
            ...
            | HlsTerminalTailCompatibility::SpliceTopologyMismatch => {
                Some(empty("topology-mismatch"))
            }
            HlsTerminalTailCompatibility::SpliceTransportFailure(reason) => {
                ...
                match reason {
                    ...
                    | super::HlsTsSpliceIncompatibility::TransportError {
                        packet_index,
                        ..
                    } => {
                    ...
                    super::HlsTsSpliceIncompatibility::ContinuityFailure {
                        pid,
                        packet_index,
                        expected,
                        actual,
...
fn log_terminal_splice_compatibility(
    base_proxy_seq: u64,
    compatibility: HlsTerminalTailCompatibility,
) {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct HlsTerminalSegmentPath {
    pub generation: HlsTerminalTailGeneration,
    pub index: u16,
}

impl HlsTerminalSegmentPath {
    pub(crate) fn parse(generation: &str, terminal_file: &str) -> Option<Self> {
...
pub(crate) enum HlsTerminalTailRenderError {
...
pub(crate) fn evaluate_terminal_tail_compatibility(
...
pub(crate) fn build_terminal_tail_plan(
    ...
    if !input
        .anchored_bundle
        .matches_key_and_shape(expected_bundle_key, input.asset.duration_ticks_90khz())
    {
    ...
    let Some(expected_anchor) =
        crate::api::model::HlsTsSpliceAnchor::between(base_timing.profile, asset_profile)
    else {
        return Err(HlsTerminalTailCompatibility::InvalidTimestampTransition);
    };
...
pub(crate) fn terminal_tail_manifest_body<'a>(
...
mod tests {
    ...
    async fn unresolved_aes_terminal_base_evidence(iv: &str) -> HlsTerminalBaseEvidence {
        ...
        let mut session = super::super::HlsSession::new(
            super::super::HlsSessionKey::new(1, "terminal-evidence"),
            b"secret",
            1,
        );
        let proxy_seq = 194;
    ...
    async fn terminal_timing_session(
        ...
        let first_cache_key =
            super::super::SegmentCacheKey::new(proxy_session_id.clone(), first_proxy_seq, "ts");
        let last_cache_key =
            super::super::SegmentCacheKey::new(proxy_session_id.clone(), last_proxy_seq, "ts");
        cache
            .write_bytes_and_commit(&first_cache_key, first_bytes)
            .await
            .expect("first segment commits");
        cache
            .write_bytes_and_commit(&last_cache_key, last_bytes)
            .await
            .expect("last segment commits");
        ...
        base.visible_segments = Arc::from(
            [first_proxy_seq, last_proxy_seq].map(|proxy_seq| HlsLeaseManifestSegment {
                proxy_seq,
                duration_ms: terminal_asset.duration_ms(),
    ...
    async fn terminal_base_preserves_invalid_iv_and_missing_key_evidence() {
        ...
        let missing_key =
            unresolved_aes_terminal_base_evidence("0x00000000000000000000000000000384").await;
        assert!(missing_key.track_base().is_some());
        assert_eq!(
            missing_key.track_resolution(),
            Some(&HlsTrackEvidenceResolution::KeyUnavailable)
        );
    ...
    fn hls_terminal_response_manifest_keeps_live_tail_and_appends_finite_endlist_tail() {
        ...
        assert_eq!(
            terminal_media
                .matches(&format!("#EXTINF:{},", format_hls_duration_ms(asset.duration_ms())))
                .count(),
            usize::from(HLS_TERMINAL_TAIL_SEGMENT_COUNT)
        );
        assert_eq!(
    ...
    fn hls_terminal_response_rejects_bundle_for_a_different_target_duration() {
        ...
        assert_eq!(
            ...
            Err(HlsTerminalTailCompatibility::AssetRevisionMismatch)
        );
    }
    ...
    fn unsafe_transport_evidence_cannot_commit_or_expose_any_runtime_custom_tail() {
        ...
        for reason in [
            ...
            input.expected_asset = HlsRuntimeCustomTailAssetIdentity {
                reason,
                media: HlsTerminalAssetIdentity::from_asset(&asset),
            };
    ...
    fn terminal_plan_rejects_missing_or_topologically_different_exact_evidence() {
        ...
        assert_eq!(
            build_terminal_tail_plan(missing),
            Err(HlsTerminalTailCompatibility::MissingSpliceEvidence)
        );
        ...
        assert_eq!(
            build_terminal_tail_plan(different),
            Err(HlsTerminalTailCompatibility::SpliceTopologyMismatch)
        );
    }
    ...
    fn terminal_splice_diagnostic_preserves_typed_boundary_fields() {
        ...
        assert_eq!(
            HlsTerminalSpliceDiagnostic::from_compatibility(
                HlsTerminalTailCompatibility::SpliceTopologyMismatch
            )
