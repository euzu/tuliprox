use super::{
    ...
    refresh::HlsPostRefreshAvailabilityReason,
...
pub(crate) struct HlsAvailabilityReevaluationOwnerKey {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct HlsRecoveryPressureGuard {
    pub session_incarnation: HlsSessionIncarnation,
    pub proxy_session_id: ProxySessionId,
    pub origin_progress_generation: u64,
    pub media_readiness_generation: u64,
...
impl HlsRecoveryPressureGuard {
    pub(crate) fn from_owner_key(owner_key: &HlsAvailabilityReevaluationOwnerKey) -> Self {
        Self {
...
pub(crate) enum HlsRecoveryPressureGuardAccess<R> {
    Acquired(R),
    Superseded,
    LockBusy,
}
...
#[must_use]
pub(crate) enum HlsAvailabilityReevaluationRegistration {
    Scheduled,
    AlreadyOwned,
    Superseded,
    CapacityExceeded,
...
pub(crate) enum HlsAvailabilityReevaluationMode {
    RecoveryPressure,
...
#[derive(Debug, Clone, PartialEq, Eq)]
#[must_use]
pub(crate) enum HlsAvailabilityOwnerHandoffDecision {
...
pub(crate) struct HlsAvailabilityReevaluationCoordinator {
...
impl HlsAvailabilityReevaluationCoordinator {
    ...
    pub(crate) fn register<Work, WorkFuture>(
        self: &Arc<Self>,
        key: HlsAvailabilityReevaluationOwnerKey,
        mode: HlsAvailabilityReevaluationMode,
        work: Work,
    ...
    ) -> Option<HlsAvailabilityReevaluationObserver> {
        self.lock_state()
            .owners
            .get(proxy_session_id)
            .map(|owner| HlsAvailabilityReevaluationObserver {
                receiver: owner.observer_tx.subscribe(),
            })
    }
    ...
    fn current_mode(
        ...
        self.lock_state().owners.get(&key.proxy_session_id).and_then(|owner| {
            (owner.owner_token == owner_token && owner.key == *key).then_some(owner.mode)
        })
    }
    ...
    fn abandon(&self, proxy_session_id: &ProxySessionId, owner_token: u64) {
        ...
        if state
            .owners
            .get(proxy_session_id)
            .is_some_and(|owner| owner.owner_token == owner_token)
        {
    ...
    #[cfg(test)]
    pub(crate) fn owner_count(&self) -> usize { self.lock_state().owners.len() }

    #[cfg(test)]
    pub(crate) fn available_task_permits_for_test(&self) -> usize { self.task_capacity.available_permits() }
...
pub(crate) struct HlsAvailabilityReevaluationObserver {
    receiver: watch::Receiver<u64>,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum HlsAvailabilityReevaluationObservation {
    EvidenceChanged,
    OwnerFinished,
}

impl HlsAvailabilityReevaluationObserver {
    pub(crate) async fn changed(&mut self) -> HlsAvailabilityReevaluationObservation {
...
#[derive(Clone)]
pub(crate) struct HlsAvailabilityReevaluationOwnership {
...
impl HlsAvailabilityReevaluationOwnership {
    pub(crate) fn is_current(&self, owner_key: &HlsAvailabilityReevaluationOwnerKey) -> bool {
        !self.cancellation.is_cancelled() && self.coordinator.is_current(owner_key, self.owner_token)
    }

    pub(crate) async fn cancelled(&self) { self.cancellation.cancelled().await }
    ...
    pub(crate) async fn wake_requested(&self) { self.wake.notified().await }
    ...
    pub(crate) fn discard_superseded(&self, owner_key: &HlsAvailabilityReevaluationOwnerKey) {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[must_use]
pub(crate) enum HlsAvailabilityReevaluationFinishReason {
    Evaluated,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[must_use]
pub(crate) enum HlsAvailabilityReevaluationFinishDecision {
    Complete,
    StartSuccessor,
    Superseded,
}
...
impl Drop for HlsAvailabilityReevaluationCompletion {
    fn drop(&mut self) {
        ...
        self.ownership
            .coordinator
            .abandon(&self.ownership.proxy_session_id, self.ownership.owner_token);
    }
}
...
mod tests {
    ...
    async fn hls_availability_reevaluation_coalesces_same_generation_pressure_without_lost_wakeup() {
        ...
        let registration = {
            ...
            coordinator.register(owner_key.clone(), HlsAvailabilityReevaluationMode::RecoveryPressure, move |ownership| async move {
                loop {
    ...
    async fn availability_owner_evidence_change_wakes_every_request_observer() {
        ...
        let task_key = owner_key.clone();
        assert_eq!(
            coordinator.register(owner_key, HlsAvailabilityReevaluationMode::RecoveryPressure, move |ownership| async move {
                task_release.notified().await;
                let _ =
                    ownership.finish_cycle(&task_key, HlsAvailabilityReevaluationFinishReason::Evaluated);
    ...
    async fn hls_availability_reevaluation_capacity_one_owner_hands_off_to_current_generation() {
        ...
        let registration = {
            ...
            coordinator.register(stale_key.clone(), HlsAvailabilityReevaluationMode::RecoveryPressure, move |ownership| async move {
                ...
                assert_eq!(
                    ownership
                        .finish_cycle(&task_stale_key, HlsAvailabilityReevaluationFinishReason::Evaluated),
        ...
        let successor_registration = {
            ...
            coordinator.register(current_key.clone(), HlsAvailabilityReevaluationMode::RecoveryPressure, move |_| async move {
        ...
        let other_registration = {
            ...
            coordinator.register(other_key, HlsAvailabilityReevaluationMode::RecoveryPressure, move |ownership| async move {
                assert_eq!(
                    ownership.finish_cycle(
                        &task_other_key,
                        HlsAvailabilityReevaluationFinishReason::Evaluated,
                    ),
                    HlsAvailabilityReevaluationFinishDecision::Complete
    ...
    async fn hls_availability_reevaluation_capacity_is_typed_and_bounded() {
        ...
        let registration = {
            let release = Arc::clone(&release);
            coordinator.register(key(1, 1), HlsAvailabilityReevaluationMode::RecoveryPressure, move |_| async move { release.notified().await })
        };
        assert_eq!(registration, HlsAvailabilityReevaluationRegistration::Scheduled);
    ...
    async fn assert_dirty_demand_starts_one_successor_cycle(
        finish_reason: HlsAvailabilityReevaluationFinishReason,
    ) {
        let coordinator = Arc::new(HlsAvailabilityReevaluationCoordinator::with_capacity(1));
        let owner_key = key(1, 1);
    ...
    async fn hls_availability_reevaluation_dirty_demand_survives_deadline_cycle_exit() {
        assert_dirty_demand_starts_one_successor_cycle(
            HlsAvailabilityReevaluationFinishReason::CycleBudgetExhausted,
        )
        .await;
    }
    ...
    async fn assert_rekeyed_availability_owner_is_cancelled(clear_all: bool) {
        ...
        let registration = {
            ...
            coordinator.register(stale_key, HlsAvailabilityReevaluationMode::RecoveryPressure, move |ownership| async move {
                started.notify_one();
                handoff.notified().await;
                assert_eq!(
                    ownership.handoff_to(&task_stale_key, task_current_key.clone()),
