...
pub(crate) struct HlsManifestOriginBinding {
    request_url: Url,
    provider_url_index: Option<usize>,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq, thiserror::Error)]
pub(crate) enum HlsManifestOriginBindingError {
...
impl HlsManifestOriginBinding {
    pub(crate) fn new(
        request_url: Url,
        provider_url_index: Option<usize>,
    ) -> Result<Self, HlsManifestOriginBindingError> {
    ...
    pub(crate) fn request_url(&self) -> &Url { &self.request_url }

    pub(crate) const fn provider_url_index(&self) -> Option<usize> { self.provider_url_index }
}
