...
/// Parsed normal timeline segment file from `/proxy/hls/live/{id}/{segment_file}`.
...
/// Parsed EXT-X-MAP file from `/proxy/hls/live/{id}/map/{map_file}`.
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// Parsed transient passthrough resource from `/proxy/hls/live/{id}/r/{resource_file}`.
#[derive(Debug, Clone, PartialEq, Eq)]
...
mod tests {
    ...
    fn segment_file_parser_accepts_supported_extensions_and_minimum_width() {
        ...
        assert_eq!(HlsSegmentFile::parse("000123.m4a").expect("valid segment").extension, "m4a");
    ...
    fn map_file_parser_accepts_supported_extensions_and_minimum_width() {
        ...
        assert_eq!(HlsMapFile::parse("000001.m4a").expect("valid map").extension, "m4a");
    ...
    fn transient_resource_parser_rejects_missing_parts_and_origin_urls() {
        for file_name in [".ts", "f91ac2", "f91ac2.", "http://origin/seg.ts"] {
