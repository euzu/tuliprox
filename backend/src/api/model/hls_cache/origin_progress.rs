...
pub(crate) enum HlsOriginProgressPhase {
    Cold,
    Fresh,
    PublicationLate,
    RecoveryRequired,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsOriginPathCondition {
    ProgressExpected,
    PublicationLate,
...
impl HlsOriginPathCondition {
    pub(crate) const fn is_degraded(self) -> bool { !matches!(self, Self::ProgressExpected) }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct HlsOriginProgressSnapshot {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct HlsOriginProgressDecision {
...
pub(crate) fn publication_late_after_ms(target_duration_ms: u64) -> u64 {
...
pub(crate) fn evaluate_origin_progress(snapshot: HlsOriginProgressSnapshot) -> HlsOriginProgressDecision {
    ...
    let critical = condition.is_degraded()
        && snapshot.session_cutover_evaluation_required
        && !snapshot.recovery_committed;
...
pub(crate) struct HlsBoundedRecoverySamples {
...
impl HlsBoundedRecoverySamples {
    ...
    pub(crate) fn record(&mut self, elapsed_ms: u64) {
    ...
    pub(crate) fn p95_ms(&self) -> Option<u64> { self.percentile_ms(95) }

    pub(crate) fn p99_ms(&self) -> Option<u64> { self.percentile_ms(99) }

    pub(crate) fn latency_snapshot(&self) -> HlsObservedRecoveryLatency {
