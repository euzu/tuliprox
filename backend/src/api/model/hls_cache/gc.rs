use super::{
    renderer_candidate_window_proxy_seqs, safe_proxy_session_id, transient::extract_transient_resource_ids,
    CacheInvalidationOutcome, HlsAccessLeaseStore, HlsCacheCapacityReclaimOutcome, HlsCacheCapacityReclaimRequest,
    HlsCacheCapacityReclaimer, HlsCacheMetrics, HlsExpiredSessionReason, HlsSegmentCache, HlsSession, HlsSessionHandle,
    HlsSessionStore, MapCacheKey, MapCacheStatus, ProxyMapId, ProxySessionId, SegmentCacheKey, SegmentCacheStatus,
...
impl HlsGarbageCollector {
    ...
    pub(crate) fn install_access_leases(&self, access_leases: &Arc<RwLock<HlsAccessLeaseStore>>) {
    ...
    pub(crate) async fn update_cache_path(&self, cache_path: impl Into<PathBuf>) -> bool {
    ...
    async fn reclaim_for_projected_write(
        ...
        deletion_attempt_budget = deletion_attempt_budget
            .saturating_sub(pending_attempts_before.saturating_sub(pending_attempt_budget));
        ...
        if required_session_bytes > 0 {
            if let Some(session) = self.sessions.get_by_proxy_session_id(&request.proxy_session_id).await {
                ...
                let release_through = lease_guard
                    .as_ref()
                    .and_then(|leases| leases.capacity_release_through(&request.proxy_session_id));
                ...
                if let Ok(mut session) = session.try_write() {
                    ...
                    if !deletions.deletions.is_empty() {
                        let rendered_at_ms = session.last_rendered_manifest.as_ref().map_or(
                            session.last_client_access_at_ms,
                            |rendered| rendered.rendered_at_ms.saturating_add(1),
                        );
                ...
                deletions
                    .execute_prioritized(&self.cache, &mut report, &mut deletion_attempt_budget)
                    .await;
            }
        }
    ...
    async fn collect_global_size_deletions(
        ...
        loop {
            ...
            let Some(candidate) =
                oldest_global_transient_object_candidate(
                    sessions,
                    &self.cache,
                    None,
    ...
    async fn collect_projected_global_reclamation(
        ...
        while planned_bytes < required_bytes && planned_deletions < max_deletions && deletions.has_capacity() {
            ...
            let Some(removal) = session.transient.remove_oldest_ready_object_except(&protected.key_resource_ids)
            else {
                skipped_transient_sessions.insert(candidate.proxy_session_id);
                continue;
            };
        ...
        while planned_bytes < required_bytes && planned_deletions < max_deletions && deletions.has_capacity() {
            let Some(candidate) =
                oldest_global_fifo_head_candidate(
                    sessions,
                    &self.cache,
                    Some(excluded_path),
            ...
            for map_id in unprotected_unreferenced_map_ids(
                ...
                if planned_bytes >= required_bytes
                    || planned_deletions >= max_deletions
                    || !deletions.has_capacity()
                {
                    break;
                }
                if let Some(deletion) = remove_map_entry(&mut session, map_id) {
    ...
    pub(crate) fn reserve_switch_segment_cleanup(
        &self,
        key: SegmentCacheKey,
    ) -> Option<HlsSwitchCacheCleanupReservation> {
    ...
    pub(crate) fn has_pending_switch_cleanup(
        &self,
        segment_key: &SegmentCacheKey,
        map_key: Option<&MapCacheKey>,
    ) -> bool {
    ...
    #[cfg(test)]
    pub(crate) fn cache_deletion_queue_usage(&self) -> (usize, usize) {
        let state = self.pending_cache_deletions.lock().unwrap_or_else(std::sync::PoisonError::into_inner);
...
pub(crate) struct HlsSwitchCacheCleanupReservation {
...
fn capacity_reclamation_evidence(
    session: &HlsSession,
    protected: &ProtectedSet,
    ...
    let reclaimable_bytes = fifo_head_size_candidate(session, protected).map_or(0, |candidate| candidate.content_length);
    CapacityReclamationEvidence {
...
mod tests {
    use super::{
        ...
        build_rewrite_secret_fingerprint, oldest_global_fifo_head_candidate, remove_segment_entry,
        CacheObjectDeletion, GarbageCollectionPolicy, GarbageCollectionReport, HlsGarbageCollector,
        PendingCacheObjectDeletion, ProtectedSet, SegmentCacheDeletionReason, MAX_PENDING_CACHE_DELETIONS,
        SWITCH_CACHE_CLEANUP_HEADROOM,
    };
    use crate::{
        api::model::{
    ...
    ) -> (Arc<HlsGarbageCollector>, crate::api::model::HlsSessionHandle) {
    ...
    async fn cache_selected_ready_segments(
        ...
        let keys = {
            ...
            proxy_seqs
                .iter()
                .map(|proxy_seq| {
                    (*proxy_seq, session.segments.get(proxy_seq).expect("segment").cache_key.clone())
                })
                .collect::<Vec<_>>()
        };
        for (_, key) in &keys {
    ...
    async fn commit_sparse_segment(gc: &HlsGarbageCollector, key: &crate::api::model::SegmentCacheKey, size: u64) {
            ...
            for proxy_seq in 0..MAX_PENDING_CACHE_DELETIONS {
                queue.pending.push_back(PendingCacheObjectDeletion {
                    deletion: CacheObjectDeletion::Segment {
                        key: crate::api::model::SegmentCacheKey::new(
                            crate::api::model::ProxySessionId("queued".to_string()),
    ...
    async fn dropping_unpersisted_deletion_batch_keeps_reserved_deletion_retryable() {
        ...
        let key = crate::api::model::SegmentCacheKey::new(
            crate::api::model::ProxySessionId("cancelled".to_string()),
            1,
            "ts",
        );
        {
    ...
    async fn ordinary_gc_batch_preserves_bounded_switch_rollback_headroom() {
        ...
        let segment = gc.reserve_switch_segment_cleanup(crate::api::model::SegmentCacheKey::new(
            proxy_session_id.clone(),
            1,
            "ts",
        ));
        let map = gc.reserve_switch_map_cleanup(crate::api::model::MapCacheKey::new(
            proxy_session_id,
            ProxyMapId(1),
            "mp4",
        ));
    ...
    async fn cache_root_handoff_discards_old_logical_deletion_tickets_before_new_root_use() {
        ...
        gc.cache
            .write_bytes_and_commit(&key, b"new-root-sentinel")
            .await
            .expect("new-root sentinel writes");
    ...
    async fn cache_root_handoff_invalidates_armed_switch_cleanup_reservations() {
        ...
        let (gc, session) = gc_with_session(&old_root).await;
        let proxy_session_id = session.read().await.proxy_session_id.clone();
        let key = crate::api::model::SegmentCacheKey::new(proxy_session_id, 1, "ts");
    ...
    async fn protected_startup_tail_defers_until_playback_releases_fifo_head() {
        ...
        }
        {
            ...
            session.segments.get_mut(&6).expect("tail segment").status = SegmentCacheStatus::Queued {
        ...
        {
            let mut session = session.write().await;
            session.segments.get_mut(&6).expect("tail segment").status = SegmentCacheStatus::CapacityDeferred {
                priority: SegmentFetchPriority::Prefetch,
                deferred_at_ms: 11,
            };
    ...
    async fn high_bitrate_sequence_advances_beyond_the_512_mib_aggregate_wall() {
        ...
        }
        {
            ...
            for proxy_seq in 1..=READY_SEGMENTS {
                session.segments.get_mut(&proxy_seq).expect("ready segment").status = SegmentCacheStatus::Ready {
                    content_length: OLD_SEGMENT_BYTES,
                    ready_at_ms: proxy_seq,
                };
            }
        }
        ...
        let aggregate_sequence_bytes = READY_SEGMENTS
            .saturating_mul(OLD_SEGMENT_BYTES)
    ...
    fn encrypted_terminal_evidence_manifest(
        ...
        let encryption = HlsEncryptionSignature {
            ...
            key_uri: Some(format!(
                "/hls/shared/live/{}/lease/r/{}.key",
                proxy_session_id.0, resource_id.0
            )),
