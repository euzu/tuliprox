use super::{safe_proxy_session_id, HlsSession, HlsSessionHandle, HlsSessionKey, ProxySessionId};
use crate::{
    api::model::{AppState, ConnectionKind, ProviderAllocation, ProviderHandle},
...
pub fn hls_origin_account_status(
    app_state: &Arc<AppState>,
    binding: &HlsOriginAccountBinding,
) -> HlsOriginAccountStatus {
    let Some(provider_config) = app_state.active_provider.find_provider_config(&binding.account_name) else {
        return HlsOriginAccountStatus::Missing;
    };
    if !app_state.active_provider.is_provider_for_input(&binding.account_name, &binding.input_name) {
        return HlsOriginAccountStatus::Missing;
    }
...
pub async fn acquire_bound_hls_origin_account_handle(
    ...
    let kind = if app_state.active_provider.is_exhausted(&binding.account_name).await {
