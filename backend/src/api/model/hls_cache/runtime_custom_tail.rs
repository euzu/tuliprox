use super::{
    ...
    terminal_tail::{
        snapshot_terminal_media_asset, terminal_media_asset_identity, HlsLeasePlaybackMode,
        HlsTerminalAssetIdentity, HlsTerminalMediaAsset, HlsTerminalTailCompatibility,
...
use crate::{
    api::model::{is_custom_video_stream_enabled, AppState, CustomVideoStreamType},
    model::CustomStreamResponse,
...
pub(crate) enum HlsRuntimeCustomTailReason {
    ChannelUnavailable,
...
impl HlsRuntimeCustomTailReason {
    pub(crate) const fn video_type(self) -> CustomVideoStreamType {
        match self {
    ...
    pub(crate) const fn trigger_class(self) -> HlsRuntimeCustomTailTriggerClass {
        match self {
    ...
    pub(crate) const fn base_policy(self) -> HlsRuntimeCustomTailBasePolicy {
        match self {
    ...
    pub(crate) const fn permits_unpublished_lease_standalone_tail(self) -> bool {
    ...
    pub(crate) fn from_video_type(video_type: CustomVideoStreamType) -> Option<Self> {
        match video_type {
...
pub(crate) enum HlsRuntimeCustomTailBasePolicy {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum HlsRuntimeCustomTailTriggerClass {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum HlsFiniteTailTrigger {
    AvailabilityReserve,
    RuntimePolicy(HlsRuntimeCustomTailReason),
}

impl HlsFiniteTailTrigger {
    pub(crate) const fn reason(self) -> HlsRuntimeCustomTailReason {
        match self {
    ...
    pub(crate) const fn is_runtime_policy(self) -> bool { matches!(self, Self::RuntimePolicy(_)) }
}

#[derive(Clone)]
pub(crate) struct HlsRuntimeCustomTailAsset {
    pub reason: HlsRuntimeCustomTailReason,
    pub asset: Arc<HlsTerminalMediaAsset>,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
pub(crate) struct HlsRuntimeCustomTailAssetIdentity {
    pub reason: HlsRuntimeCustomTailReason,
...
impl HlsRuntimeCustomTailAssetIdentity {
    pub(crate) const fn new(
        ...
        media: HlsTerminalAssetIdentity,
    ) -> Self {
        Self { reason, media }
    }

    #[cfg(test)]
    pub(crate) const fn channel_unavailable(media: HlsTerminalAssetIdentity) -> Self {
        Self::new(HlsRuntimeCustomTailReason::ChannelUnavailable, media)
    }

    pub(crate) fn from_asset(asset: &HlsRuntimeCustomTailAsset) -> Self {
        Self::new(asset.reason, HlsTerminalAssetIdentity::from_asset(&asset.asset))
    }
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct HlsRuntimeCustomTailRevision {
    pub reason: HlsRuntimeCustomTailReason,
    pub asset: Option<HlsTerminalAssetIdentity>,
}

impl HlsRuntimeCustomTailRevision {
    pub(crate) const fn from_identity(identity: HlsRuntimeCustomTailAssetIdentity) -> Self {
        Self { reason: identity.reason, asset: Some(identity.media) }
    }

    pub(crate) const fn missing(reason: HlsRuntimeCustomTailReason) -> Self { Self { reason, asset: None } }
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum HlsRuntimeCustomTailCompatibility {
    MissingAsset,
    Incompatible(HlsTerminalTailCompatibility),
}

impl HlsRuntimeCustomTailCompatibility {
    pub(crate) const fn terminal_compatibility(self) -> HlsTerminalTailCompatibility {
        match self {
...
fn custom_tail_buffer(
    ...
    reason: HlsRuntimeCustomTailReason,
) -> Option<&crate::api::model::TransportStreamBuffer> {
    match reason {
        ...
        HlsRuntimeCustomTailReason::ProviderConnectionsExhausted => {
            response.provider_connections_exhausted.as_ref()
        }
...
pub(crate) fn snapshot_hls_runtime_custom_tail_asset(
    app_state: &AppState,
    reason: HlsRuntimeCustomTailReason,
...
pub(crate) fn current_hls_runtime_custom_tail_identity(
    app_state: &AppState,
    reason: HlsRuntimeCustomTailReason,
...
#[derive(Clone)]
pub(crate) struct HlsRuntimeCustomTailRequest {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum HlsRuntimeCustomTailOutcome {
    Committed,
    AlreadyCommitted,
        ...
        HlsRuntimeCustomTailTriggerClass::ImmediatePolicyCutover => {
                ...
                .await
            {
                ...
                HlsRuntimePolicyRevocationOutcome::NoLongerEligible => {
                    Err(HlsRuntimeCustomTailOutcome::Superseded)
                }
            }
        }
    }
...
async fn fail_runtime_custom_tail_revocation(
    ...
    if let Some(token) = revocation {
        let _ = app_state
            .hls_proxy
            .fail_runtime_policy_revocation(&request.lease_id, &request.proxy_session_id, token)
            .await;
    }
}

pub(crate) async fn commit_hls_runtime_custom_tail(
    app_state: Arc<AppState>,
    request: HlsRuntimeCustomTailRequest,
    ...
    let revocation = match begin_runtime_custom_tail_revocation(&app_state, &request).await {
        Ok(revocation) => revocation,
        Err(outcome) => return outcome,
    };
    let outcome = resolve_hls_runtime_custom_tail_after_revocation(&app_state, &request).await;
    if !outcome.retains_runtime_policy_revocation() {
        fail_runtime_custom_tail_revocation(&app_state, &request, revocation.as_ref()).await;
    }
    outcome
}
...
async fn resolve_hls_runtime_custom_tail_after_revocation(
    ...
    let asset = match snapshot_hls_runtime_custom_tail_asset(app_state, request.reason) {
        Ok(asset) => asset,
    ...
    let resolution = match request.reason.trigger_class() {
        ...
        HlsRuntimeCustomTailTriggerClass::ImmediatePolicyCutover => {
            let Some(preparation) = app_state
                .hls_proxy
...
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
pub(crate) struct HlsStandaloneCustomPlanKey {
    pub reason: HlsRuntimeCustomTailReason,
...
#[derive(Clone)]
pub(crate) struct HlsStandaloneCustomPlan {
...
impl HlsStandaloneCustomPlan {
    pub(crate) fn segment_bytes(&self, index: u16) -> Option<bytes::Bytes> {
...
/// remain server-side.
#[derive(Clone)]
pub(crate) struct HlsStandaloneCustomAccess {
...
impl HlsStandaloneCustomAccess {
    pub(crate) fn for_user(username: impl Into<Arc<str>>) -> Self {
        Self {
    ...
    pub(crate) fn for_shared_lease(
        ...
        Self {
            lease_id,
            username: username.into(),
            shared_lease: Some(HlsStandaloneSharedLeaseBinding {
                proxy_session_id,
                lease_issued_at_ms,
            }),
    ...
    pub(crate) fn lease_id(&self) -> &HlsAccessLeaseId { &self.lease_id }
    ...
    fn valid_until_ms(&self, now_ms: u64, media_duration_ms: u64, segment_count: u16) -> u64 {
        self.shared_valid_until_ms.unwrap_or_else(|| {
            ...
            let ttl_ms = playback_ms
                ...
                .clamp(
                    HLS_STANDALONE_CUSTOM_ACCESS_MIN_TTL_MS,
                    HLS_STANDALONE_CUSTOM_ACCESS_MAX_TTL_MS,
                );
...
#[derive(Clone)]
pub(crate) struct HlsStandaloneSharedLeaseBinding {
    pub(crate) proxy_session_id: ProxySessionId,
    pub(crate) lease_issued_at_ms: u64,
}

#[derive(Clone)]
pub(crate) struct HlsStandaloneCustomAccessEntry {
...
#[derive(Default)]
pub(crate) struct HlsStandaloneCustomAccessStore {
...
impl HlsStandaloneCustomAccessStore {
    ...
    fn remove_expired(inner: &mut HlsStandaloneCustomAccessStoreInner, now_ms: u64) {
        let expired = inner
            .entries
            .iter()
            .filter(|&(_lease_id, entry)| entry.valid_until_ms <= now_ms ).map(|(lease_id, _entry)| lease_id.clone())
            .collect::<Vec<_>>();
    ...
    pub(crate) fn register(&self, entry: HlsStandaloneCustomAccessEntry, now_ms: u64) {
    ...
    pub(crate) fn remove(&self, lease_id: &HlsAccessLeaseId) {
        let mut inner = self.lock();
        inner.entries.remove(lease_id);
        inner.insertion_order.retain(|current| current != lease_id);
    }
...
pub(crate) struct HlsStandaloneCustomSegmentAccess {
    pub(crate) username: Arc<str>,
    pub(crate) shared_lease: Option<HlsStandaloneSharedLeaseBinding>,
    pub(crate) bytes: bytes::Bytes,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum HlsStandaloneCustomSegmentError {
    InvalidIndex,
    UnknownAccessLease,
    StaleAssetFingerprint,
}

pub(crate) fn standalone_custom_asset_fingerprint(identity: HlsTerminalAssetIdentity) -> String {
...
pub(crate) fn resolve_hls_standalone_custom_segment(
    app_state: &Arc<AppState>,
    lease_id: &HlsAccessLeaseId,
    asset_fingerprint: &str,
    index: u16,
    ...
    app_state
        .hls_proxy
        .resolve_standalone_custom_segment(lease_id, asset_fingerprint, index, now_ms)
}
...
async fn await_prepared_standalone_bundle(
    ...
    let state = app_state.hls_proxy.prepared_terminal_bundle_state(key).unwrap_or_else(|| {
        app_state
            .hls_proxy
            .start_prepared_terminal_bundle(asset, key.target_duration_ms, key.segment_count)
    });
    match state {
        ...
        HlsPreparedTerminalBundleState::Preparing { .. } => {
            let HlsPreparedTerminalBundleObservation::Flight(ticket) =
                app_state.hls_proxy.observe_prepared_terminal_bundle(key)
            else {
                return None;
            };
            let timeout_ms = app_state.hls_proxy.origin_manifest_timeout_ms().max(1);
            match tokio::time::timeout(Duration::from_millis(timeout_ms), ticket.wait()).await {
                Ok(HlsPreparedTerminalBundleCompletion::Ready { bundle })
                    if bundle.matches_key_and_shape(key) =>
                {
                    Some(bundle)
                }
                Ok(
...
pub(crate) async fn build_hls_standalone_custom_plan(
    app_state: &Arc<AppState>,
    base_url: &str,
    access: HlsStandaloneCustomAccess,
    reason: HlsRuntimeCustomTailReason,
    ...
    let snapshot = snapshot_hls_runtime_custom_tail_asset(app_state, reason)?;
    ...
    let Some(prepared_bundle) =
        await_prepared_standalone_bundle(app_state, Arc::clone(&snapshot.asset), prepared_key).await
    else {
        return Err(HlsRuntimeCustomTailCompatibility::Incompatible(
            HlsTerminalTailCompatibility::TerminalMediaNotReady,
        ));
    };
    let key = HlsStandaloneCustomPlanKey {
        reason,
        asset: media,
        segment_count: HLS_TERMINAL_TAIL_SEGMENT_COUNT,
    };
    ...
    let valid_until_ms =
        access.valid_until_ms(now_ms, snapshot.asset.duration_ms(), HLS_TERMINAL_TAIL_SEGMENT_COUNT);
    app_state.hls_proxy.register_standalone_custom_access(
...
mod tests {
    ...
    fn standalone_access_store_evicts_oldest_entry_at_hard_capacity() {
        ...
        for index in 0..=HLS_STANDALONE_CUSTOM_ACCESS_CAPACITY {
            store.register(
                standalone_access_entry(&format!("lease-{index}"), index as u64, 10_000),
                0,
            );
        }
        ...
        assert!(store
            .resolve(
                &HlsAccessLeaseId("lease-1".to_string()),
                "0000000000000001",
                0,
                1,
            )
