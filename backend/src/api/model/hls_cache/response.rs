...
use super::{
    ...
    ProxyMapId, ProxySessionId,
    SegmentCacheKey, SegmentCacheStatus, TransientObjectCacheKey, TransientResourceFile, TransientResourceKind,
};
use crate::api::{
    api_utils::mark_response_as_uncompressed,
    model::{resolve_single_byte_range, SingleByteRange, StreamMeterHandle},
};
...
use tokio::{io::AsyncReadExt, sync::Semaphore, time::{sleep, Sleep}};
...
pub(crate) fn finite_hls_media_response(
    bytes: Bytes,
    range_header: Option<&HeaderValue>,
    content_type: &'static str,
    cache_control: &'static str,
...
pub(crate) fn finite_hls_terminal_key_response(
    bytes: Bytes,
    range_header: Option<&HeaderValue>,
    content_type: &str,
    cache_control: &'static str,
...
pub(crate) fn finite_hls_media_head_response(
    full_size: u64,
    range_header: Option<&HeaderValue>,
    content_type: &'static str,
    cache_control: &'static str,
...
impl HlsMediaActivityMarker {
    ...
    fn record_startup_segment_request(&self, proxy_seq: u64, now_ms: u64) {
        self.manager
            ...
            .record_first_visible_segment_request(&self.lease_id, proxy_seq, now_ms);
    }

    fn record_startup_repair_decision(&self, proxy_seq: u64, now_ms: u64) {
        self.manager
            .startup_observability()
            .record_repair_decision(&self.lease_id, proxy_seq, now_ms);
    }
    ...
    fn begin_startup_cache_response(
        ...
        self.manager
            .startup_observability()
            .begin_cache_response(&self.lease_id, proxy_seq, body_id, now_ms)
    }
...
fn spawn_bounded_media_completion(completion_permits: Arc<Semaphore>, completion: impl Future<Output = ()> + Send + 'static) {
...
pub async fn serve_hls_segment_cache_outcome(
    ...
    match lookup_segment_cache_object(&session, &segment_file, &context.hls_access_lease_id).await {
        CacheObjectLookup::Ready(object) => {
            cache_object_serve_outcome(
                serve_cache_object(
                    segment_cache,
                    object,
                    range_header,
                    CacheObjectServeContext {
                        playback_cursor_tracking: HlsPlaybackCursorTracking::Segment {
                            proxy_seq: segment_file.proxy_seq,
                        },
...
fn cache_object_serve_outcome(
    result: Result<Response<Body>, HlsResourceServeFailure>,
) -> HlsResourceServeOutcome {
    match result {
    ...
    K: super::HlsCacheObjectKey + Send + Sync + 'static,
{
    ...
    let cursor_marker = if let (Some(proxy_seq), Some(marker)) = (cursor_proxy_seq, context.media_activity_marker.clone())
    {
...
mod tests {
    use super::{
        ...
        spawn_bounded_media_completion,
        transient_body_object_kind, ActiveReaderStream, CacheBodyLogContext, CacheObject, CacheObjectLogContext,
        CacheObjectServeContext, HlsCacheResponseContext, HlsMediaActivityMarker,
        HlsPlaybackCursorTracking, HlsResourceServeFailure, HlsResourceServeOutcome, PREPARED_MEDIA_CHUNK_SIZE,
    };
    use crate::{
        api::model::{
            CacheAccessState, HlsAccessLease, HlsAccessLeaseId, HlsLogIdentity, HlsPlaybackFamilyKey, HlsProxyManager,
            HlsSegmentCache, HlsSegmentFile, HlsSegmentRepairManager, HlsSessionHandle, HlsSessionKey, OriginSegmentKey,
            ProxySessionId, SegmentCacheKey, SegmentCacheStatus, SegmentEntry, StreamMeterHandle,
            TransientResourceKind,
        },
        model::HlsSegmentRepairConfig,
    };
    use crate::api::model::hls_cache::media_reserve::HlsLeasePlaybackCursor;
    ...
    impl CachedSegmentFixture {
        async fn new(bytes: Vec<u8>) -> Self {
            ...
            segment_cache
                .write_bytes_and_commit(&cache_key, &bytes)
                .await
                .expect("segment cache commit");
    ...
    async fn finite_terminal_media_supports_full_range_and_unsatisfiable_responses() {
        ...
        assert!(!crate::api::api_utils::should_compress_response(&full));
        ...
        assert!(!crate::api::api_utils::should_compress_response(&partial));
        ...
        assert!(!crate::api::api_utils::should_compress_response(&invalid));
    ...
    async fn finite_terminal_media_uses_qos_completion_and_drop_semantics() {
        ...
        assert!(!crate::api::api_utils::should_compress_response(&completed));
    ...
    async fn unsatisfiable_or_missing_segment_records_neither_activity_nor_cursor() {
        ...
        assert!(matches!(
            missing,
            HlsResourceServeOutcome::Failure(HlsResourceServeFailure::Missing)
        ));
        assert_eq!(fixture.session.read().await.activity.last_authorized_media_at_ms, None);
        assert_eq!(fixture.cursor().await, HlsLeasePlaybackCursor::default());
        assert_eq!(fixture.meter.bytes_total(), 0);
