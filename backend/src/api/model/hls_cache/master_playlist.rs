...
pub(crate) const HLS_BANDWIDTH_PERSISTENCE_RETRY_MS: u64 = 30_000;

/// Measured stream bitrate and its conservative HLS master-playlist advertisement.
#[derive(Debug, Clone, Copy, Eq, PartialEq)]
pub(crate) struct HlsMasterBandwidth {
    measured_bps: Option<u32>,
}

impl HlsMasterBandwidth {
    pub(crate) const fn new(measured_bps: Option<u32>) -> Self {
    ...
    pub(crate) const fn is_unknown(self) -> bool { self.measured_bps.is_none() }

    pub(crate) fn advertised_bps(self) -> u32 {
...
#[derive(Debug, Clone, Copy, Eq, PartialEq)]
pub(crate) enum HlsMasterBandwidthSource {
    Item,
    Database,
    Fallback,
}
...
#[derive(Debug, Clone, Copy, Eq, PartialEq)]
pub(crate) struct HlsMasterBandwidthSelection {
    bandwidth: HlsMasterBandwidth,
    source: HlsMasterBandwidthSource,
}

impl HlsMasterBandwidthSelection {
    pub(crate) fn resolve(item_bitrate_bps: Option<u32>, database_bitrate_bps: Option<u32>) -> Self {
    ...
    pub(crate) const fn bandwidth(self) -> HlsMasterBandwidth { self.bandwidth }

    pub(crate) const fn source(self) -> HlsMasterBandwidthSource { self.source }

    pub(crate) const fn known_bitrate_bps(self) -> Option<u32> { self.bandwidth.measured_bps }
}

/// Minimal single-variant HLS master playlist for one lease-bound media playlist.
#[derive(Debug, Clone, Eq, PartialEq)]
pub(crate) struct HlsSingleVariantMasterPlaylist {
    bandwidth: HlsMasterBandwidth,
    media_playlist_uri: String,
}

impl HlsSingleVariantMasterPlaylist {
    pub(crate) fn new(bandwidth: HlsMasterBandwidth, media_playlist_uri: String) -> Self {
...
#[derive(Debug, Clone, Copy, Eq, PartialEq)]
pub(crate) struct HlsBandwidthSample {
    proxy_seq: u64,
    duration_ms: u64,
    content_length: u64,
}
...
impl HlsBandwidthSample {
    pub(crate) fn from_segment(entry: &SegmentEntry) -> Option<Self> {
...
pub(crate) enum HlsBandwidthPersistenceState {
    #[default]
    Idle,
    InFlight { bitrate_bps: u32 },
    Persisted { bitrate_bps: u32 },
    PermanentlyInapplicable { bitrate_bps: u32 },
    RetryAfter { retry_at_ms: u64 },
}

#[derive(Debug, Clone, Copy, Eq, PartialEq)]
pub(crate) enum HlsBandwidthPersistenceOutcome {
    Persisted,
    RetryAfter,
    PermanentlyInapplicable,
}

pub(crate) fn estimate_hls_peak_bandwidth_bps(
    samples: &[HlsBandwidthSample],
...
mod tests {
    use super::{
        estimate_hls_peak_bandwidth_bps, HlsBandwidthSample, HlsMasterBandwidth,
        HlsMasterBandwidthSelection, HlsMasterBandwidthSource, HlsSingleVariantMasterPlaylist,
    };
    ...
    fn hls_runtime_bandwidth_uses_highest_contiguous_target_window() {
        let samples = [
            sample(10, 2_000, 250_000),
            sample(11, 2_000, 500_000),
            sample(12, 2_000, 250_000),
        ];
    ...
    fn hls_runtime_bandwidth_ceiling_and_overflow_are_safe() {
        ...
        assert_eq!(
            estimate_hls_peak_bandwidth_bps(
                &[sample(1, 3_000, 1), sample(2, 3_000, 1), sample(3, 3_000, 1)],
                4_000,
            ),
            Some(3)
        );
