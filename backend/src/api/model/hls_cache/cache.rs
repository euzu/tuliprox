...
impl HlsCacheCapacityRevision {
    #[cfg(test)]
    pub(crate) fn for_test() -> Self { Self(Arc::new(CapacityRevision)) }
}
...
pub(crate) struct HlsCacheCapacityError {
...
impl HlsCacheCapacityError {
    pub(crate) fn configured_session_bytes(&self) -> u64 { self.configured_session_bytes }

    pub(crate) fn configured_global_bytes(&self) -> u64 { self.configured_global_bytes }

    pub(crate) fn current_session_bytes(&self) -> u64 { self.current_session_bytes }

    pub(crate) fn current_global_bytes(&self) -> u64 { self.current_global_bytes }

    pub(crate) fn staged_bytes(&self) -> u64 { self.staged_bytes }

    pub(crate) fn required_session_bytes(&self) -> u64 { self.required_session_bytes }

    pub(crate) fn required_global_bytes(&self) -> u64 { self.required_global_bytes }

    pub(crate) fn revision(&self) -> &HlsCacheCapacityRevision { &self.revision }

    #[cfg(test)]
    pub(crate) fn protected_working_set_bytes(&self) -> u64 { self.protected_working_set_bytes }

    #[cfg(test)]
    pub(crate) fn reclaimable_bytes(&self) -> u64 { self.reclaimable_bytes }
...
pub(crate) fn hls_cache_capacity_from_io(error: &io::Error) -> Option<&HlsCacheCapacityError> {
    let mut source: &(dyn std::error::Error + 'static) = error.get_ref()?;
    loop {
...
#[derive(Debug, Clone, Eq, PartialEq)]
pub(crate) struct HlsCacheCapacityReclaimRequest {
...
pub(crate) struct HlsCacheCapacityReclaimOutcome {
...
#[derive(Debug, Clone, Copy, Default, Eq, PartialEq)]
pub(crate) struct HlsCacheCapacityUsage {
    pub session_bytes: u64,
    pub global_bytes: u64,
}

/// Existing GC integration used to make room for one already-staged cache object.
pub(crate) trait HlsCacheCapacityReclaimer: Send + Sync {
    fn reclaim_capacity(
        &self,
...
impl HlsSegmentCache {
    ...
    pub(crate) fn install_capacity_reclaimer<T>(&self, reclaimer: &Arc<T>)
    where
    ...
    #[cfg(test)]
    pub(crate) fn capacity_revision(&self) -> HlsCacheCapacityRevision {
        let capacity = self.capacity.lock().unwrap_or_else(std::sync::PoisonError::into_inner);
    ...
    pub(crate) async fn wait_for_capacity_change(&self, revision: &HlsCacheCapacityRevision) {
        loop {
    ...
    pub(crate) fn notify_capacity_protection_changed(&self) {
        let mut capacity = self.capacity.lock().unwrap_or_else(std::sync::PoisonError::into_inner);
        capacity.revision = Arc::new(CapacityRevision);
        drop(capacity);
    ...
    /// an overshoot.
    pub(crate) async fn ensure_projected_write_capacity<K>(
        &self,
        key: &K,
        content_length: u64,
    ) -> io::Result<()>
    ...
    pub(crate) async fn commit_staged_with_guard<K, G>(
        &self,
        key: &K,
    ...
    pub(crate) async fn delete_if_inactive<K: HlsCacheObjectKey>(&self, key: &K) -> io::Result<()> {
        let cache_path = self.cache_path_snapshot();
        let path = Self::path_for_key_in(&cache_path.path, key);
        let reservation = self
    ...
    async fn delete_with_reservation(
        &self,
        path: PathBuf,
    ...
    pub(crate) fn contains_current_cache_path(&self, path: &Path) -> bool {
        path.starts_with(self.cache_path_snapshot().path)
    }

    pub(crate) fn has_active_mutation(&self, path: &Path) -> bool {
        self.capacity
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .active_mutations
    ...
    pub(crate) async fn capacity_usage(
        &self,
        proxy_session_id: &ProxySessionId,
    ) -> io::Result<HlsCacheCapacityUsage> {
        let cache_path = self.cache_path_snapshot();
    ...
    pub(crate) fn adopt_staged_file(&self, path: PathBuf, size: u64) -> io::Result<StagedCacheObject> {
        let cache_path = self.cache_path_snapshot();
    ...
    async fn begin_capacity_mutation(
        ...
        loop {
            ...
            let Some(reservation) =
                self.try_begin_capacity_mutation(cache_path, path, session_component.clone())
            else {
                notified.await;
                continue;
            };
...
mod tests {
    use super::{
        ...
        MAX_CONCURRENT_OWNED_CACHE_OPERATIONS, MAX_TEMP_FILE_CLEANUP_CANDIDATES_PER_RUN,
    };
    ...
    async fn abandoned_capacity_reservation_rolls_back_before_filesystem_mutation() {
        ...
        {
            let capacity =
                cache.capacity.lock().unwrap_or_else(std::sync::PoisonError::into_inner);
