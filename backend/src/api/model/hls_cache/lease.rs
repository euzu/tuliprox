use super::{
    ...
    runtime_custom_tail::{
        HlsFiniteTailTrigger, HlsRuntimeCustomTailBasePolicy, HlsRuntimeCustomTailReason,
    },
...
pub(crate) struct HlsAvailabilityEvidenceGeneration(u64);
...
#[derive(Debug, Clone, Copy, Eq, PartialEq)]
pub(crate) enum HlsLeaseStartupAdmissionState {
    Pending,
    Admitted,
}
...
pub(crate) struct HlsRuntimePolicyRevocation {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) enum HlsRuntimePolicyRevocationOutcome {
...
pub(crate) enum HlsAccessLeaseDenialMode {
    #[cfg(test)]
    ImmediateEnd,
    ImmediateRuntimePolicyEnd { reason: HlsRuntimeCustomTailReason },
    PreserveCommittedFiniteTail,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) enum HlsAccessLeaseDenialOutcome {
    UnknownLease,
    PolicyRevocationPending,
    FiniteDecisionPreserved,
    Ended {
        terminal_release: Option<HlsDeniedTerminalTailRelease>,
    },
}
...
pub struct HlsAccessLease {
    ...
    pub(crate) playback_mode: HlsLeasePlaybackMode,
    pub(crate) startup_admission: HlsLeaseStartupAdmissionState,
    pub(crate) playback_cursor: HlsLeasePlaybackCursor,
    pub(crate) last_manifest_snapshot: Option<HlsLeaseManifestSnapshot>,
    manifest_snapshot_generation: u64,
    pub(crate) admission_generation: u64,
    pub(crate) runtime_policy_revocation: Option<HlsRuntimePolicyRevocation>,
...
#[derive(Debug, Clone, Copy, Eq, PartialEq)]
pub(crate) struct HlsMediaLeaseIdentity {
    issued_at_ms: u64,
    playback: HlsMediaLeasePlaybackIdentity,
}

impl HlsMediaLeaseIdentity {
    pub(crate) const fn is_live(self) -> bool { matches!(self.playback, HlsMediaLeasePlaybackIdentity::Live { .. }) }
}
...
#[derive(Debug, Clone, Copy, Eq, PartialEq)]
pub(crate) struct HlsLeaseManifestPublicationGuard {
    issued_at_ms: u64,
    admission_generation: u64,
}

#[derive(Debug, Clone, Copy, Eq, PartialEq)]
pub(crate) enum HlsLeaseManifestPublicationRejectReason {
    UnknownLease,
    SessionMismatch,
    LeaseExpired,
    LeaseUnavailable,
...
#[derive(Debug, Clone, Copy, Eq, PartialEq)]
#[must_use]
pub(crate) enum HlsLeaseManifestPublicationOutcome {
    Committed { snapshot_generation: u64 },
    Rejected(HlsLeaseManifestPublicationRejectReason),
}

impl HlsLeaseManifestPublicationOutcome {
    pub(crate) const fn is_committed(self) -> bool { matches!(self, Self::Committed { .. }) }

    pub(crate) const fn snapshot_generation(self) -> Option<u64> {
        match self {
...
impl HlsAccessLease {
    ...
    pub(crate) const fn with_known_bitrate_bps(mut self, known_bitrate_bps: Option<u32>) -> Self {
    ...
    pub(crate) fn media_identity(&self) -> Option<HlsMediaLeaseIdentity> {
    ...
    pub(crate) fn runtime_policy_revocation_outcome(
        &self,
        ...
        match &self.playback_mode {
            ...
            HlsLeasePlaybackMode::Live => self.runtime_policy_revocation.clone().map(|token| {
    ...
    pub(crate) fn runtime_policy_denial_reason(&self) -> Option<HlsRuntimeCustomTailReason> {
        match &self.playback_mode {
    ...
    pub(crate) fn permits_unpublished_standalone_tail(
        &self,
...
fn terminal_commit_existing_outcome(
    ...
    match &lease.playback_mode {
        ...
        HlsLeasePlaybackMode::TerminalTail(current)
            if current.generation.0 == preparation.decision_generation => {
...
pub(crate) struct HlsTerminalTailPreparation {
...
impl HlsTerminalTailPreparation {
    pub(crate) fn bind_ready_terminal_media_requirement(
        &mut self,
...
pub(crate) enum HlsTerminalMediaRequirementOrigin {
    AcceptanceEpisode { generation: HlsManifestAcceptanceGeneration },
    CutoverSnapshot,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsTerminalMediaRequirementSource {
    AcceptanceEpisode { generation: HlsManifestAcceptanceGeneration },
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct HlsTerminalTailPreparationInput {
    pub trigger: HlsFiniteTailTrigger,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct HlsDeniedTerminalTailRelease {
    pub proxy_session_id: ProxySessionId,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct HlsAccessLeaseRemovalPreparation {
    pub proxy_session_id: ProxySessionId,
    pub issued_at_ms: u64,
...
impl HlsAccessLeaseStore {
    pub(crate) fn repair_prewarm_is_current(
        &self,
        lease_id: &HlsAccessLeaseId,
        proxy_session_id: &ProxySessionId,
        issued_at_ms: u64,
    ...
    pub(crate) fn availability_evidence_generation(
        &self,
        proxy_session_id: &ProxySessionId,
    ...
    pub(crate) fn begin_runtime_policy_revocation(
        &mut self,
        lease_id: &HlsAccessLeaseId,
        proxy_session_id: &ProxySessionId,
        reason: HlsRuntimeCustomTailReason,
        ...
        if lease.state == HlsAccessLeaseState::PolicyRevoking {
            return lease
                ...
                .map_or(
                    ...
                    |token| HlsRuntimePolicyRevocationOutcome::AlreadyPending { token },
                );
        }
    ...
    pub(crate) fn fail_runtime_policy_revocation(
        &mut self,
        lease_id: &HlsAccessLeaseId,
        proxy_session_id: &ProxySessionId,
    ...
    pub(crate) fn prepare_access_lease_removal(
        &self,
        lease_id: &HlsAccessLeaseId,
    ...
    pub(crate) fn remove_access_lease_if_preparation_matches(
        &mut self,
        lease_id: &HlsAccessLeaseId,
    ...
    pub(crate) fn all_live_leases_terminal_for_session(&self, proxy_session_id: &ProxySessionId) -> bool {
        let mut found = false;
    ...
    pub(crate) fn capacity_release_through(&self, proxy_session_id: &ProxySessionId) -> Option<u64> {
        ...
        if found_live_lease { release_through } else { None }
    }
    ...
    pub(crate) fn active_live_playback_snapshots_for_session(
        &mut self,
        proxy_session_id: &ProxySessionId,
        now_ms: u64,
    ) -> Vec<HlsAccessLease> {
    ...
    pub(crate) fn prepare_manifest_publication(
        &mut self,
        lease_id: &HlsAccessLeaseId,
        proxy_session_id: &ProxySessionId,
        now_ms: u64,
    ...
    pub(crate) fn commit_manifest_publication(
        &mut self,
        lease_id: &HlsAccessLeaseId,
        proxy_session_id: &ProxySessionId,
    ...
    pub(crate) fn media_identity_is_current(
        &mut self,
        lease_id: &HlsAccessLeaseId,
        proxy_session_id: &ProxySessionId,
    ...
    pub(crate) fn record_segment_request_started_if_identity_matches(
        &mut self,
        lease_id: &HlsAccessLeaseId,
        proxy_session_id: &ProxySessionId,
        expected: HlsMediaLeaseIdentity,
        ...
        let token = self
            ...
            .get_mut(lease_id)?
            .playback_cursor
            .record_request_started(proxy_seq, requested_at_ms);
        self.advance_availability_evidence(proxy_session_id).ok()?;
        Some(token)
    }

    pub(crate) fn record_segment_request_completed_if_identity_matches(
        &mut self,
        lease_id: &HlsAccessLeaseId,
        proxy_session_id: &ProxySessionId,
        expected: HlsMediaLeaseIdentity,
    ...
    pub(crate) fn prepare_terminal_tail(
        &self,
        lease_id: &HlsAccessLeaseId,
        proxy_session_id: &ProxySessionId,
    ...
    pub(crate) fn commit_terminal_tail_if_generation_matches(
        &mut self,
        lease_id: &HlsAccessLeaseId,
        proxy_session_id: &ProxySessionId,
        preparation: &HlsTerminalTailPreparation,
        ...
        if let Err(outcome) =
            lease.terminal_decision_commit_preconditions_match(proxy_session_id, preparation, now_ms)
        {
            return outcome;
        }
        if self.availability_evidence_can_advance().is_err() {
    ...
    pub(crate) fn terminal_tail_replay_outcome(
        &mut self,
        lease_id: &HlsAccessLeaseId,
        proxy_session_id: &ProxySessionId,
        preparation: &HlsTerminalTailPreparation,
    ...
    pub(crate) fn commit_terminal_unavailable_if_generation_matches(
        &mut self,
        lease_id: &HlsAccessLeaseId,
        proxy_session_id: &ProxySessionId,
        preparation: &HlsTerminalTailPreparation,
    ...
    pub(crate) fn terminal_unavailable_replay_outcome(
        &mut self,
        lease_id: &HlsAccessLeaseId,
        proxy_session_id: &ProxySessionId,
        preparation: &HlsTerminalTailPreparation,
    ...
    pub(crate) fn deny_access_lease(
        &mut self,
        lease_id: &HlsAccessLeaseId,
        ...
        HlsAccessLeaseDenialOutcome::Ended {
            terminal_release: generation.map(|generation| HlsDeniedTerminalTailRelease {
                proxy_session_id,
                generation,
            }),
        }
    ...
    pub(crate) fn acknowledge_terminal_protection_release(
        &mut self,
        lease_id: &HlsAccessLeaseId,
        proxy_session_id: &ProxySessionId,
...
fn runtime_policy_authorized_manifest_prefix(
    ...
    let authorized_last_proxy_seq = match (
        cursor.highest_contiguous_completed_proxy_seq,
        cursor.last_requested_proxy_seq,
    ) {
    ...
    let playlist_duration_ms =
        selected.iter().fold(0_u64, |total, segment| total.saturating_add(segment.duration_ms));
...
mod tests {
    use super::{
        super::{
            ...
            terminal_commit::HlsTerminalCommitOutcome,
            terminal_tail::{
                snapshot_terminal_media_asset, HlsTerminalTailCompatibility, HlsTerminalTailPlan,
            },
            HlsLeaseManifestSegment, HlsLeaseManifestSnapshot, HlsLeasePlaybackMode, HlsManifestDeliveryMode,
            HlsManifestSourceRenderMarker, HlsMediaContainer, HlsTerminalAssetIdentity, HlsTerminalBaseMediaState,
            HlsTerminalBaseProtection, HlsTerminalBaseSegmentAvailability, HlsTerminalTailBuildInput,
            HlsTerminalTailGeneration,
        },
        new_hls_access_lease_id, HlsAccessLease, HlsAccessLeaseActivation, HlsAccessLeaseId,
        HlsAccessLeasePendingDeadline, HlsAccessLeaseState, HlsAccessLeaseStore, HlsAccessLeaseTiming,
        HlsAccessLeaseDenialMode, HlsAccessLeaseDenialOutcome, HlsAccessLeaseTouch,
        HlsAvailabilityEvidenceGeneration, HlsLeaseManifestPublicationOutcome,
        HlsLeaseManifestPublicationRejectReason, HlsPlaybackFamilyKey, HlsRuntimePolicyRevocationOutcome,
        HlsTerminalMediaRequirementOrigin, HlsTerminalTailPreparationInput,
    };
    use crate::api::model::{ConnectionKind, ProxySessionId, TransportStreamBuffer};
    use std::sync::Arc;
    ...
    fn known_bitrate_builder_normalizes_zero_and_survives_response_snapshot() {
        ...
        assert!(store.prepare_access_lease(
            lease(measured_lease_id.clone(), &proxy_session_id.0, 1_000)
                .with_known_bitrate_bps(Some(2_500_000)),
        ));
    ...
    fn standalone_tail_policy_is_limited_to_pending_unpublished_lease() {
        ...
        assert!(candidate.permits_unpublished_standalone_tail(
        ...
        assert!(!activated.permits_unpublished_standalone_tail(
            HlsRuntimeCustomTailReason::ChannelUnavailable
        ));
        ...
        assert!(!terminal.permits_unpublished_standalone_tail(
            HlsRuntimeCustomTailReason::ChannelUnavailable
        ));
    }
    ...
    fn repair_prewarm_guard_rejects_newer_publication_and_terminal_lease() {
        ...
        let first_generation = publish_manifest_snapshot(
            &mut store,
            &lease_id,
            &proxy_session_id,
            manifest_snapshot(1),
        ...
        assert!(store.repair_prewarm_is_current(
            &lease_id,
            &proxy_session_id,
            1_000,
            first_generation,
        ...
        let second_generation = publish_manifest_snapshot(
            &mut store,
            &lease_id,
            &proxy_session_id,
            manifest_snapshot(2),
    ...
    fn unpublished_runtime_policy_denial_retains_first_reason_without_live_access() {
        ...
        assert_eq!(
            repeated.runtime_policy_denial_reason(),
            Some(HlsRuntimeCustomTailReason::UserConnectionsExhausted)
        );
    }
    ...
    fn user_revocation_tail_does_not_authorize_unfetched_live_suffix() {
        ...
        assert_eq!(
            preparation.manifest_snapshot.visible_proxy_seqs().collect::<Vec<_>>(),
            vec![40, 41]
        );
    }
    ...
    fn hls_terminal_commit_exact_replay_is_idempotent_and_sticky() {
        ...
        let concurrent_plan =
            terminal_plan_at(preparation.decision_generation, &proxy_session_id, &lease_id, 3_100);
    ...
    fn hls_availability_reevaluation_evidence_generation_is_per_session_and_noop_stable() {
        ...
        assert!(store
            .activate_access_lease(&primary_lease_id, &proxy_a, 2_000, timing(5_000, 30_000))
            .is_activated());
