...
use super::{
    ...
    HlsAccessLeaseStore, HlsBackpressureState, HlsBoundAccountAcquireErrorKind, HlsCacheMetrics, HlsCacheObjectKey,
    ...
    HlsCacheCapacityRevision, SegmentFetchPriority, StagedCacheObject, TransientObjectFetchDecision,
    TransientObjectFetchToken,
...
pub(crate) enum HlsSegmentFetchWorkload {
    Clear,
    EncryptedWithKey,
}
...
impl SegmentFetchPolicy {
    ...
    pub(crate) fn workload_budget_ms(&self, workload: HlsSegmentFetchWorkload) -> u64 {
    ...
    pub(crate) fn recovery_object_eta_ms(&self) -> u64 {
    ...
    pub(crate) fn demand_wait_timeout_for(&self, workload: HlsSegmentFetchWorkload) -> Duration {
...
#[derive(Debug, Clone, Copy, Eq, PartialEq)]
pub(crate) enum HlsSwitchResourceKind {
    Segment,
    Map,
}
...
pub(crate) async fn stage_hls_switch_resource<K>(
...
impl HlsSegmentWorkerPool {
    ...
    pub fn with_global_semaphore_and_metrics(
        ...
        Self::with_global_semaphore_metrics_and_availability(
            policy,
            global_semaphore,
            access_leases,
            metrics,
    ...
    pub(crate) fn with_global_semaphore_metrics_and_availability(
        policy: SegmentFetchPolicy,
        global_semaphore: Arc<Semaphore>,
        access_leases: Arc<RwLock<HlsAccessLeaseStore>>,
        metrics: Arc<HlsCacheMetrics>,
    ...
    pub async fn wake_scheduler(self: &Arc<Self>, context: SegmentFetchContext, now_ms: u64) {
        loop {
                ...
                self.next_ready_segment_key_fetch_snapshot(&context, now_ms, &runtime.policy).await
            {
                ...
                self.next_fetch_snapshot(&context, now_ms, &runtime.policy)
                    .await
                    .map(SegmentWorkerTask::Segment)
            };
            ...
            tokio::spawn(async move {
                match task {
                    SegmentWorkerTask::ReadyKey(snapshot) => {
                        worker
                            .fetch_ready_segment_key(task_context, snapshot, runtime.policy.clone(), permit)
                            .await;
                    }
    ...
    async fn next_ready_segment_key_fetch_snapshot(
        ...
        let rendered_proxy_seqs =
            session.last_rendered_manifest.as_ref()?.segment_proxy_seqs.clone();
    ...
    async fn fetch_one_segment(
        ...
        if let (Some(coordinator), Some(proxy_session_id)) = (
            ...
            completion.evidence_changed_for.as_ref(),
        ) {
            coordinator.notify_session_evidence_changed(proxy_session_id);
        }
        drop(permit);
    ...
    fn apply_segment_fetch_result(
        ...
        let notifier = (!stale_commit_cleanup_reserved && scheduled_retry.is_none())
            ...
            .flatten();
        let evidence_changed_for =
            (generation_valid && fetch_succeeded).then(|| session.proxy_session_id.clone());
        SegmentFetchCompletion {
            generation_valid,
            ...
            let (requeued, abandoned_notifier) = {
                ...
                let retry_state_matches = session.segments.get(&snapshot.proxy_seq).is_some_and(|entry| {
                    matches!(
                        ...
                        (
                            ...
                            SegmentRetryWake::CapacityRevision { .. }
                        )
                    )
                });
                if retry_state_matches {
                        ...
                        || capacity_admission != CapacityRetryAdmission::Ready
                    {
                        ...
                        (
                            session.queue_segment_fetch_candidate(snapshot.proxy_seq, retry.priority, now_ms),
                            None,
                        )
                    }
                } else {
...
async fn wait_for_capacity_retry_admission(
    ...
    loop {
        ...
        match context
            .segment_cache
            .ensure_projected_write_capacity(&snapshot.cache_key, projected_write_bytes)
            .await
        {
...
mod tests {
    use super::{
        build_segment_origin_headers, commit_failed_segment_fetch, wait_for_segment_key_dependency,
        take_queued_segment_fetch_candidate, HlsSegmentFetchWorkload, SegmentFetchContext, SegmentFetchPolicy,
    ...
    async fn popped_candidate_without_usable_fetch_binding_returns_to_discovered() {
        ...
        assert!(take_queued_segment_fetch_candidate(
            &mut session,
            1,
    ...
    async fn wait_for_concurrent_capacity_deferral(
        context: &SegmentFetchContext,
    ...
    async fn assert_segment_request_counts(
        server: &TestSegmentServer,
        expected: usize,
        failure_context: &str,
    ) {
        ...
        for proxy_seq in [2_u64, 3] {
            assert_eq!(
                requests
                    .iter()
                    .filter(|request| request.starts_with(&format!("GET /{proxy_seq}.ts ")))
                    .count(),
                expected,
    ...
    async fn concurrent_capacity_deferred_segments_retry_once_after_real_cache_release() {
        ...
        context
            .segment_cache
            .write_bytes_and_commit(&tail_key, b"t")
            .await
            .expect("tail fixture commits");
        {
            let mut session = context.session.write().await;
        ...
        let first = tokio::spawn(async move {
            first_worker
                .demand_fetch_and_wait(
                    first_context,
                    &HlsSegmentFile { proxy_seq: 2, extension: "ts".to_string() },
                    20,
                )
                .await
        });
    ...
    async fn projected_capacity_reclamation_commits_one_origin_download() {
        ...
        let old_keys = {
            ...
            [1_u64, 2]
                .map(|proxy_seq| session.segments.get(&proxy_seq).expect("old segment").cache_key.clone())
        };
        for key in &old_keys {
        ...
        let outcome = worker
            .demand_fetch_and_wait(
                context.clone(),
                &HlsSegmentFile { proxy_seq: 6, extension: "ts".to_string() },
                20,
            )
            .await;
    ...
    async fn ready_encrypted_segments_schedule_one_key_only_fetch() {
        ...
        tokio::join!(
            worker.wake_scheduler(context.clone(), 20),
            worker.wake_scheduler(context.clone(), 20)
        );
    ...
    async fn same_origin_sequence_resource_conflict_preserves_in_flight_binding() {
        ...
        let (origin_work_generation, cache_key) = {
            ...
            assert!(matches!(
                ...
                Err(TimelineMapError::OriginSequenceResourceConflict {
                    candidate_origin_seq: 1,
                    ..
                })
            ));
