...
pub(crate) const HLS_TERMINAL_TAIL_PROTECTION_CAPACITY: usize = 1_024;

#[derive(Debug, Clone, Eq, PartialEq)]
pub(crate) struct HlsTerminalTailProtection {
...
pub(crate) enum HlsTerminalTailProtectionInstall {
    Installed,
    CapacityExceeded,
}

#[derive(Debug, Clone, Copy, Eq, PartialEq)]
pub(crate) enum HlsTerminalTailProtectionRemoval {
    Removed,
    Missing,
...
pub(crate) struct HlsPublishedLiveOriginBaseline {
...
#[derive(Debug, Clone)]
pub(crate) struct HlsSessionOriginControl {
...
impl HlsSessionOriginControl {
    ...
    pub(crate) fn record_origin_response(&mut self, now_ms: u64) { self.last_origin_response_at_ms = Some(now_ms); }

    pub(crate) fn record_media_progress(&mut self, now_ms: u64, target_duration_ms: u64) {
    ...
    pub(crate) fn begin_acceptance_episode(
        &mut self,
        now_ms: u64,
    ...
    pub(crate) fn record_host_local_highwater(
        &mut self,
        origin_epoch: u64,
...
pub struct HlsSession {
    ...
    pub(crate) origin_control: HlsSessionOriginControl,
    ...
    pub(crate) published_resource_history: HlsPublishedResourceHistory,
    ...
    pub(crate) bandwidth_persistence: HlsBandwidthPersistenceState,
...
impl HlsSession {
    ...
    pub(crate) fn established_manifest_recovery_binding(&self) -> Option<HlsManifestOriginBinding> {
    ...
    pub(crate) fn fresh_manifest_commit_requirement_generation(
        &self,
    ...
    pub(crate) fn clear_fresh_manifest_commit_requirement_if_current(
        &mut self,
        reason: HlsFreshManifestRequiredReason,
        generation: u64,
    ) {
    ...
    pub(crate) fn install_terminal_tail_protection(
        &mut self,
    ...
    pub(crate) fn can_install_terminal_tail_protection(&self, lease_id: &HlsAccessLeaseId) -> bool {
        self.terminal_tail_protections.contains_key(lease_id)
            || self.terminal_tail_protections.len() < HLS_TERMINAL_TAIL_PROTECTION_CAPACITY
    }

    pub(crate) fn remove_terminal_tail_protection(
        &mut self,
        lease_id: &HlsAccessLeaseId,
    ...
    pub(crate) fn remove_terminal_tail_protection_after_lease_end(
        &mut self,
        lease_id: &HlsAccessLeaseId,
    ...
    pub(crate) fn rollback_terminal_tail_protection(
        &mut self,
        lease_id: HlsAccessLeaseId,
    ...
    pub(crate) fn clear_terminal_tail_protections(&mut self) { self.terminal_tail_protections.clear(); }

    #[cfg(test)]
    pub(crate) fn terminal_tail_protection(&self, lease_id: &HlsAccessLeaseId) -> Option<&HlsTerminalTailProtection> {
        self.terminal_tail_protections.get(lease_id)
    }

    pub(crate) fn terminal_tail_protections(
        &self,
    ) -> impl Iterator<Item = (&HlsAccessLeaseId, &HlsTerminalTailProtection)> {
        self.terminal_tail_protections.iter()
    }

    pub(crate) fn terminal_key_binding_is_current(
        &self,
        lease_id: &HlsAccessLeaseId,
    ...
    #[cfg(test)]
    pub(crate) fn terminal_tail_protection_count(&self) -> usize { self.terminal_tail_protections.len() }

    #[cfg(test)]
    pub(crate) fn has_terminal_tail_protections(&self) -> bool { !self.terminal_tail_protections.is_empty() }
    ...
    pub(crate) fn begin_bandwidth_persistence(&mut self, now_ms: u64) -> Option<u32> {
        ...
        let samples = self
            .segments
            .values()
            .filter_map(HlsBandwidthSample::from_segment)
            .collect::<Vec<_>>();
    ...
    pub(crate) fn finish_bandwidth_persistence(
        &mut self,
        bitrate_bps: u32,
    ...
    pub(crate) fn advance_media_readiness_generation(&mut self) {
...
mod tests {
    use super::{
        ...
        HLS_TERMINAL_TAIL_PROTECTION_CAPACITY,
    };
    use super::super::master_playlist::{HlsBandwidthPersistenceOutcome, HlsBandwidthPersistenceState};
    use crate::api::model::{
        hls_cache::{
            manifest_acceptance::{
        ...
        CacheAccessState, ConnectionKind, HlsAccessLeaseId, HlsAccountBindingProtection,
        HlsEffectiveOriginAcquirePolicy, HlsSegmentFailureObject, HlsSegmentFailureTransition, HlsSessionKey,
        HlsTerminalTailGeneration, OriginSegmentKey, RenderedManifest, RenderedManifestStoreOutcome, SegmentCacheKey,
        SegmentCacheStatus, SegmentEntry, HLS_PROVISIONING_GAP_ORIGIN_EPOCH, HLS_PROVISIONING_ORIGIN_EPOCH,
    };
    ...
    fn provisioning_render_plus_origin_commit_is_not_recovery_eligible() {
        ...
        session.segments.insert(
            0,
            ready_segment(&session, 0, HLS_PROVISIONING_ORIGIN_EPOCH, 4_000, 100),
        );
    ...
    fn hls_bandwidth_persistence_uses_only_three_real_ready_segments() {
        ...
        for (proxy_seq, origin_epoch) in [
            (1, HLS_PROVISIONING_ORIGIN_EPOCH),
            (2, HLS_PROVISIONING_GAP_ORIGIN_EPOCH),
            (10, 1),
            (11, 1),
        ] {
            session.segments.insert(
                proxy_seq,
                ready_segment(&session, proxy_seq, origin_epoch, 2_000, 250_000),
            );
        }
        ...
        assert_eq!(
            ...
            HlsBandwidthPersistenceState::InFlight { bitrate_bps: 1_000_000 }
        );
    }
    ...
    fn hls_bandwidth_persistence_deduplicates_and_retries_only_after_deadline() {
        ...
        assert_eq!(
            session.bandwidth_persistence,
            HlsBandwidthPersistenceState::InFlight { bitrate_bps }
        );

        session.finish_bandwidth_persistence(bitrate_bps, HlsBandwidthPersistenceOutcome::RetryAfter, 2_000);
        assert_eq!(
            session.bandwidth_persistence,
            HlsBandwidthPersistenceState::RetryAfter { retry_at_ms: 32_000 }
        );
        ...
        assert_eq!(
            session.bandwidth_persistence,
            HlsBandwidthPersistenceState::Persisted { bitrate_bps }
        );
    ...
    fn hls_recovery_timing_session_begin_stores_exact_episode_snapshot() {
        ...
        assert_eq!(
            session.origin_control.acceptance_episode.as_ref().map(super::super::manifest_acceptance::HlsManifestAcceptanceEpisode::timing),
            Some(frozen_timing)
        );
        assert_eq!(
