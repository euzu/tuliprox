...
#[derive(Debug, Default)]
pub(crate) struct HlsStartupObservability {
    registry: Mutex<HlsStartupObservationRegistry>,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct HlsStartupSummary {
    lease: String,
    session: String,
...
impl HlsStartupObservability {
    pub(crate) fn record_entry_master_response(
        &self,
    ...
    pub(crate) fn record_media_manifest_request(&self, lease_id: &HlsAccessLeaseId, now_ms: u64) {
        self.update(lease_id, |observation| {
            observation.media_manifest_request_at_ms.get_or_insert(now_ms);
        });
    }

    pub(crate) fn record_origin_manifest_commit(&self, lease_id: &HlsAccessLeaseId, now_ms: u64) {
        self.update(lease_id, |observation| {
            observation.origin_manifest_commit_at_ms.get_or_insert(now_ms);
        });
    }

    pub(crate) fn record_manifest_publication(
        &self,
        lease_id: &HlsAccessLeaseId,
        generation: u64,
        admission_at_ms: u64,
    ...
    pub(crate) fn record_first_visible_segment_request(
        &self,
        lease_id: &HlsAccessLeaseId,
        proxy_seq: u64,
        now_ms: u64,
        ...
        self.update(lease_id, |observation| {
            if observation.first_segment_request_at_ms.is_some()
                || !observation.visible_proxy_seqs.contains(&proxy_seq)
            {
                return;
            }
    ...
    pub(crate) fn record_repair_decision(
        &self,
        lease_id: &HlsAccessLeaseId,
        proxy_seq: u64,
        now_ms: u64,
    ...
    pub(crate) fn begin_cache_response(
        self: &Arc<Self>,
        lease_id: &HlsAccessLeaseId,
        proxy_seq: u64,
        body_id: &str,
    ...
    pub(crate) fn remove_access_lease(&self, lease_id: &HlsAccessLeaseId) {
        let mut registry = self.registry.lock().unwrap_or_else(std::sync::PoisonError::into_inner);
...
#[derive(Clone)]
pub(crate) struct HlsStartupBodyObservation {
...
impl HlsStartupBodyObservation {
    pub(crate) fn record_first_chunk(&self, now_ms: u64) -> bool {
        self.owner.record_first_chunk(&self.lease_id, self.generation, &self.body_id, now_ms)
    }

    pub(crate) fn finish(&self, now_ms: u64, outcome: &'static str) {
        if let Some(summary) = self.owner.finish_body(
            &self.lease_id,
            self.generation,
            &self.body_id,
            now_ms,
