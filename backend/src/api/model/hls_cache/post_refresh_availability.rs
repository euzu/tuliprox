...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
#[must_use]
pub(crate) enum HlsPostRefreshFallbackOutcome {
    TerminalCommitted,
    PendingOwnerRegistered,
    RecoveredLive,
    NoLiveLease,
...
pub(super) async fn evaluate_active_terminal_leases_for_reevaluation(
    ...
    let leases = app_state.hls_proxy.active_live_playback_snapshots_for_session(proxy_session_id, now_ms).await;
    if leases.is_empty() {
...
pub(super) async fn wait_for_owner_resolution(
    ...
    let wake_at_ms = match resolution {
        HlsPostRefreshOwnerResolution::RetryAt { at_ms }
        | HlsPostRefreshOwnerResolution::RetryAfter { at_ms, .. } => Some(at_ms),
...
pub(crate) async fn commit_post_refresh_terminal_fallback(
    app_state: Arc<AppState>,
    session: HlsSessionHandle,
    ...
    evaluate_owner_failure_fallback(&app_state, &session, &proxy_session_id, current_time_millis()).await.outcome()
}
