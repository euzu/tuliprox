...
#[cfg(test)]
use super::media_reserve::minimum_hls_startup_window_ms;
use crate::{
    model::StripConfig,
...
impl HlsManifestRenderer {
    pub fn render(session: &HlsSession, rendered_at_ms: u64) -> Result<RenderedManifest, RenderError> {
        for render_gap_segments in 0..=session.render_policy.initial_render_gap_segments {
            let Some(window) =
                select_window(session, render_gap_segments, RenderTailPolicy::TruncateUnavailableTail)
            else {
                continue;
            };
...
mod tests {
    ...
    use crate::{
        api::model::{
            hls_cache::manifest_origin_binding::HlsManifestOriginBinding, HlsSession, HlsSessionKey,
            MapCacheStatus, SegmentCacheStatus, SegmentFetchPriority, HLS_PROVISIONING_GAP_ORIGIN_EPOCH,
    ...
    fn mixed_provisioning_and_origin_manifest_enables_recovery_binding() {
        ...
        session.apply_origin_manifest(&six_segment_manifest()).expect("manifest should map");
        mark_all_segments_ready(&mut session);
        session.segments.get_mut(&0).expect("first segment").origin_key.origin_epoch =
            HLS_PROVISIONING_ORIGIN_EPOCH;
    ...
    fn published_live_baseline_survives_gc_of_evidence_segment() {
        ...
        assert_eq!(
            session.store_rendered_manifest(rendered_manifest(1, 5)),
            RenderedManifestStoreOutcome::Stored
        );
    ...
    fn hls_startup_policy_renderer_uses_shared_clamped_threshold() {
        ...
        assert_eq!(
            super::minimum_hls_startup_window_ms(rendered.target_duration_ms, rendered.playlist_duration_ms),
            6_000
        );
    }
