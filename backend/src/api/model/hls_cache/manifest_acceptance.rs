use super::deterministic_conflict::HlsDeterministicTimelineConflict;
use super::recovery_timing::{
    HlsAcceptanceEpisodeTiming, HlsEstimatedRecoveryCompletionAtMs, HlsRecoveryWorkload, HlsRecoveryWorkloadEnvelope,
};
use super::resource_identity::HlsMediaResourceIdentity;
#[cfg(test)]
use super::recovery_timing::HlsRecoveryEtaMs;
use sha2::{Digest, Sha256};
use shared::model::HlsManifestRecoveryBurstPlan;

pub(crate) const HLS_MANIFEST_RECOVERY_BURST_SLOT_DELAY_MS: u64 = 100;
pub(crate) const HLS_MANIFEST_FINGERPRINT_SEGMENT_LIMIT: usize = 64;
const HLS_MANIFEST_ACCEPTANCE_COHORT_SAMPLE_LIMIT: usize = 32;
pub(crate) const HLS_MANIFEST_ACCEPTANCE_MAX_REQUALIFICATIONS_PER_REFRESH: u8 = 1;

#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub(crate) struct HlsManifestAcceptanceGeneration(pub(crate) u64);

/// Fixed-size, non-sensitive identity of one selected recovery candidate.
#[derive(Debug, Clone, Copy)]
pub(crate) struct HlsManifestRecoveryCandidateIdentity {
...
impl HlsManifestRecoveryCandidateIdentity {
    pub(crate) fn from_candidate(candidate_index: usize, effective_host: Option<&str>, manifest_body: &str) -> Self {
    ...
    pub(crate) fn matches_candidate(self, effective_host: Option<&str>, manifest_body: &str) -> bool {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsRecoveryWorkloadBindingUpdate {
    Applied,
    StaleGeneration,
    EpisodeInactive,
    CandidateMismatch,
...
pub(crate) enum HlsManifestAcceptanceTrigger {
    #[default]
    None,
    Observe,
    RecoveryRequired,
...
impl HlsManifestAcceptanceTrigger {
    pub(crate) const fn starts_episode(self) -> bool { !matches!(self, Self::None) }

    pub(crate) const fn recovery_required(self) -> bool { matches!(self, Self::RecoveryRequired | Self::Critical) }
...
pub(crate) struct HlsManifestAcceptanceEpisode {
...
pub(crate) struct HlsDeterministicConflictReceipt {
...
impl HlsManifestAcceptanceEpisode {
    pub(crate) fn new(
        ...
        Self {
            ...
            trigger,
            timing: *timing,
            workload_binding: HlsRecoveryWorkloadBinding::CandidateUnknown {
                envelope,
    ...
    pub(crate) fn required_candidates(&self) -> usize { self.burst_plan.total_candidates() }

    pub(crate) const fn trigger(&self) -> HlsManifestAcceptanceTrigger { self.trigger }

    pub(crate) const fn timing(&self) -> HlsAcceptanceEpisodeTiming { self.timing }

    pub(crate) fn deterministic_conflict_receipt(&self) -> Option<&HlsDeterministicConflictReceipt> {
        self.deterministic_conflict_receipt.as_ref()
    }

    pub(crate) fn record_deterministic_conflict(&mut self, receipt: HlsDeterministicConflictReceipt) {
    ...
    pub(crate) fn selected_candidate_identity(&self) -> Option<HlsManifestRecoveryCandidateIdentity> {
        match self.workload_binding {
            HlsRecoveryWorkloadBinding::CandidateBound { candidate_identity, .. } => Some(candidate_identity),
            HlsRecoveryWorkloadBinding::CandidateUnknown { selected_candidate, .. } => {
    ...
    pub(crate) fn select_candidate(
        ...
        let selected = HlsSelectedRecoveryCandidate {
    ...
    pub(crate) fn bind_selected_candidate(
        &mut self,
        expected_generation: HlsManifestAcceptanceGeneration,
        candidate_identity: HlsManifestRecoveryCandidateIdentity,
        workload: HlsRecoveryWorkload,
        ...
        if selected_candidate
            != Some(HlsSelectedRecoveryCandidate {
                acceptance_generation: expected_generation,
                candidate_identity,
            })
        {
    ...
    pub(crate) fn advance_bound_candidate(
        &mut self,
        expected_generation: HlsManifestAcceptanceGeneration,
    ...
    pub(crate) fn remaining_recovery_workload(
        &self,
        expected_generation: HlsManifestAcceptanceGeneration,
    ...
    #[cfg(test)]
    pub(crate) fn remaining_recovery_eta(
        &self,
        expected_generation: HlsManifestAcceptanceGeneration,
    ...
    pub(crate) fn estimated_recovery_completion_at(
        &self,
        expected_generation: HlsManifestAcceptanceGeneration,
        now_ms: u64,
    ...
    pub(crate) const fn exhaustion_reason(&self) -> Option<HlsManifestAcceptanceExhaustionReason> {
        match self.outcome {
    ...
    pub(crate) fn burst_max_stagger_ms(&self) -> u64 {
    ...
    #[cfg(test)]
    pub(crate) fn record_full_burst(&mut self) { self.record_full_burst_candidates(self.required_candidates()); }

    pub(crate) fn record_full_burst_candidates(&mut self, completed_candidates: usize) {
    ...
    pub(crate) fn record_exhaustion(&mut self, reason: HlsManifestAcceptanceExhaustionReason) {
    ...
    pub(crate) fn hold_after_uncommitted_burst(
        &mut self,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsManifestAcceptanceState {
    FullBurstPending,
    Collecting,
    Evaluating,
    Holding,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsManifestAcceptanceExhaustionReason {
    AllFailed,
    NoProgress,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsManifestAcceptanceEpisodeStatus {
    Missing,
...
pub(crate) fn manifest_acceptance_episode_status(
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsCandidateHostRelation {
    PinnedHost,
    OtherHost,
    InitialBaseline,
    Unknown,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsHostLocalSequenceRelation {
    NoBaseline,
    Same,
    Next,
    PlausibleForward,
...
pub(crate) struct HlsManifestSegmentFingerprint {
    pub duration_ms: u64,
...
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub(crate) struct HlsManifestTimelineFingerprint {
...
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub(crate) struct HlsManifestTechnicalSignature {
    pub map_and_encryption_hash: [u8; 32],
    pub container_signature_hash: [u8; 32],
}
...
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub(crate) struct HlsAlternativeOriginCohortIdentity {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct HlsAlternativeOriginWindow {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsCrossHostAcceptanceEvidence {
    Insufficient,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsSwitchSegmentReadiness {
    Unavailable,
    RequiresStaging,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsEmergencyLiveHandoffCompatibility {
    Incompatible,
    RequiresStagedTrackVerification,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsTerminalAlternativeCompatibility {
...
impl HlsEmergencyAcceptanceEvidence {
    pub(crate) const INCOMPATIBLE: Self = Self {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct HlsManifestCandidateObservation {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct HlsAlternativeOriginCohort {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsManifestCommitKind {
    Pinned,
    AnchoredAlternative,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsPinnedOriginObservationState {
    Missing,
    Unchanged,
    Progressed,
    Rejected,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct HlsManifestAcceptanceLandscape {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsReducedRetryLandscapeChange {
    Unchanged,
    NewCohort,
    TimelineConflict,
    PinnedStateChanged,
...
impl HlsReducedRetryLandscapeChange {
    pub(crate) const fn requires_full_requalification(self) -> bool { !matches!(self, Self::Unchanged) }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum HlsManifestCommitPlan {
...
#[derive(Clone, Copy)]
pub(crate) struct HlsManifestAcceptanceInput<'a> {
    pub full_burst_completed: bool,
...
pub(crate) fn evaluate_manifest_acceptance(input: HlsManifestAcceptanceInput<'_>) -> HlsManifestCommitPlan {
...
pub(crate) fn classify_host_local_sequence(
...
pub(crate) fn alternative_cohorts(observations: &[HlsManifestCandidateObservation]) -> Vec<HlsAlternativeOriginCohort> {
...
pub(crate) fn manifest_acceptance_landscape(
    observations: &[HlsManifestCandidateObservation],
...
pub(crate) fn classify_reduced_retry_landscape(
...
pub(crate) fn alternative_cohorts_with_history(
    observations: &[HlsManifestCandidateObservation],
...
pub(crate) fn held_alternative_after_burst(
    observations: &[HlsManifestCandidateObservation],
    previous: Option<&HlsAlternativeOriginCohort>,
    current_burst_is_full_plan: bool,
...
mod tests {
    use super::*;
    use crate::api::model::hls_cache::recovery_timing::{
        HlsAcceptanceEpisodeTimingInput, HlsObservedRecoveryLatency, HlsOperationTimeoutMs, HlsRecoveryTimingPolicy,
        HlsRecoveryBurstWorkload, HlsRecoveryMapWorkload, HlsRecoverySegmentWorkload,
    ...
    fn hls_recovery_timing_candidate_binding_and_staging_reduce_eta_without_moving_deadline() {
        ...
        assert_eq!(
            episode.select_candidate(generation, identity),
            HlsRecoveryWorkloadBindingUpdate::Applied
        );
        assert_eq!(
    ...
    fn hls_recovery_timing_candidate_stale_generation_and_wrong_identity_cannot_update_binding() {
        ...
        assert_eq!(
            episode.select_candidate(generation, selected),
            HlsRecoveryWorkloadBindingUpdate::Applied
        );
        assert_eq!(
    ...
    fn strong_pdt_anchor_qualifies_alternative_without_sequence_comparison() {
        ...
        for (index, segment) in alternative.timeline_fingerprint.segment_samples.iter_mut().enumerate() {
            segment.normalized_resource_identity = Some(HlsMediaResourceIdentity::for_test(
                u8::try_from(index).unwrap_or(u8::MAX).saturating_add(90),
            ));
        }
