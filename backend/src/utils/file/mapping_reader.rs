...
mod tests {
    use super::*;
    use shared::model::{CounterModifier, MappingStage, MapperDto};
    ...
    fn dto(id: &str, stage: MappingStage, mappers: Vec<&str>, counters: usize) -> MappingDto {
        MappingDto {
            ...
            mapper: if mappers.is_empty() {
                ...
                Some(mappers.into_iter().map(|script| MapperDto { filter: String::new(), script: script.to_string(), ..Default::default() }).collect())
            },
