...
pub fn get_input_raw_epg_file_path(
    url: &str,
    input: &ConfigInput,
    working_dir: &str,
    ...
    if let Some(persist_path) = input.persist.as_deref() {
        if !persist_path.is_empty() {
            if let Some(path) =
                prepare_file_path(input.persist.as_deref(), working_dir, "").map(|path| {
                    add_prefix_to_filename(
                        &path,
                        format!("{file_prefix}_epg_").as_str(),
                        Some("xml"),
                    )
                })
            {
            ...
            for epg_source in &epg_config.sources {
                match download_epg_file(&epg_source.url, ctx, input, working_dir).await {
                    Ok(file_path) => {
                        ...
                        file_paths.push(PersistedEpgSource {
                            file_path,
