...
use crate::repository::xtream_repository::{CategoryEntry, get_live_cat_collection_path, get_series_cat_collection_path, get_vod_cat_collection_path, xtream_get_file_path};
...
pub async fn download_xtream_playlist(cfg: &Arc<Config>, client: &reqwest::Client, input: &ConfigInput, clusters: Option<&[XtreamCluster]>)
        ...
        if is_requested && !skip_cluster.contains(xtream_cluster) {
            ...
            match futures::join!(
                ...
                (Ok(category_content), Ok(stream_content)) => {
                    if cfg.use_disk_input_playlist {
    ...
    (playlist_groups, errors, cfg.use_disk_input_playlist)
}
    ...
    tokio::task::spawn_blocking(move || {
         if let Ok(file) = File::create(&col_path_buf) {
             if file.lock_exclusive().is_ok() {
                  let _ = serde_json::to_writer(&file, &cat_entries).map_err(|e| error!("Failed to write categories to file {}: {e}", col_path_buf.display()));
                  let _ = file.unlock().map_err(|e| error!("Failed to unlock category file {}: {e}", col_path_buf.display()));
             } else {
                 let _ = serde_json::to_writer(&file, &cat_entries).map_err(|e| error!("Failed to write categories to file {}: {e}", col_path_buf.display()));
             }
         } else {
