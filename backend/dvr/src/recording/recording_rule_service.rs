...
use shared::model::{
    recording_rule::{RecordingRule, RuleVisibility},
    UserId,
};
...
mod tests {
    use super::*;
    use shared::model::{
        recording_rule::{RuleBody, RuleSource},
        UserId,
    };
    ...
    fn validate_rule_rejects_new_episode_until_epg_horizon_is_wired() {
        ...
        assert_eq!(validate_rule(&r), Err(RuleServiceError::Unsupported { feature: "new_episode_rule" }));
        assert_eq!(validate_rule(&r).err().map(|e| e.code()), Some("recording_rule_new_episode_unsupported"));
    }
    ...
    fn error_codes_are_stable() {
        ...
        assert_eq!(RuleServiceError::SharedManagementNotAdministrator.code(), "recording_shared_not_administrator");
        ...
        assert_eq!(
            RuleServiceError::PartialOperation { primary: "rule".into(), secondary: "tombstone".into() }.code(),
