...
pub fn validate_patch(patch: &EditPatch, current: &RecordingMetadata, bounds: PaddingBounds) -> Result<(), EditError> {
...
pub fn apply_interval_patch(current: &RecordingMetadata, patch: &EditPatch) -> (i64, i64, i64, i64) {
...
mod tests {
    ...
    fn epg_metadata_preserved_when_channel_unchanged() {
        let meta =
            RecordingMetadata { channel_id: Some("a".into()), channel_name: Some("A".into()), ..make_meta(0, 0, 0, 0) };
        let patch = EditPatch::default();
        assert!(!epg_metadata_should_be_cleared(&patch, &meta, false));
    }
    ...
    fn visibility_unchanged_keeps_current_when_none() {
        ...
        assert_eq!(visibility_unchanged(RecordingVisibility::Private, None), RecordingVisibility::Private);
    }
    ...
    fn edit_error_codes_are_stable() {
        ...
        assert_eq!(edit_error_code(&EditError::PaddingLimitExceeded), "recording_padding_limit_exceeded");
    }
