...
pub enum ReconcileAction {
    ...
    AddScheduledTombstone { rule_id: String, occurrence_key: String },
    /// Materialize a new task for a `Scheduled` tombstone whose
    /// task disappeared and no terminal tombstone exists.
    Materialize { rule_id: String, occurrence_key: String },
    ...
    UpdateTombstone { rule_id: String, occurrence_key: String, new_kind: TombstoneKind },
...
pub fn prune_tombstones(tombstones: &TombstoneSet, now: i64) -> Vec<RecordingTombstone> {
    tombstones.tombstones.iter().filter(|t| t.expires_at > now).cloned().collect()
}
...
pub fn reconcile(
    ...
    let mut by_key: std::collections::HashMap<(String, String), &ReconcilableTask> = std::collections::HashMap::new();
    for task in tasks {
    ...
    let mut tombs: std::collections::HashMap<(String, String), &RecordingTombstone> = std::collections::HashMap::new();
...
mod tests {
    ...
    fn plan_rule_2_materializes_when_scheduled_tombstone_orphan() {
        let rules = vec![rule("r1")];
        let tasks = vec![];
        let set = TombstoneSet { tombstones: vec![tomb("r1", "k1", TombstoneKind::Scheduled, 0, 1_000_000)] };
        let actions = reconcile(&rules, &tasks, &set, 1_000);
    ...
    fn plan_rule_3_finalizes_cancelled_task() {
        let rules = vec![rule("r1")];
        let tasks = vec![task("u1", "r1", "k1", false, false, true)];
        let set = TombstoneSet { tombstones: vec![tomb("r1", "k1", TombstoneKind::Cancelled, 0, 1_000_000)] };
        let actions = reconcile(&rules, &tasks, &set, 1_000);
    ...
    fn plan_rule_4_completed_tombstone_suppresses_rematerialization() {
        let rules = vec![rule("r1")];
        let tasks = vec![];
        let set = TombstoneSet { tombstones: vec![tomb("r1", "k1", TombstoneKind::Completed, 0, 1_000_000)] };
        let actions = reconcile(&rules, &tasks, &set, 1_000);
        assert!(!actions.iter().any(|a| matches!(a, ReconcileAction::Materialize { .. })));
    }
    ...
    fn plan_rule_4_updates_tombstone_when_task_completes() {
        ...
        assert!(actions
            .iter()
            .any(|a| matches!(a, ReconcileAction::UpdateTombstone { new_kind: TombstoneKind::Completed, .. })));
    }
    ...
    fn expired_tombstones_are_pruned() {
        let rules = vec![rule("r1")];
        let tasks = vec![];
        let set = TombstoneSet { tombstones: vec![tomb("r1", "k1", TombstoneKind::Scheduled, 0, 100)] };
        let actions = reconcile(&rules, &tasks, &set, 1_000);
