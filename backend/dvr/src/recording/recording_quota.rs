...
pub fn limit_for_pool(pool: &QuotaPool, limits: &QuotaLimits) -> Option<u64> {
    match pool {
        QuotaPool::Private(uid) => limits.per_user_bytes.get(uid).copied().or(limits.default_private_bytes),
...
pub fn regular_user_dto<V: QuotaRecordingTaskView>(
    ...
    QuotaDto {
        ...
        shared: SharedAvailabilityDto { used_bytes: totals.shared, limit_bytes: limits.shared_bytes },
    }
}
...
mod tests {
    ...
    fn admission_ok_under_limit() {
        let limits = QuotaLimits { default_private_bytes: Some(1000), ..Default::default() };
    ...
    fn admission_over_limit() {
        ...
        assert_eq!(out, AdmissionOutcome::OverLimit { used: 800, limit: 1000, would_be: 1100 });
    }
    ...
    fn per_user_override_beats_default() {
        let mut limits = QuotaLimits { default_private_bytes: Some(1000), ..Default::default() };
        limits.per_user_bytes.insert(UserId::from("web:alice"), 5_000);
        assert_eq!(limit_for_pool(&QuotaPool::Private(UserId::from("web:alice")), &limits), Some(5_000));
        // bob falls back to the default
        assert_eq!(limit_for_pool(&QuotaPool::Private(UserId::from("web:bob")), &limits), Some(1000));
    }
