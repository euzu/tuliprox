...
pub async fn mutate_optional<F, R>(this: &DownloadQueue, op: F) -> Result<Option<R>, QueueMutationError>
where
...
async fn mutate_optional_locked<F, R>(this: &DownloadQueue, op: F) -> Result<Option<R>, QueueMutationError>
where
    F: FnOnce(&mut PersistedDownloadQueue) -> Result<Option<R>, QueueMutationError>,
{
...
fn normalize_legacy_recording(task: &mut FileDownload, recording_root: Option<&Path>, legacy_root: Option<&Path>) {
...
fn strip_prefix(path: &Path, root: &Path) -> Option<String> {
    ...
    if !stripped.components().all(|c| matches!(c, std::path::Component::Normal(_))) {
        return None;
    }
...
pub fn pre_scan_recording_user_ids(path: &Path) -> Vec<UserId> {
    ...
    for task in queue.queue.iter().chain(queue.scheduled.iter()).chain(queue.active.iter()).chain(queue.finished.iter())
    {
    ...
    pub fn new(
        ...
        match reqwest::Url::parse(req_url) {
            Ok(url) => {
                ...
                let mut filename =
                    if file_ext.is_empty() { file_stem.to_string() } else { format!("{file_stem}.{file_ext}") };
...
impl DownloadQueue {
    ...
    pub async fn committed_download_snapshot(&self) -> (Vec<FileDownload>, Option<FileDownload>, Vec<FileDownload>) {
        let _mutation = self.mutation_guard.lock().await;
        let queue = self.queue.lock().await;
        let scheduled = self.scheduled.read().await;
        let active = self.active.read().await;
    ...
    pub fn from_persisted_with(
        ...
        if matches!(kind, DownloadKind::Download) && recording.is_some() {
            return Err(PersistedError::KindMetadataInvariant { uuid: download.uuid.clone() });
        }
    ...
    pub async fn load_from_disk(&self) -> std::io::Result<()> {
        ...
        let scheduled_loaded = persisted
            ...
            .map(Self::recover_loaded_download)
            .collect::<Vec<_>>();
        let (scheduled, missed_scheduled): (Vec<_>, Vec<_>) =
            scheduled_loaded.into_iter().partition(|download| !Self::recording_start_missed_window(download, now_ts));
        let active = persisted.active.and_then(|p| Self::from_persisted(p).ok()).map(Self::recover_loaded_download);
        let mut finished =
    ...
    pub async fn find_duplicate(&self, candidate: &FileDownload) -> Option<FileDownload> {
        ...
        if let Some(queued) =
            self.queue.lock().await.iter().find(|download| download.matches_existing_task(candidate)).cloned()
        {
            return Some(queued);
        }

        if let Some(scheduled) =
            self.scheduled.read().await.iter().find(|download| download.matches_existing_task(candidate)).cloned()
        {
    ...
    pub async fn cancel_requested(&self, uuid: &str) -> Result<Option<bool>, QueueMutationError> {
        ...
        if let Some(was_paused) = was_paused {
            *self.control_signal.write().await =
                if was_paused { DownloadControl::None } else { DownloadControl::Cancel };
            self.control_notify.notify_waiters();
        }
        Ok(was_paused)
    }
        ...
        let result = mutate_optional(self, |candidate| {
            ...
            candidate.scheduled.retain(|download| {
                let is_missed = download.kind == DownloadKind::Recording
                    && download.start_at.zip(download.duration_secs).is_some_and(|(start_at, duration_secs)| {
                        now_ts >= start_at.saturating_add(i64::try_from(duration_secs).unwrap_or(i64::MAX))
                    });
                if is_missed {
...
mod tests {
    ...
    fn recover_loaded_download_requeues_waiting_states() {
        ...
        let retry_waiting = FileDownload { state: DownloadState::RetryWaiting, ..waiting_for_capacity.clone() };
    ...
    async fn find_duplicate_matches_active_queue_scheduled_and_finished_downloads() {
        ...
        *queue.active.write().await = Some(FileDownload { uuid: "active".to_string(), ..candidate.clone() });
        ...
        *queue.active.write().await = None;
        queue.queue.lock().await.push_back(FileDownload { uuid: "queued".to_string(), ..candidate.clone() });
    ...
    fn legacy_recording_derives_relative_path_when_contained() {
        ...
        let task = DownloadQueue::from_persisted_with(persisted, Some(&recording_root), None).expect("restore");
    ...
    async fn mutate_persists_and_increments_revision_on_success() {
        ...
        let restored: PersistedDownloadQueue = serde_json::from_slice(&content).expect("parse state file");
    ...
    fn find_recording_relative_path(candidate: &PersistedDownloadQueue, recording_uuid: &str) -> Option<String> {
        for d in &candidate.queue {
            if d.uuid == recording_uuid {
                if let Some(meta) = &d.recording {
                    if let Some(p) = &meta.relative_path {
    ...
    fn derive_legacy_relative_path_rejects_parent_dir_traversal() {
        ...
        assert_eq!(derive_legacy_relative_path(inside, Some(root), None).as_deref(), Some("2026-08/rec.ts"),);
    }
}
