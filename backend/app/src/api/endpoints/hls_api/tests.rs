...
use crate::{
    api::model::{
        ...
        build_terminal_tail_plan, build_transient_resource_id, commit_terminal_tail_if_lease_reserve_requires_cutover,
        finish_hls_origin_account_io, prepare_terminal_base_evidence, prepared_terminal_bundle_key,
        snapshot_terminal_media_asset, trigger_origin_refresh_sync, ActiveProviderManager, ActiveUserManager, AppState,
        CacheAccessState, CancelTokens, ConnectionKind, ConnectionManager, CreateUserSessionParams,
        ...
        HlsAccessLeaseTiming, HlsAccessLeaseTouch, HlsAccessLeaseValidationError,
        HlsAvailabilityReevaluationFinishReason, HlsAvailabilityReevaluationMode,
        HlsAvailabilityReevaluationRegistration, HlsBandwidthPersistenceState, HlsEffectiveOriginAcquirePolicy,
        HlsFreshManifestRequiredReason, HlsLeaseManifestSegment, HlsLeaseManifestSnapshot, HlsLeasePlaybackMode,
        HlsLifecycleEvent, HlsLifecycleEventKey, HlsManifestAcceptanceDirective,
        HlsManifestAcceptanceEvaluationOutcome, HlsManifestAcceptanceExhaustionReason, HlsManifestAcceptanceTrigger,
        HlsManifestCommitIdentity, HlsManifestCommitRequirement, HlsManifestDeliveryMode, HlsMapSignature,
        HlsMediaContainer, HlsObservedRecoveryLatency, HlsOperationTimeoutMs, HlsOriginAccountBinding,
        HlsOriginAccountBindingMode, HlsOriginAccountDetachedReason, HlsOriginIoContext, HlsOriginPathCondition,
        HlsOriginSource, HlsOriginSourceKind, HlsPlaybackFamilyKey, HlsPreparedTerminalBundleState, HlsProxyManager,
        HlsPublishedTransientResourceIds, HlsRecoveryEtaMs, HlsRecoveryTimingPolicy, HlsRecoveryWorkload,
        HlsRuntimeCustomTailAssetIdentity, HlsRuntimeCustomTailReason, HlsSegmentFile, HlsSession, HlsSessionHandle,
        HlsSessionKey, HlsSessionMode, HlsSessionStoreOutcome, HlsTerminalAssetIdentity, HlsTerminalBaseMediaState,
...
use shared::{
    model::{
        provider_saturation::build_group_lookup, ConfigPaths, ConfigProviderDto, ConfigTargetDto, ConfigTargetOptions,
        ConfigTargetShareLiveStreams, HlsCacheConfigDto, HlsManifestRecoveryBurstConfigDto,
...
fn test_hls_entry_stream_context(
    ...
    super::HlsEntryStreamContext {
        identity: super::HlsEntryStreamIdentity::new(virtual_id, input_stream_id).expect("valid input stream identity"),
        known_bitrate_bps,
    }
}
...
async fn terminal_head_content_length(app_state: &Arc<AppState>, proxy_session_id: &str, segment_uri: &str) -> usize {
...
async fn legacy_hls_manifest_decodes_supported_origin_codings_and_enforces_identity() {
    ...
    for coding in ["gzip", "deflate", "br", "zstd"] {
        ...
        let (manifest, final_url, _) = super::download_legacy_hls_manifest(&test_app_state(), &input, &client_headers)
            .await
...
async fn legacy_hls_manifest_limit_applies_after_decompression() {
    ...
    let error = super::download_legacy_hls_manifest(&test_app_state(), &input, &legacy_manifest_test_client_headers())
        .await
...
async fn legacy_hls_manifest_distinguishes_invalid_utf8_from_decoder_failure() {
    ...
    let decoder_failure =
        super::download_legacy_hls_manifest(&test_app_state(), &corrupt_input, &legacy_manifest_test_client_headers())
            .await
...
fn hls_origin_resolution_preserves_legacy_built_xtream_origin_url() {
    ...
    let origin = super::build_hls_origin_resolution(&input, "http://other.example.com/live/other/creds/1025126.m3u8")
...
fn hls_origin_resolution_keeps_provider_failover_out_of_identity() {
    ...
    let origin =
        super::build_hls_origin_resolution(&input, "provider://mirror-group/live/source-user/source-pass/1025126.m3u8")
...
async fn hls_account_overlap_selects_soft_candidate_but_not_hard_active() {
    ...
    let soft_candidate = super::find_hls_account_overlap_candidate(&app_state, &input, &new_proxy_session_id, 21_000)
        .await
...
async fn hls_origin_runtime_uses_soft_overlap_before_grace_for_interactive_work() {
    ...
    app_state
        ...
        .await;
    app_state.active_provider.confirm_playback_activity(&old_binding.session_owner).await;
...
async fn hls_origin_runtime_normal_policy_preempts_active_soft_hls_binding() {
    ...
    app_state
        ...
        .await;
    app_state.active_provider.confirm_playback_activity(&soft_binding.session_owner).await;
    // The binding acquired a reservation without a handle; capture its tag so the
    // preempt path can prove it still targets this exact incarnation.
...
async fn hls_origin_policy_preemption_rejects_soft_request_against_active_normal_binding() {
    ...
    app_state
        ...
        .await;
    app_state.active_provider.confirm_playback_activity(&normal_binding.session_owner).await;
...
async fn provisioning_handoff_finds_shared_session_by_input_stream_id_not_virtual_id() {
    ...
    assert!(
        super::mark_hls_provisioning_handoff_discontinuity(&app_state, &input, &stream_identity, None, 2_000,).await
    );
...
fn test_segment_entry(proxy_session_id: &ProxySessionId, proxy_seq: u64, status: SegmentCacheStatus) -> SegmentEntry {
    SegmentEntry {
        origin_key: OriginSegmentKey {
            origin_epoch: 0,
            effective_host_id: 0,
...
async fn hls_access_lease_idle_releases_user_but_keeps_origin_binding_and_queues() {
    ...
    app_state.active_provider.confirm_playback_activity(&binding.session_owner).await;
...
fn test_hls_access_context(proxy_session_id: ProxySessionId, access_lease_id: HlsAccessLeaseId) -> HlsAccessContext {
...
async fn hls_cache_manifest_context_restores_leased_archive_origin() -> Result<(), StatusCode> {
    ...
    let context = super::resolve_hls_playback_manifest_request_context(&app_state, &access, &HeaderMap::new()).await?;
...
async fn shared_request_flow_entry(fixture: &SharedRequestFlowFixture, fingerprint: &Fingerprint) -> Response<Body> {
...
fn cache_enabled_legacy_hls_route_only_allows_existing_m3u_catchup_session() {
    assert!(super::legacy_hls_route_allowed_with_cache(true, Some("m3u-catchup|session"), Some("m3u-catchup|session")));
    assert!(super::legacy_hls_route_allowed_with_cache(true, Some("catchup|session"), Some("catchup|session")));
    assert!(!super::legacy_hls_route_allowed_with_cache(true, Some("m3u-catchup|session"), Some("m3u-catchup|other")));
...
async fn hls_entry_origin_reservation_blocks_foreign_sessions_only_after_media_confirmation() {
    ...
    app_state.connection_manager.release_provider_handle(other_handle).await;
    ...
    app_state.active_provider.confirm_playback_activity(session_owner).await;
    assert!(
        app_state
            .active_provider
            .acquire_connection_with_grace_for_session(
...
fn normal_manifest_body(proxy_session_id: &str) -> String { normal_manifest_body_from_sequence(proxy_session_id, 0, 6) }
...
async fn request_response(app_state: Arc<AppState>, method: Method, uri: &str, range: Option<&str>) -> Response<Body> {
