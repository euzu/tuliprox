...
fn copy_platform_acl(source: &Path, target: &Path) -> io::Result<()> {
    ...
    let written =
        unsafe { libc::setxattr(target.as_ptr(), ACL_NAME.as_ptr().cast(), acl.as_ptr().cast(), acl.len(), 0) };
    if written == 0 {
        Ok(())
    } else {
        Err(io::Error::last_os_error())
                ...
                && matches!(tokio::fs::try_exists(&backup_path).await, Ok(true))
            {
                tokio::fs::rename(&backup_path, target).await.map_err(|restore_err| {
                    io::Error::other(format!(
                        "File replacement failed ({replace_err}); restoring the original failed: {restore_err}"
                    ))
                })?;
            }
            Err(replace_err)
...
pub async fn csv_patch_batch_update_exp_dates(
    ...
    let updates_by_name =
        updates.iter().map(|update| (update.account_name.as_ref(), update)).collect::<HashMap<_, _>>();
...
input_1;user;password;http://provider.tv;1;0;keep-me;old-enabled;old-expiry;trailing\n";
    ...
    async fn alias_backup_preserves_posix_acl() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let status = std::process::Command::new("setfacl")
            .args(["-m", "u:12345:r", csv_path.to_string_lossy().as_ref()])
            .status();
        ...
        let before =
            std::process::Command::new("getfacl").args(["-cp", csv_path.to_string_lossy().as_ref()]).output()?;

        csv_backup_file(&csv_path, backup_dir.to_string_lossy().as_ref()).await?;

        let backup_path =
            tokio::fs::read_dir(&backup_dir).await?.next_entry().await?.ok_or("alias backup is missing")?.path();
        let after =
            std::process::Command::new("getfacl").args(["-cp", backup_path.to_string_lossy().as_ref()]).output()?;
