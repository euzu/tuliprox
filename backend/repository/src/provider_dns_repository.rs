...
pub async fn load_dns_resolved_store_from_path(
    path: &Path,
) -> Result<Option<DnsResolvedStore>, DnsResolvedStoreLoadError> {
...
async fn recover_dns_resolved_store_from_tmp(
    path: &Path,
) -> Result<Option<DnsResolvedStore>, DnsResolvedStoreLoadError> {
    ...
    match fs::rename(&tmp_path, path).await {
        ...
        Err(err) => {
            warn!("Recovered DNS resolved store from '{}' but could not move it into place: {err}", tmp_path.display());
        }
    }
    Ok(Some(store))
}
...
pub async fn persist_dns_resolved_store(path: &Path, store: &DnsResolvedStore) -> Result<(), String> {
    ...
    if let Err(err) = fs::rename(&tmp_path, path).await {
        ...
        #[cfg(not(windows))]
        {
            ...
            return Err(format!("rename temp file '{}' -> '{}' failed: {err}", tmp_path.display(), path.display()));
        }
    }

    debug!("Persisted DNS resolved store to '{}' (providers={})", path.display(), store.providers.len());
    Ok(())
}
...
fn prune_store_to_enabled_provider_names(
    ...
    store.providers.retain(|provider_name, _| enabled_provider_names.contains(provider_name));
...
pub async fn prune_persisted_dns_resolved_to_runtime(app_config: &Arc<AppConfig>) {
    ...
    let mut store = match load_dns_resolved_store_from_path(&path).await {
        ...
        Ok(None) => return,
        Err(err) => {
            warn!("Failed to load DNS resolved store for runtime-prune '{}': {err}", path.display());
            return;
        }
    };
    ...
    if let Err(err) = persist_dns_resolved_store(&path, &store).await {
        warn!("Failed to persist DNS resolved store after runtime-prune '{}': {err}", path.display());
    } else {
        info!("Pruned {removed} stale provider(s) from DNS resolved store '{}'", path.display());
    }
}
...
pub fn spawn_dns_resolved_writer(
    ...
    tokio::spawn(async move {
        ...
        if removed_on_start > 0 {
            ...
            if let Err(err) = persist_dns_resolved_store(&path, &store).await {
                warn!("Failed to persist DNS resolved store after startup prune '{}': {err}", path.display());
            }
        }
        let mut dirty = false;
        let mut dirty_updates = 0usize;
        let mut flush_before_exit = false;
        let mut flush_timer = std::pin::pin!(tokio::time::sleep(Duration::from_secs(DNS_WRITER_FLUSH_INTERVAL_SECS,)));
        let mut flush_timer_active = false;

        loop {
            if !is_dns_writer_generation_current(generation) {
                debug!("Stopping stale DNS resolved writer generation {} for '{}'", generation, path.display());
                break;
            }
            tokio::select! {
                () = cancel.cancelled() => {
        ...
        if flush_before_exit && dirty {
            let _ = prune_store_to_runtime_enabled_providers(&app_config, &mut store);
            if let Err(err) = persist_dns_resolved_store(&path, &store).await {
                warn!("Failed to persist DNS resolved store during writer shutdown '{}': {err}", path.display());
            }
        }
    });
...
mod tests {
    ...
    async fn persist_and_load_dns_resolved_store_roundtrip() {
        ...
        let loaded =
            load_dns_resolved_store_from_path(&path).await.expect("load should succeed").expect("store should exist");

        assert_eq!(loaded.providers.len(), 1);
        assert_eq!(
            loaded.providers.get("provider-a").and_then(|hosts| hosts.get("example.com")),
