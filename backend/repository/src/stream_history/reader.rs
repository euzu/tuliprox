use crate::stream_history::storage::{
    async_read_and_verify_block_magic, async_read_and_verify_file_magic, async_read_framed, deserialize_named,
    read_and_verify_block_magic, read_and_verify_file_magic, read_framed, BlockHeaderBody, FileHeaderBody, BLOCK_MAGIC,
...
use tokio::{
    ...
    io::{AsyncReadExt, AsyncSeekExt, BufReader as TokioBufReader},
};
    ...
    fn read_next_block(&mut self) -> io::Result<Option<()>> {
        ...
        loop {
            ...
            if block_header.payload_len as usize > MAX_BLOCK_PAYLOAD_SIZE {
                eprintln!(
                    "Warning: {}: block payload_len {} exceeds MAX_BLOCK_PAYLOAD_SIZE, skipping",
            ...
            if actual_crc != block_header.payload_crc {
                eprintln!(
                    "Warning: {}: payload CRC mismatch (expected {:08x}, got {:08x}), attempting recovery",
                ...
                if b(i) == magic[0] && b(i + 1) == magic[1] && b(i + 2) == magic[2] && b(i + 3) == magic[3] {
                    ...
                    let block_header: BlockHeaderBody = match read_framed::<_, BlockHeaderBody>(&mut chained) {
                        Ok(h) if (h.payload_len as usize) <= MAX_BLOCK_PAYLOAD_SIZE => {
                            eprintln!(
                                "Warning: {}: recovered at block with {} records",
                                self.file_path, h.record_count
                            );
                            h
                        }
                        Ok(h) => {
                            eprintln!(
                                "Warning: {}: recovery candidate rejected (payload_len={})",
                                self.file_path, h.payload_len
                            );
                            continue;
                        }
                        Err(_) => continue,
                    ...
                    if actual_crc != block_header.payload_crc {
                        eprintln!(
                            "Warning: {}: recovery candidate CRC mismatch (expected {:08x}, got {:08x}), skipping",
                            self.file_path, block_header.payload_crc, actual_crc
                        );
                        continue;
                    }
...
mod tests {
    ...
    use tuliprox_core::model::RECORD_SCHEMA_VERSION;

    fn make_test_record(ts: u64, username: &str) -> StreamHistoryRecord {
        let mut r = crate::stream_history::tests::make_base_test_record();
        r.event_ts_utc = ts;
    ...
    fn test_iterator_yields_all_records() {
        let records =
            vec![make_test_record(1000, "alice"), make_test_record(2000, "bob"), make_test_record(3000, "carol")];
    ...
    fn test_archive_lz4_readable() {
        use crate::stream_history::archive::archive_pending_file;
        use tempfile::TempDir;

        let tmp = TempDir::new().unwrap();
        let records = vec![make_test_record(1000, "alice"), make_test_record(2000, "bob")];
