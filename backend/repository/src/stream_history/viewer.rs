use super::{
    extract_day_from_filename, read_and_verify_file_magic, read_framed, FileHeaderBody, StreamHistoryFileReader,
};
use serde::Deserialize;
use shared::{
    model::{FieldFilter, SearchFieldKind},
    utils::Internable,
};
use std::{
    collections::HashMap,
...
use tuliprox_core::model::StreamHistoryRecord;
...
pub fn parse_date_or_datetime(input: &str) -> Result<u64, String> {
    ...
    if let Ok(date) = chrono::NaiveDate::parse_from_str(trimmed, "%Y-%m-%d") {
        let dt = date.and_hms_opt(0, 0, 0).ok_or_else(|| format!("Invalid date: '{trimmed}'"))?;
    ...
    Err(format!("Invalid date format: '{trimmed}'. Expected: YYYY-MM-DD, YYYY-MM-DD HH:MM, or YYYY-MM-DD HH:MM:SS"))
}
...
const NUMERIC_FIELDS: &[&str] =
    &["session_id", "virtual_id", "provider_id", "provider_http_status", "shared_stream_id"];
...
fn get_record_field<'a>(record: &'a StreamHistoryRecord, field: &str) -> RecordFieldValue<'a> {
    match field {
        ...
        "item_type" => {
            RecordFieldValue::ArcStr(record.item_type.as_ref().map(ToString::to_string).map(Internable::intern))
        }
        ...
        "shared_joined_existing" => {
            RecordFieldValue::String(record.shared_joined_existing.map(|shared| if shared { "true" } else { "false" }))
        }
        "disconnect_reason" => {
            RecordFieldValue::ArcStr(record.disconnect_reason.as_ref().map(|r| r.to_string().intern()))
        }
        "connect_failure_reason" => {
            RecordFieldValue::ArcStr(record.connect_failure_reason.as_ref().map(|r| r.to_string().intern()))
        }
...
fn load_query(input: &str) -> Result<StreamHistoryQuery, String> {
    let json_str = if let Some(file_path) = input.strip_prefix('@') {
        fs::read_to_string(file_path).map_err(|e| format!("Failed to read query file '{file_path}': {e}"))?
    } else {
        input.to_string()
    };
...
pub async fn discover_files(dir: &Path, time_range: &TimeRange) -> io::Result<Vec<HistoryFile>> {
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        files.push(HistoryFile { path, partition_day: header.partition_day_ts_utc, is_archive });
    }

    // Sort: primary by partition_day, secondary archive before pending
    files.sort_by(|a, b| a.partition_day.cmp(&b.partition_day).then(b.is_archive.cmp(&a.is_archive)));
...
fn stream_output(files: &[HistoryFile], time_range: &TimeRange, filters: &CompiledFilter) {
    let (range_start, range_end) = *time_range;
...
mod tests {
    use super::*;
    use shared::{
        model::{DisconnectReason, FailureStage, StreamHistoryEventType},
        utils::Internable,
    };
    ...
    fn test_parse_datetime_no_seconds() {
        ...
        let expected = chrono::NaiveDate::from_ymd_opt(2026, 3, 22)
            .unwrap()
            .and_hms_opt(14, 30, 0)
            .unwrap()
            .and_utc()
    ...
    fn test_single_date_expands_to_full_day() {
        let query = StreamHistoryQuery { from: Some("2026-03-22".to_string()), to: None, path: None, filter: None };
    ...
    fn test_single_datetime_expands_to_full_day() {
        ...
        let query =
            StreamHistoryQuery { from: Some("2026-03-22 14:30".to_string()), to: None, path: None, filter: None };
        let (start, end) = resolve_time_range(&query).unwrap();
        assert_eq!(end - start, SECS_PER_DAY - 1);
    }
    ...
    fn test_no_dates_is_error() {
        let query = StreamHistoryQuery { from: None, to: None, path: None, filter: None };
