use crate::stream_history::{
    archive::finalize_and_archive, write_framed, BlockHeaderBody, CompressionKind, FileHeaderBody, RecordEncodingKind,
...
use tuliprox_core::{
    model::{StreamHistoryConfig, StreamHistoryRecord, RECORD_SCHEMA_VERSION},
    utils::{current_utc_day, now_utc_secs, utc_day_from_secs, SECS_PER_DAY},
};
            ...
            match cmd {
                WriterCommand::Record(record) => {
                    ...
                    if record.partition_day_utc != state.current_day {
                        if let Err(e) = state
                            .flush_and_rollover_to(&record.partition_day_utc, &self.config, self.writer_instance_id)
                            .await
                        {
                    ...
                    if state.batch.len() >= self.config.stream_history_batch_size {
                        if let Err(e) = state.flush_batch().await {
                            error!(
                                "Stream history batch flush failed: {e}. Dropped events: {}",
                ...
                WriterCommand::Shutdown(resp) => {
                    ...
                    info!(
                        "Stream history writer shut down. Total blocks: {}, records: {}",
...
impl WriterState {
    ...
    async fn flush_batch(&mut self) -> io::Result<()> {
        ...
        let (first_ts, last_ts) = self.batch.iter().fold((u64::MAX, 0u64), |(min_ts, max_ts), record| {
        ...
        let block_magic = crate::stream_history::BLOCK_MAGIC;
        if let Err(e) = async {
    ...
    async fn flush_and_rollover_to(
        &mut self,
        target_day: &str,
        config: &StreamHistoryConfig,
        writer_instance_id: u64,
...
async fn open_or_create_pending_file(path: &PathBuf, day: &str, writer_instance_id: u64) -> io::Result<fs::File> {
    ...
    if file.metadata().await.map_or(0, |m| m.len()) == 0 {
        ...
        let mut header_buf = Vec::new();
        write_framed(&mut header_buf, &header).map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))?;

        let file_magic = crate::stream_history::FILE_MAGIC;
        let mut f = file;
...
mod tests {
    use super::*;
    use crate::{
        read_and_verify_block_magic, read_and_verify_file_magic, read_framed, BlockHeaderBody, FileHeaderBody,
    };
    use shared::{
        model::{PlaylistItemType, StreamHistoryEventType},
        utils::Internable,
    };
    use std::{io::BufReader, sync::atomic::Ordering};
    use tempfile::TempDir;
    use tuliprox_core::utils::secs_until_next_utc_midnight;
