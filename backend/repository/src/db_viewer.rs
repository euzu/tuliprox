use super::{
    BPlusTreeDiskIterator, BPlusTreeQuery, MetadataRetryDbKey, MetadataRetryDbValue, QosSnapshotRecord, VirtualIdRecord,
};
...
pub fn db_viewer(args: &DbViewerArgs<'_>) {
    let requests = [
        DumpRequest { filename: args.xtream_filename, label: "xtream", dump_fn: dump_xtream_db },
        DumpRequest { filename: args.m3u_filename, label: "m3u", dump_fn: dump_m3u_db },
        DumpRequest { filename: args.epg_filename, label: "epg", dump_fn: dump_epg_db },
        DumpRequest { filename: args.tim_filename, label: "target_id_mapping", dump_fn: dump_target_mapping_db },
        DumpRequest {
            filename: args.metadata_status_filename,
            label: "metadata_status",
            dump_fn: dump_metadata_status_db,
        },
        DumpRequest { filename: args.qos_snapshot_filename, label: "qos_snapshot", dump_fn: dump_qos_snapshot_db },
    ];
...
mod tests {
    ...
    use crate::{BPlusTree, QosSnapshotDailyBucket, QosSnapshotRecord, QosSnapshotWindow};
    use base64::{engine::general_purpose, Engine as _};
    ...
    fn dump_qos_snapshot_db_reads_bplustree_records() {
        ...
        let record = QosSnapshotRecord {
            ...
            window_24h: QosSnapshotWindow { score: 87, confidence: 70, ..QosSnapshotWindow::default() },
