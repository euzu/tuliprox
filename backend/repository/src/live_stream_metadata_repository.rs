use crate::{
    build_input_storage_path, get_input_m3u_playlist_file_path, xtream_get_file_path, BPlusTreeQuery, BPlusTreeUpdate,
};
use shared::{
    error::TuliproxError,
    model::{
        LiveStreamProperties, M3uPlaylistItem, PlaylistItemType, StreamProperties, XtreamCluster, XtreamPlaylistItem,
    },
};
use std::sync::Arc;
use tuliprox_core::{
...
pub enum LiveBitratePersistenceOutcome {
    Updated,
...
#[derive(Debug, Clone, Copy, Eq, PartialEq)]
pub enum LiveBitratePersistenceInapplicableReason {
    InvalidBitrate,
...
pub async fn load_input_live_bitrate_bps(
...
pub async fn persist_input_live_bitrate_bps(
    app_config: &Arc<AppConfig>,
    input: &ConfigInput,
    stream_ref: &str,
    bitrate_bps: u32,
...
mod tests {
    ...
    use crate::{
        build_input_storage_path, get_input_m3u_playlist_file_path, xtream_get_file_path, BPlusTree, BPlusTreeQuery,
    };
    ...
    async fn loads_positive_xtream_and_m3u_live_bitrate_and_normalizes_zero() {
        ...
        let xtream_input =
            ConfigInput { name: Arc::from("xtream-input"), input_type: InputType::Xtream, ..ConfigInput::default() };
        ...
        xtream_tree.store(&xtream_get_file_path(&xtream_storage, XtreamCluster::Live)).expect("xtream tree");

        let m3u_input =
            ConfigInput { name: Arc::from("m3u-input"), input_type: InputType::M3u, ..ConfigInput::default() };
        ...
        let m3u_item = shared::model::M3uPlaylistItem::from(&playlist_item(&m3u_input, "channel-a", 2_000_000));
        let m3u_zero_item = shared::model::M3uPlaylistItem::from(&playlist_item(&m3u_input, "channel-zero", 0));
        ...
        m3u_tree.store(&get_input_m3u_playlist_file_path(&m3u_storage, &m3u_input.name)).expect("m3u tree");
        ...
        assert_eq!(
            load_input_live_bitrate_bps(&app_config, &m3u_input, "channel-zero").await.expect("m3u zero read"),
            None
        );
    }
    ...
    async fn missing_invalid_or_unsupported_input_metadata_is_unknown() {
        ...
        let app_config = test_app_config(temp.path().to_string_lossy().as_ref());
        let xtream_input =
            ConfigInput { name: Arc::from("missing-xtream"), input_type: InputType::Xtream, ..ConfigInput::default() };
        let unsupported_input =
            ConfigInput { name: Arc::from("library-input"), input_type: InputType::Library, ..ConfigInput::default() };

        assert_eq!(
            load_input_live_bitrate_bps(&app_config, &xtream_input, "not-numeric").await.expect("invalid stream ref"),
            None
        );
        assert_eq!(load_input_live_bitrate_bps(&app_config, &xtream_input, "42").await.expect("missing db"), None);
        assert_eq!(
            load_input_live_bitrate_bps(&app_config, &unsupported_input, "42").await.expect("unsupported input"),
            None
        );
    }
    ...
    async fn persists_only_higher_positive_live_bitrate_and_preserves_metadata() {
        ...
        assert_eq!(
            persist_input_live_bitrate_bps(&app_config, &xtream_input, "42", 0).await.expect("zero bitrate ignored"),
            LiveBitratePersistenceOutcome::PermanentlyInapplicable(
                LiveBitratePersistenceInapplicableReason::InvalidBitrate
            )
        );
        ...
        assert_eq!(
            load_input_live_bitrate_bps(&app_config, &m3u_input, "channel-a").await.expect("m3u bitrate read"),
            Some(1_750_000)
        );
    }
    ...
    async fn persistence_distinguishes_missing_database_and_stream_item() {
        ...
        let mut tree = BPlusTree::new();
        tree.insert(7_u32, shared::model::XtreamPlaylistItem::from(&playlist_item(&input, "7", 1_000_000)));
        tree.store(&xtream_get_file_path(&storage, XtreamCluster::Live)).expect("xtream tree");

        assert_eq!(
            persist_input_live_bitrate_bps(&app_config, &input, "42", 2_000_000).await.expect("missing item outcome"),
    ...
    async fn persistence_distinguishes_permanently_inapplicable_inputs_and_metadata() {
        ...
        tree.store(&get_input_m3u_playlist_file_path(&storage, &m3u_input.name)).expect("m3u tree");
