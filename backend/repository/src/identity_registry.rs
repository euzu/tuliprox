...
//! persisted atomically via the [`atomic_json_store`](tuliprox_core::utils::atomic_json_store)
...
pub enum BootstrapOutcome {
    ...
    /// restored.
    FailClosed { persisted_user_ids: Vec<UserId>, reason: FailClosedReason },
...
impl IdentityRegistry {
        ...
        match Self::load_or_detect_failure(&path).await {
            Ok(Some(state)) => {
                ...
                let outcome = if migrated.version < CURRENT_REGISTRY_VERSION {
                    BootstrapOutcome::Migrated { migrated_to: migrated.version }
                } else {
                    BootstrapOutcome::Restored { current_principal_count: count }
                };
                let reg = Self { state: RwLock::new(migrated), path };
            ...
            Ok(None) => {
                ...
                if pre_scanned_user_ids.is_empty() {
                    let reg = Self { state: RwLock::new(PersistedIdentityRegistry::default()), path };
                    reg.sync_current_principals(current_web_users, current_api_users).await;
            ...
            Err(reason) => {
                ...
                (Self::empty(path), BootstrapOutcome::FailClosed { persisted_user_ids: pre_scanned_user_ids, reason })
            }
        }
    }
    ...
    pub fn empty(path: PathBuf) -> Self { Self { state: RwLock::new(PersistedIdentityRegistry::default()), path } }
    ...
    async fn sync_current_principals(&self, current_web_users: &[String], current_api_users: &[String]) {
    ...
    pub async fn rename(&self, old_username: &str, new_username: &str) -> Result<Option<UserId>, RegistryError> {
        ...
        let user_id = state.web_users.remove(&old_key).or_else(|| state.api_users.remove(&old_key));
    ...
    pub async fn restore_from_user_ids(
        ...
        let state = PersistedIdentityRegistry { version: CURRENT_REGISTRY_VERSION, web_users, api_users };
        *self.state.write().await = state.clone();
        Self::write_to_disk(&self.path, &state).await.map_err(RegistryError::Persist)
    }
    ...
    async fn write_to_disk(path: &Path, state: &PersistedIdentityRegistry) -> std::io::Result<()> {
        ...
        tuliprox_core::utils::atomic_json_store::write_json_atomic_to_tmp(path, &tmp, &content)
            .await
...
mod tests {
    ...
    async fn bootstrap_initializes_on_empty_storage() {
        ...
        let (reg, outcome) = IdentityRegistry::bootstrap(path.clone(), Vec::new(), &web_users(3), &[]).await;
    ...
    async fn bootstrap_restores_and_preserves_existing_ids() {
        ...
        let (reg, _) = IdentityRegistry::bootstrap(path.clone(), Vec::new(), &web_users(2), &[]).await;
        let first_id = reg.lookup_by_username("user-0").await.expect("registered");
        drop(reg);

        // Second bootstrap reads the persisted file. The existing
        // `UserId` is preserved.
        let (reg2, outcome) = IdentityRegistry::bootstrap(path, Vec::new(), &web_users(2), &[]).await;
    ...
    async fn bootstrap_fails_closed_when_user_ids_persist_but_registry_missing() {
        ...
        let (reg, outcome) = IdentityRegistry::bootstrap(path.clone(), pre_scanned.clone(), &[], &[]).await;
        assert!(matches!(outcome, BootstrapOutcome::FailClosed { reason: FailClosedReason::Missing, .. }));
    ...
    async fn bootstrap_fails_closed_on_corrupt_registry() {
        ...
        let (_, outcome) = IdentityRegistry::bootstrap(path, pre_scanned.clone(), &[], &[]).await;
        assert!(matches!(outcome, BootstrapOutcome::FailClosed { reason: FailClosedReason::Corrupt, .. }));
    }
    ...
    async fn register_rejects_empty_username() {
        ...
        let path = dir.path().join("web_user_ids.json");
        let (reg, _) = IdentityRegistry::bootstrap(path, Vec::new(), &[], &[]).await;
        assert!(matches!(reg.register("").await.unwrap_err(), RegistryError::EmptyUsername));
        assert!(matches!(reg.register("   ").await.unwrap_err(), RegistryError::EmptyUsername));
    }
    ...
    async fn restore_from_user_ids_rebuilds_registry_for_repair() {
        ...
        reg.restore_from_user_ids(web.clone(), HashMap::new()).await.expect("restore");
    ...
    async fn concurrent_register_yields_distinct_ids() {
        ...
        for i in 0..20u64 {
            ...
            handles.push(tokio::spawn(async move { reg.register(&format!("concurrent-{i}")).await }));
        }
