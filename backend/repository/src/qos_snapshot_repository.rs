use crate::{BPlusTree, BPlusTreeUpdate, FlushPolicy};
...
use tuliprox_core::{model::macros, utils::arc_str_serde};
...
impl QosSnapshotRepository {
    ...
    pub fn open(storage_dir: &Path) -> io::Result<Self> {
        ...
        let mut checkpoint_tree =
            BPlusTreeUpdate::<u8, QosAggregationCheckpoint>::try_new_with_backoff(&checkpoint_path)?;
    ...
    pub fn get_snapshot(&self, stream_identity_key: &str) -> io::Result<Option<QosSnapshotRecord>> {
        ...
        tree.query(&stream_identity_key.to_string()).map_err(|err| io::Error::other(err.to_string()))
    }
    ...
    pub fn get_snapshot_read_only(
        storage_dir: &Path,
        stream_identity_key: &str,
    ) -> io::Result<Option<QosSnapshotRecord>> {
        let snapshot_path = Self::snapshot_db_path(storage_dir);
    ...
    pub fn load_checkpoint(&self) -> io::Result<QosAggregationCheckpoint> {
        ...
        Ok(tree.query(&CHECKPOINT_KEY).map_err(|err| io::Error::other(err.to_string()))?.unwrap_or_default())
    }
...
mod tests {
    ...
    use serde::{Deserialize, Serialize};
    use shared::{model::PlaylistItemType, utils::Internable};
    ...
    fn make_test_snapshot(identity_key: &str, score: u8) -> QosSnapshotRecord {
        QosSnapshotRecord {
            ...
            window_24h: QosSnapshotWindow { connect_count: 3, score, confidence: 60, ..QosSnapshotWindow::default() },
    ...
    fn qos_snapshot_repository_roundtrips_snapshot_and_checkpoint() {
        ...
        let loaded =
            repo.get_snapshot("stream-a").expect("get snapshot should succeed").expect("snapshot should exist");
    ...
    fn qos_aggregation_checkpoint_defaults_missing_compaction_timestamp() {
        ...
        let bytes = crate::serialize_named(&legacy).expect("legacy checkpoint should serialize");
        let checkpoint: QosAggregationCheckpoint =
            crate::deserialize_named(&bytes).expect("legacy checkpoint should deserialize");
    ...
    fn qos_snapshot_repository_deletes_snapshot() {
        ...
        assert!(repo.get_snapshot("stream-a").expect("get snapshot should succeed").is_none());
    }
    ...
    fn entity_to_dto_macro_round_trips_qos_snapshot_window_fields() {
        ...
        assert_eq!(round_tripped.avg_provider_reconnect_count, entity.avg_provider_reconnect_count);
    }
    ...
    fn entity_to_dto_macro_round_trips_qos_snapshot_daily_bucket_fields() {
        ...
        assert_eq!(round_tripped.total_provider_reconnect_samples, entity.total_provider_reconnect_samples);
