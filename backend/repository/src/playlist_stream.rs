...
use tuliprox_core::utils::FileReadGuard;

pub enum PlaylistIteratorReader<K, V, SortKey> {
    Sorted { iterator: SortedIndexIterator<K, V, SortKey>, fallback_path: std::path::PathBuf, yielded: bool },
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        match BPlusTreeQuery::try_new(&path) {
            ...
            Err(tree_error) => Some(Err(io::Error::new(
                tree_error.kind(),
                format!(
                    "sorted index failed before its first entry ({index_error}); tree fallback failed: {tree_error}"
                ),
            ))),
        }
    }
...
impl<T> LockedReceiverStream<T> {
    pub fn new(rx: mpsc::Receiver<T>, guard: FileReadGuard) -> Self {
        Self { rx: ReceiverStream::new(rx), _guard: Some(guard) }
    }
    ...
    SortKey: Ord + for<'de> Deserialize<'de>,
{
    let query = BPlusTreeQuery::<K, V>::try_new(path)
        .map_err(|err| TuliproxError::Config(format!("Could not open BPlusTreeQuery {} - {err}", path.display())))?;
    ...
    let query = BPlusTreeQuery::<K, V>::try_new(path)
        .map_err(|err| TuliproxError::Config(format!("Could not reopen BPlusTreeQuery {} - {err}", path.display())))?;
...
mod tests {
    ...
    fn corrupt_first_index_entry_falls_back_before_yielding() -> io::Result<()> {
        ...
        *bytes.get_mut(72).ok_or_else(|| io::Error::other("index body missing"))? ^= 1;
        ...
        let reader = open_playlist_reader::<u32, String, usize>(&database, &index, None).map_err(io::Error::other)?;
    ...
    fn corrupt_late_index_entry_does_not_hide_following_entries() -> io::Result<()> {
        ...
        let mut reader = open_playlist_reader::<u32, String, u32>(&database, &index, None).map_err(io::Error::other)?;
