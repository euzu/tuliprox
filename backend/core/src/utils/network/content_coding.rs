...
pub enum OutboundContentCodingPolicy {
    #[default]
    Inherit,
    Identity,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct ContentCodingObservation {
...
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq)]
pub enum ContentCodingDetection {
...
pub fn force_accept_encoding_identity(headers: &mut HeaderMap) {
    headers.insert(header::ACCEPT_ENCODING, HeaderValue::from_static("identity"));
}

pub fn apply_outbound_content_coding_policy(headers: &mut HeaderMap, policy: OutboundContentCodingPolicy) {
...
pub fn parse_content_encoding_tokens(headers: &HeaderMap) -> Result<Vec<String>, InvalidContentEncodingHeader> {
...
pub fn parse_content_codings(headers: &HeaderMap) -> Result<Vec<ContentCoding>, ContentCodingError> {
...
pub async fn decode_response_to_identity(
...
pub async fn read_to_end_limited<R>(reader: &mut R, max_bytes: usize) -> Result<Vec<u8>, ContentBodyReadError>
where
    R: AsyncRead + Unpin,
{
...
pub async fn read_utf8_limited<R>(reader: &mut R, max_bytes: usize) -> Result<String, ContentBodyReadError>
where
    R: AsyncRead + Unpin,
{
...
pub fn normalize_headers_after_content_decoding(headers: &mut HeaderMap) {
    for name in [
        CONTENT_ENCODING,
        CONTENT_LENGTH,
        CONTENT_RANGE,
...
pub fn content_decoding_error_from_io(error: &io::Error) -> Option<&ContentDecodingIoError> {
...
pub fn is_http_body_transport_error(error: &io::Error) -> bool {
...
pub fn hls_origin_content_coding_log_fields(
    observation: ContentCodingObservation,
    object_kind: HlsOriginContentCodingObjectKind,
    range_requested: bool,
    source: HlsOriginContentCodingSource,
