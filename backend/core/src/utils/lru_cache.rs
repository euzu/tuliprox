use crate::utils::{decode_base64_string, encode_base64_hash, encode_base64_string, trace_if_enabled, traverse_dir};
...
impl LRUResourceCache {
    ...
    pub fn get_size_text(&self) -> String {
        format!(
            "{} / {}",
            human_readable_byte_size(self.current_size as u64),
            human_readable_byte_size(self.capacity as u64)
        )
    }
    ...
    pub fn get_content(&mut self, url: &str) -> Option<(PathBuf, Option<String>)> {
        ...
        if let Some((path, mime_type, size, gen)) = self.cache.get_mut(&key) {
            if path.exists() {
                trace_if_enabled!(
                    "Responding resource from cache with key: {key} for url: {}",
            ...
            trace_if_enabled!(
                "Cache inconsistency: file missing for key: {key}, url: {}",
                sanitize_sensitive_info(url)
            );
