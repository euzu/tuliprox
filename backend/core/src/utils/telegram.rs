...
pub async fn telegram_send_message(
    ...
    let url = match Url::parse(&raw_url_str) {
        ...
        Err(e) => {
            ...
            return TelegramSendResult { delivered: false, parse_error: false };
        }
    };
        ...
        for attempt in 0..=MAX_RETRIES_PER_CHUNK {
            ...
            match result {
                Ok(response) => {
                    ...
                    let retriable_status = status == StatusCode::TOO_MANY_REQUESTS || status.is_server_error();
                    if retriable_status && attempt < MAX_RETRIES_PER_CHUNK {
                        if status == StatusCode::TOO_MANY_REQUESTS {
                            if let Some(retry_after_secs) = telegram_error.as_ref().and_then(extract_retry_after_secs) {
                        ...
                        warn!(
                            "Message chunk {}/{} send attempt {}/{} failed for {chat_id}: {}; retrying in {}s",
                            i + 1,
                            chunks.len(),
                            attempt + 1,
                            MAX_RETRIES_PER_CHUNK + 1,
    ...
    TelegramSendResult { delivered: all_delivered, parse_error }
}

fn is_parse_entities_error(description: &str) -> bool {
    description.to_ascii_lowercase().contains("can't parse entities")
}
...
mod tests {
    ...
    fn test_parse_retry_after_from_description() {
        ...
        assert_eq!(parse_retry_after_from_description("Too Many Requests"), None);
        assert_eq!(parse_retry_after_from_description("Too Many Requests: retry after now"), None);
        assert_eq!(parse_retry_after_from_description("Too Many Requests: Retry After 30"), Some(30));
    }
}
