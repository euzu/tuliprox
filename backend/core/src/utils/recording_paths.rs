...
pub async fn no_follow_path_in_root(root: &Path, target: &Path) -> Option<std::fs::Metadata> {
...
pub async fn clean_empty_parents(path: &Path, root: &Path) -> Result<(), RecordingPathError> {
    ...
    while let Some(dir) = current {
        ...
        if let Some(r) = canonical_root.as_ref() {
            if fs::canonicalize(dir_for_check).await.is_ok_and(|c| c == *r) {
                stop_here = true;
            }
        }
        if stop_here {
...
mod tests {
    ...
    fn validate_relative_path_rejects_absolute() {
        assert!(matches!(validate_relative_path(Path::new("/etc/passwd")).unwrap_err(), RecordingPathError::Absolute));
    }
    ...
    fn validate_relative_path_rejects_nul_byte() {
        ...
        assert!(matches!(validate_relative_path(Path::new(&bad)).unwrap_err(), RecordingPathError::NulByte));
    }
    ...
    async fn open_partial_no_clobber_fails_when_file_exists() {
        ...
        assert!(
            matches!(result.unwrap_err(), RecordingPathError::Io(err) if err.kind() == io::ErrorKind::AlreadyExists)
        );
