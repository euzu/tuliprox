...
fn parse_linux_status_kib_value(line: &[u8]) -> Option<u64> {
    line.split(u8::is_ascii_whitespace).filter(|token| !token.is_empty()).nth(1).and_then(parse_ascii_u64_bytes)
}
...
fn read_fallback_memory_snapshot() -> Option<(u64, u64)> {
    ...
    sys.processes().get(&pid).map(|process| (process.memory(), process.virtual_memory()))
}
