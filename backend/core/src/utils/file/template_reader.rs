...
use log::{debug, warn};
use shared::{error::TuliproxError, model::TemplateDefinitionDto};
use std::{
    io::ErrorKind,
    path::{Path, PathBuf},
};
    ...
    match open_file(template_file) {
        ...
        Err(err) => {
            if err.kind() == ErrorKind::NotFound {
                debug!("Optional template file not found: {}", template_file.to_str().unwrap_or("?"));
                Ok(None)
            } else {
                warn!("Can't read template file {}: {err}", template_file.to_str().unwrap_or("?"));
    ...
    match std::fs::metadata(&path) {
        Ok(metadata) => {
            if metadata.is_file() {
                ...
                warn!("Template path exists but is neither file nor directory: {}", path.to_string_lossy());
                Ok(None)
            }
        }
        Err(err) => {
            if err.kind() == ErrorKind::NotFound {
                ...
                warn!("Can't read template path metadata for {}: {err}", path.to_string_lossy());
                Err(TuliproxError::Config(format!(
