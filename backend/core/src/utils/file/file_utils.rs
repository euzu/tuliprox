...
use shared::{
    defaults::{
        API_PROXY_FILE, CONFIG_FILE, CONFIG_PATH, DEFAULT_WEB_DIR, MAPPING_FILE, SOURCE_FILE, TEMPLATE_FILE, USER_FILE,
        USER_GROUP_FILE,
    },
...
pub fn get_default_web_root_path_for_home(home_path: &Path) -> PathBuf {
    if let Some(web_root) = env::var(DEFAULT_WEB_ROOT_ENV_VAR).ok().filter(|p| !p.trim().is_empty()) {
        ...
        return if configured.is_absolute() { configured } else { home_path.join(configured) };
    }
...
pub fn resolve_directory_path(input: &str) -> String {
    ...
    final_path.canonicalize().map_or_else(
        |_| {
...
pub fn sanitize_filename(file_name: &str) -> String {
    file_name.chars().map(|c| if c.is_alphanumeric() || c == '_' || c == '-' { c } else { '_' }).collect()
}
...
pub async fn create_new_file_for_write(path: &Path) -> tokio::io::Result<tokio_fs::File> {
    tokio_fs::OpenOptions::new().write(true).create(true).truncate(true).open(path).await
}
...
pub fn remove_file_if_exists(path: &Path) -> std::io::Result<()> {
    match fs::remove_file(path) {
        ...
        Err(error) => {
            Err(std::io::Error::new(error.kind(), format!("failed to remove file {}: {error}", path.display())))
        }
    }
}
...
pub fn prepare_file_path(persist: Option<&str>, storage_dir: &str, action: &str) -> Option<PathBuf> {
    let persist_file: Option<PathBuf> = persist.map(|persist_path| prepare_persist_path(persist_path, action));
...
pub fn cleanup_unlisted_files_with_suffix(keep_files: &Vec<PathBuf>, suffix: &str) -> std::io::Result<()> {
    ...
    for dir in dirs {
        for entry in (fs::read_dir(dir)?).flatten() {
            ...
            let delete = {
                let zero_size = entry.metadata().is_ok_and(|m| m.len() == 0);

                let suffix_match = path.file_name().and_then(|n| n.to_str()).is_some_and(|name| name.ends_with(suffix));
    ...
    let truncated_name = if file_name.chars().count() > max_len {
        ...
        if let Some(ext) = path.extension().and_then(|e| e.to_str()) {
            ...
            if max_len > ext_len {
                ...
                let name_without_ext = path.file_stem().and_then(|s| s.to_str()).unwrap_or_default();
...
pub fn normalize_string_path(path: &str) -> String {
    std::path::PathBuf::from(path).components().collect::<std::path::PathBuf>().to_string_lossy().to_string()
}

pub fn get_file_extension(path: &str) -> Option<String> {
    Path::new(path).extension().and_then(|s| s.to_str()).map(ToString::to_string)
}
...
mod tests {
    ...
    fn test_resolve_template_default_path_uses_config_dir() {
        ...
        let resolved =
            resolve_template_file_path(config_path.to_string_lossy().as_ref(), Some("./config/template.yml"));
    ...
    fn test_resolve_template_persist_file_for_directory_path() {
        let config_path = std::env::temp_dir().join("tuliprox").join("settings").join("config");
        let resolved = resolve_template_persist_file_path(Some("template.d"), config_path.to_string_lossy().as_ref());
