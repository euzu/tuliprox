use crate::utils::{arc_str_option_serde, encode_base64_hash, now_utc_secs, utc_day_from_secs};
use serde::{Deserialize, Serialize};
use shared::model::{
    ConnectFailureReason, DisconnectReason, FailureStage, PlaylistItemType, StreamHistoryEventType,
    StreamHistoryRecordDto, StreamInfo,
};
use std::sync::Arc;
...
impl StreamHistoryRecord {
    ...
    fn base(info: &StreamInfo, event_ts: u64) -> Self {
        Self {
            ...
            input_name: if info.channel.input_name.is_empty() { None } else { Some(info.channel.input_name.clone()) },
            ...
            container: info.channel.technical.as_ref().and_then(|t| {
                if t.container.is_empty() {
                    None
                } else {
                    Some(t.container.clone())
                }
            }),
            ...
            video_codec: info.channel.technical.as_ref().and_then(|t| {
                if t.video_codec.is_empty() {
                    None
                } else {
                    Some(t.video_codec.clone())
                }
            }),
            audio_codec: info.channel.technical.as_ref().and_then(|t| {
                if t.audio_codec.is_empty() {
                    None
                } else {
                    Some(t.audio_codec.clone())
                }
            }),
            audio_channels: info.channel.technical.as_ref().and_then(|t| {
                if t.audio_channels.is_empty() {
                    None
                } else {
                    Some(t.audio_channels.clone())
                }
            }),
            resolution: info.channel.technical.as_ref().and_then(|t| {
                if t.resolution.is_empty() {
                    None
                } else {
                    Some(t.resolution.clone())
                }
            }),
            fps: info
                .channel
                .technical
                .as_ref()
                .and_then(|t| if t.fps.is_empty() { None } else { Some(t.fps.clone()) }),
            connect_ts_utc: None,
