use crate::model::{
    config::cache::CacheConfig, macros, GeoIpConfig, QosAggregationConfig, RateLimitConfig, StreamConfig,
};
use regex::Regex;
use shared::{
    defaults::{
...
impl ReverseProxyDisabledHeaderConfig {
    pub fn should_remove(&self, header: &str) -> bool {
        ...
        self.custom_header.iter().any(|h| h.trim().eq_ignore_ascii_case(&header_lc))
    }
}
...
impl ResourceRetryConfig {
    pub fn get_retry_values(&self) -> (u32, u64, f64) {
        (
            ...
            if self.backoff_multiplier.is_finite() { self.backoff_multiplier.max(1.0) } else { 1.0 },
        )
    }
...
impl From<&ResourceRetryConfigDto> for ResourceRetryConfig {
    fn from(dto: &ResourceRetryConfigDto) -> Self {
        let multiplier = if dto.backoff_multiplier.is_finite() { dto.backoff_multiplier.max(1.0) } else { 1.0 };

        // Compile patterns, default to service-abuse if none or empty
        let patterns = dto.failover_redirect_patterns.as_ref().filter(|v| !v.is_empty()).map_or_else(
            default_failover_redirect_patterns,
            |patterns| {
                patterns
                    .iter()
                    .filter_map(|p| {
...
impl From<&ResourceRetryConfig> for ResourceRetryConfigDto {
    fn from(cfg: &ResourceRetryConfig) -> Self {
        let patterns: Vec<String> = cfg.failover_redirect_patterns.iter().map(|re| re.as_str().to_string()).collect();
        Self {
...
impl From<&HlsSegmentRepairSizeIncreaseConfigDto> for HlsSegmentRepairSizeIncreaseConfig {
    fn from(dto: &HlsSegmentRepairSizeIncreaseConfigDto) -> Self {
        Self { low_percent: dto.low_percent, medium_percent: dto.medium_percent, high_percent: dto.high_percent }
    }
}
...
impl From<&HlsCorruptSegmentWatchdogConfigDto> for HlsCorruptSegmentWatchdogConfig {
    fn from(dto: &HlsCorruptSegmentWatchdogConfigDto) -> Self {
        Self { mode: dto.mode, max_parallel_jobs: dto.max_parallel_jobs }
    }
}
...
impl From<&HlsCorruptSegmentWatchdogConfig> for HlsCorruptSegmentWatchdogConfigDto {
    fn from(config: &HlsCorruptSegmentWatchdogConfig) -> Self {
        Self { mode: config.mode, max_parallel_jobs: config.max_parallel_jobs }
    }
}
...
impl From<&HlsCacheConfigDto> for HlsCacheConfig {
    fn from(dto: &HlsCacheConfigDto) -> Self {
        Self {
            ...
            initial_manifest_wait_timeout_secs: dto.initial_manifest_wait_timeout_secs,
...
impl From<&ReverseProxyConfigDto> for ReverseProxyConfig {
    fn from(dto: &ReverseProxyConfigDto) -> Self {
        Self {
            ...
            resource_retry: dto.resource_retry.as_ref().map_or_else(ResourceRetryConfig::default, Into::into),
