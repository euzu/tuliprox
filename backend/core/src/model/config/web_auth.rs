...
impl WebAuthConfig {
    ...
    fn parse_groups(groupfile_path: &Path) -> Vec<RbacGroup> {
        match File::open(groupfile_path) {
            Ok(file) => {
                ...
                for line in reader.lines() {
                    ...
                    if name.eq_ignore_ascii_case("admin") {
                        warn!(
                            "groups file {}: 'admin' is reserved and cannot be redefined, skipping",
                            groupfile_path.display()
                        );
                        continue;
                    }
                    ...
                    for perm_name in perms_str.split(',').map(str::trim).filter(|perm| !perm.is_empty()) {
                        match permission_from_name(perm_name) {
                            ...
                            None => warn!(
                                "groups file {}: unknown permission '{perm_name}' in group '{name}', ignoring",
                                groupfile_path.display()
                            ),
                        }
                    }
                    ...
                    {
                        warn!(
                            "groups file {}: duplicate group '{name}' redefined, replacing prior definition",
                            groupfile_path.display()
                        );
            ...
            Err(err) => {
                error!(
                    "Could not read groups file {}: {err}, using only built-in admin permissions",
                    groupfile_path.display()
                );
                vec![]
            }
        ...
        for user in users {
            for group_name in &user.groups {
                ...
                let group_exists = self
                    .t_groups
                    .as_ref()
                    .is_some_and(|groups| groups.iter().any(|group| group.name.eq_ignore_ascii_case(group_name)));
...
mod tests {
    ...
    fn test_resolve_permissions_grouped_user() {
        ...
        let config = WebAuthConfig {
            ...
            t_groups: Some(vec![RbacGroup { name: "viewer".to_string(), permissions: viewer_perms }]),
        };
    ...
    fn test_resolve_permissions_multiple_groups_union() {
        ...
        let config = WebAuthConfig {
            ...
            t_groups: Some(vec![
                RbacGroup { name: "viewer".to_string(), permissions: viewer_perms },
                RbacGroup { name: "editor".to_string(), permissions: editor_perms },
            ]),
        };
