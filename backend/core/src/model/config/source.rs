...
use shared::{
    error::TuliproxError,
    model::{
        provider_saturation::build_group_lookup, ConfigProviderDto, ConfigSourceDto, DnsPrefer, DnsScheme,
        OnConnectErrorPolicy, OnResolveErrorPolicy, PatternTemplate, ProviderUrlSelectionPolicy, SourcesConfigDto,
    },
};
use std::{
    borrow::Cow,
...
impl ProviderDnsCache {
    pub fn preview_ip_from(&self, host: &str, ips: &[IpAddr]) -> Option<IpAddr> {
        ...
        let idx = guard.get(&host.to_ascii_lowercase()).map_or(0, |entry| entry.rr_index.load(Ordering::Relaxed) % len);
        Some(ips[idx])
    }
    ...
    pub fn select_ip_from(&self, host: &str, ips: &[IpAddr]) -> Option<IpAddr> {
        ...
        let idx = entry
            .rr_index
            .try_update(Ordering::Relaxed, Ordering::Relaxed, |i| Some((i + 1) % len))
            .unwrap_or_else(|i| i)
            % len;
        Some(ips[idx])
    }
    ...
    pub fn ip_count(&self, host: &str) -> usize {
        let guard = self.by_host.read();
        guard.get(&host.to_ascii_lowercase()).map_or(0, |entry| entry.ips.len())
    }
}
...
impl SourcesConfig {
    ...
    pub fn get_staged_input_for_provider(&self, provider_name: &Arc<str>) -> Option<&Arc<ConfigInput>> {
        self.inputs.iter().find(|input| {
            input.staged.as_ref().and_then(|staged| staged.for_input.as_ref()).is_some_and(|name| name == provider_name)
        })
    }
    ...
    pub fn validate_targets(&self, target_args: Option<&Vec<String>>) -> Result<ProcessTargets, TuliproxError> {
        ...
        if let Some(user_targets) = target_args {
            ...
            let missing_targets: Vec<String> =
                check_targets.iter().filter(|&(_, v)| *v == 0).map(|(k, _)| k.clone()).collect();
        ...
        Ok(ProcessTargets { enabled, inputs, targets, target_names })
    }
...
mod tests {
    ...
    fn snapshot_resolved_ordered_matches_url_order_and_stable_extras() {
        ...
        provider
            .store_resolved("cdn-c.example.net", vec!["203.0.113.30".parse::<IpAddr>().expect("ip parse should work")]);
        provider
            .store_resolved("cdn-a.example.net", vec!["203.0.113.10".parse::<IpAddr>().expect("ip parse should work")]);
