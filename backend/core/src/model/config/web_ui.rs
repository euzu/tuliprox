...
use shared::{
    ...
    error::TuliproxError,
    model::{
        view_type::ViewType, ContentSecurityPolicyConfigDto, StreamInfoConfigDto, StreamInfoFields,
        StreamInfoFieldsSet, WebUiConfigDto,
    },
};
...
impl From<&ContentSecurityPolicyConfig> for ContentSecurityPolicyConfigDto {
    fn from(e: &ContentSecurityPolicyConfig) -> Self {
        Self { enabled: e.enabled, custom_attributes: e.custom_attributes.clone() }
    }
}
...
impl From<&WebUiConfig> for WebUiConfigDto {
    fn from(instance: &WebUiConfig) -> Self {
        let stream_info =
            instance.stream_info.as_ref().and_then(|cfg| if cfg.is_none() { None } else { Some(cfg.into()) });
        Self {
