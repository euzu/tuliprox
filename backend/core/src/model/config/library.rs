...
use shared::{
    defaults::DEFAULT_STORAGE_DIR,
    error::TuliproxError,
    model::{ConfigDto, LibraryConfigDto, LibraryContentType, LibraryMetadataFormat},
    utils::Internable,
};
...
pub fn validate_library_paths_from_dto(cfg: &ConfigDto) -> Result<(), TuliproxError> {
    ...
    let storage_dir =
        cfg.storage_dir.as_deref().map(str::trim).filter(|dir| !dir.is_empty()).unwrap_or(DEFAULT_STORAGE_DIR);
...
mod tests {
    ...
    fn validate_library_paths_rejects_missing_scan_directory() {
        let cfg = ConfigDto {
            storage_dir: Some(
                std::env::current_dir().expect("current_dir should be available").to_string_lossy().to_string(),
            ),
