...
use shared::{
    error::TuliproxError,
    model::{
        PanelApiAliasPoolDto, PanelApiAliasPoolSizeDto, PanelApiAliasPoolSizeValue, PanelApiConfigDto,
        PanelApiProvisioningDto, PanelApiQueryParamDto, PanelApiQueryParametersDto,
    },
    utils::Internable,
};
use std::sync::Arc;
...
impl From<&PanelApiAliasPoolDto> for PanelApiAliasPool {
    fn from(dto: &PanelApiAliasPoolDto) -> Self {
        Self { size: dto.size.as_ref().map(PanelApiAliasPoolSize::from), remove_expired: dto.remove_expired }
    }
}
    ...
    fn from(instance: &PanelApiConfig) -> Self {
        Self {
            ...
            alias_pool: instance.alias_pool.as_ref().map(|p| PanelApiAliasPoolDto {
                size: p.size.as_ref().map(|s| PanelApiAliasPoolSizeDto { min: s.min.clone(), max: s.max.clone() }),
