...
impl From<&StreamBufferConfigDto> for StreamBufferConfig {
    fn from(dto: &StreamBufferConfigDto) -> Self {
        Self { enabled: dto.enabled, size: dto.size, max_bytes_mb: dto.max_bytes_mb }
    }
}
...
mod tests {
    ...
    fn stream_config_preserves_explicit_empty_admission_strategies() {
        let domain =
            StreamConfig::from(&StreamConfigDto { admission_strategies: Some(vec![]), ..StreamConfigDto::default() });
