...
- **Added `use_disk_input_playlist`** (boolean, default `false`) to `config.yml`. When enabled, input playlists are processed from disk instead of memory.
...
- **Disk-Based Provider Playlist**: New `use_disk_input_playlist` config option massively reduces RAM usage by streaming playlist data from disk (BPlusTree) during updates.
