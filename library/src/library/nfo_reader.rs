...
impl NfoReader {
    ...
    fn parse_nfo(content: &str) -> Option<MediaMetadata> {
        ...
        let is_movie =
            content.contains("<movie") || (!content.contains("<tvshow") && !content.contains("<episodedetails"));
    ...
    fn parse_movie_nfo(content: &str) -> Option<MediaMetadata> {
        ...
        let parsed = Self::read_nfo_events(content, |tag, text, in_actor, current_actor| match tag {
...
<movie>
    ...
    <actor>
        ...
        if let Some(MediaMetadata::Movie(movie)) = metadata {
            ...
            assert_eq!(
                movie.actors.as_ref().and_then(|actors| actors.first()).and_then(|actor| actor.thumb.as_deref()),
                Some("neo.jpg")
            );
        } else {
