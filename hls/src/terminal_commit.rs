...
use super::{
    ...
    session_store::{HlsCurrentProxySessionAccess, HlsSessionHandle, HlsSessionIncarnation, HlsSessionStore},
    terminal_tail::{HlsTerminalCommitMediaGuard, HlsTerminalTailCompatibility, HlsTerminalTailPlan},
...
pub const HLS_TERMINAL_COMMIT_RETRY_CAPACITY: usize = 256;
...
pub const fn terminal_commit_retry_schedule_budget_ms() -> u64 {
...
pub const fn terminal_commit_retry_handoff_budget_ms() -> u64 {
...
#[must_use]
pub enum HlsTerminalCommitOutcome {
    Committed,
    AlreadyCommitted,
...
pub(super) type HlsTerminalCommitAttemptFn =
    fn(&Arc<RwLock<HlsAccessLeaseStore>>, &HlsTerminalCommitCommand, u64, u64) -> HlsTerminalCommitAttempt;

/// Immutable terminal mutation selected before entering the final CAS.
#[derive(Debug, Clone)]
pub enum HlsTerminalLeaseDecision {
...
pub struct HlsTerminalAssetRevisionGuard {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum HlsTerminalAssetRevisionValidation {
    Current,
...
impl HlsTerminalAssetRevisionGuard {
    ...
    pub fn for_optional_runtime_tail(
    ...
    pub fn authorizes_current_asset(&self, asset: HlsRuntimeCustomTailAssetIdentity) -> bool {
    ...
    #[cfg(any(test, feature = "test-support"))]
    pub fn matching_for_test(expected: Option<super::terminal_tail::HlsTerminalAssetIdentity>) -> Self {
        Self::new(expected, move || expected)
    }

    #[cfg(any(test, feature = "test-support"))]
    pub fn matching_runtime_for_test(expected: HlsRuntimeCustomTailAssetIdentity) -> Self {
...
pub struct HlsTerminalCommitOwnerKey {
...
pub struct HlsTerminalCommitCommand {
...
fn terminal_commit_command_binding_may_replace(
    ...
    match evidence_order {
        ...
        std::cmp::Ordering::Greater | std::cmp::Ordering::Equal => incoming.strictly_supersedes_asset_binding(existing),
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[must_use]
pub enum HlsTerminalCommitRetryDecision {
...
pub fn next_terminal_commit_retry(
...
#[must_use]
pub enum HlsTerminalCommitSubmissionDecision {
    Attempt { command: Arc<HlsTerminalCommitCommand>, owner_token: HlsTerminalCommitOwnerToken },
...
#[must_use]
pub enum HlsTerminalCommitRetryScheduleDecision {
...
pub struct HlsTerminalCommitOwnerToken(u64);
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct HlsTerminalCommitWorkerToken(u64);

/// Lifecycle action after the current worker has stopped for any reason.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[must_use]
pub enum HlsTerminalCommitWorkerFinished {
    Stopped,
...
#[must_use]
pub enum HlsTerminalCommitRetryAction {
...
pub struct HlsTerminalCommitRetryCoordinator {
...
impl HlsTerminalCommitRetryCoordinator {
    ...
    /// selected command.
    pub fn submit(&self, command: HlsTerminalCommitCommand, now_ms: u64) -> HlsTerminalCommitSubmissionDecision {
    ...
    fn submit_existing(
        ...
        if !command.may_replace(&existing.command) {
            ...
            let tightened_scheduled_at_ms = if deadline_tightened {
                terminal_commit_scheduled_before_deadline(existing.scheduled_at_ms, now_ms, effective_deadline_ms)
            } else {
                None
            };
            let schedule_changed =
                tightened_scheduled_at_ms.is_some_and(|scheduled_at_ms| scheduled_at_ms != existing.scheduled_at_ms);
            let scheduling_sequence = schedule_changed.then(|| next_terminal_commit_scheduling_sequence(state));
            if let Some(owner) = state.owners.get_mut(key) {
    ...
    pub fn schedule_current(
        ...
        let mut state = self.lock_state();
        let Some(owner) = state.owners.get(key).filter(|owner| owner.owner_token == owner_token) else {
        ...
        let Some(scheduled_at_ms) =
            ...
            return HlsTerminalCommitRetryScheduleDecision::Failed(HlsTerminalCommitOutcome::SafeCommitDeadlineElapsed);
        };
    ...
    pub fn next_action(&self, worker_token: HlsTerminalCommitWorkerToken, now_ms: u64) -> HlsTerminalCommitRetryAction {
        let mut state = self.lock_state();
        if state.active_worker != Some(worker_token) {
            return HlsTerminalCommitRetryAction::Stop;
        }
        if state.active_worker_owner.as_ref().is_some_and(|owner| owner.worker_token == worker_token) {
            state.active_worker_owner = None;
        }
        for owner in state
            .owners
            .values_mut()
            .filter(|owner| owner.terminal_failure.is_none() && now_ms >= owner.effective_deadline_ms)
        {
            owner.in_flight = false;
            owner.terminal_failure = Some(HlsTerminalCommitOutcome::SafeCommitDeadlineElapsed);
        }
        ...
        state.active_worker_owner = Some(HlsTerminalCommitWorkerOwner { worker_token, key: key.clone(), owner_token });
    ...
    pub fn worker_finished(&self, worker_token: HlsTerminalCommitWorkerToken) -> HlsTerminalCommitWorkerFinished {
        let mut state = self.lock_state();
        if state.active_worker != Some(worker_token) {
            return HlsTerminalCommitWorkerFinished::StaleWorker;
        }
        if let Some(worker_owner) = state.active_worker_owner.take().filter(|owner| owner.worker_token == worker_token)
        {
            if let Some(owner) =
                state.owners.get_mut(&worker_owner.key).filter(|owner| owner.owner_token == worker_owner.owner_token)
            {
                owner.in_flight = false;
            }
        }
    ...
    #[cfg(any(test, feature = "test-support"))]
    pub fn early_worker_exit_for_test(
        &self,
        worker_token: HlsTerminalCommitWorkerToken,
    ) -> HlsTerminalCommitWorkerFinished {
    ...
    pub fn complete_owner(&self, key: &HlsTerminalCommitOwnerKey, owner_token: HlsTerminalCommitOwnerToken) {
        let mut state = self.lock_state();
    ...
    pub fn fail_owner(
        ...
        let mut state = self.lock_state();
        if let Some(owner) = state.owners.get_mut(key).filter(|owner| owner.owner_token == owner_token) {
            owner.in_flight = false;
    ...
    pub fn fail_runnable_owners(&self, outcome: HlsTerminalCommitOutcome) {
        let mut state = self.lock_state();
    ...
    pub fn discard_owner(&self, key: &HlsTerminalCommitOwnerKey, owner_token: HlsTerminalCommitOwnerToken) {
        let mut state = self.lock_state();
    ...
    pub fn with_current_owner<R>(
        ...
        let owner = state
            .owners
            .get(key)
            .filter(|owner| owner.owner_token == owner_token && owner.in_flight && owner.terminal_failure.is_none())?;
    ...
    pub fn reserve_submission(&self) -> Option<(u64, HlsTerminalCommitOwnerToken)> {
        let mut state = self.lock_state();
    ...
    pub fn owner_count(&self) -> usize {
        self.lock_state().owners.values().filter(|owner| owner.terminal_failure.is_none()).count()
    }

    #[cfg(any(test, feature = "test-support"))]
    pub fn set_capacity_for_test(&self, capacity: usize) {
...
async fn run_terminal_commit_retry_worker(
    ...
    loop {
        match retries.next_action(worker_token, clock.now_ms()) {
            HlsTerminalCommitRetryAction::Due { key, owner_token, session } => {
                let current_attempt =
                    match sessions.try_with_current_proxy_session(&key.proxy_session_id, &session, || {
                        ...
                        HlsCurrentProxySessionAccess::LockBusy => retries.with_current_owner(
                            &key,
                            owner_token,
                            |_command, latest_safe_terminal_commit_at_ms| {
                                let attempt_now_ms = clock.now_ms();
                                (attempt_now_ms, latest_safe_terminal_commit_at_ms, HlsTerminalCommitAttempt::LockBusy)
                            },
                        ),
                    };
...
fn finish_terminal_commit_retry_attempt(
    ...
    match attempt {
        ...
        HlsTerminalCommitAttempt::LockBusy => {
            match next_terminal_commit_retry(attempts_completed, attempt_now_ms, deadline_ms) {
...
mod tests {
    use super::{
        ...
        next_terminal_commit_retry, next_terminal_commit_scheduling_sequence,
        terminal_commit_command_binding_may_replace, terminal_commit_retry_handoff_budget_ms,
        terminal_commit_retry_schedule_budget_ms, terminal_commit_schedule_cmp, HlsTerminalAssetRevisionGuard,
        HlsTerminalCommitCommand, HlsTerminalCommitOutcome, HlsTerminalCommitOwnerToken, HlsTerminalCommitRetryAction,
        HlsTerminalCommitRetryCoordinator, HlsTerminalCommitRetryDecision, HlsTerminalCommitRetryScheduleDecision,
        HlsTerminalCommitRetryState, HlsTerminalCommitSubmissionDecision, HlsTerminalCommitWorkerFinished,
        HlsTerminalCommitWorkerOwnedWork, HlsTerminalLeaseDecision, HLS_TERMINAL_COMMIT_EXCLUSIVE_DEADLINE_GUARD_MS,
        HLS_TERMINAL_COMMIT_MAX_ATTEMPTS, HLS_TERMINAL_COMMIT_MAX_BACKOFF_MS, HLS_TERMINAL_COMMIT_MAX_WORKER_RESTARTS,
    };
    use crate::{
    ...
    fn retry_command(
        ...
        HlsTerminalCommitCommand {
            ...
            session: Arc::new(RwLock::new(HlsSession::new(HlsSessionKey::new(1, lease), b"terminal-retry-test", 0))),
    ...
    fn submit_and_schedule(
        ...
        scheduled_at_ms: u64,
        now_ms: u64,
    ) -> (super::HlsTerminalCommitWorkerToken, super::HlsTerminalCommitOwnerKey, HlsTerminalCommitOwnerToken) {
    ...
    fn hls_terminal_commit_retry_current_asset_binding_replaces_stale_owner_without_revision_ordering() {
        ...
        assert!(
            terminal_commit_command_binding_may_replace(std::cmp::Ordering::Equal, &current_request, &stale_owner,)
        );
        assert_eq!(
    ...
    fn unavailable_after_owner_failure_retains_the_exclusive_media_tail_deadline() {
        ...
        let media_tail_deadline =
            command.preparation.cutover_timing.latest_safe_terminal_commit_at.as_millis_since_epoch();
        assert_eq!(command.latest_safe_terminal_commit_at_ms(), media_tail_deadline);

        command.decision =
            HlsTerminalLeaseDecision::UnavailableAfterOwnerFailure(HlsTerminalTailCompatibility::TerminalMediaNotReady);
        assert_eq!(command.latest_safe_terminal_commit_at_ms(), media_tail_deadline);
    }
    ...
    fn failed_media_tail_owner_cannot_be_replaced_at_the_exclusive_deadline() {
        ...
        let HlsTerminalCommitSubmissionDecision::Attempt { owner_token, .. } = coordinator.submit(command, 1) else {
    ...
    fn hls_terminal_commit_submission_weaker_command_tightens_deadline_without_replacing_owner() {
        ...
        assert_eq!(after.command_submission_token.as_u64(), 1, "the stronger existing command remains bound");
    ...
    fn hls_terminal_commit_submission_equal_evidence_never_extends_deadline() {
        ...
        assert_eq!(coordinator.owner_snapshot(&key).map(|owner| owner.effective_deadline_ms), Some(200));
    ...
    fn hls_terminal_commit_retry_reschedule_advances_equal_due_peer_first() {
        ...
        let HlsTerminalCommitSubmissionDecision::Attempt { command, owner_token, .. } = coordinator.submit(second, 90)
        else {
    ...
    fn hls_terminal_commit_worker_exit_releases_only_its_claimed_owner() {
        ...
        assert!(coordinator.owner_snapshot(&second_key).expect("unrelated client owner").in_flight);
    }
}
