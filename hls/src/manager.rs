...
impl HlsProxyManager {
    ...
    pub async fn prepare_access_lease_runtime_custom_tail(
        ...
        if reason.trigger_class()
            != super::runtime_custom_tail::HlsRuntimeCustomTailTriggerClass::ImmediatePolicyCutover
        {
            return None;
        }
    ...
    fn commit_media_activity_locked(
        ...
        let (current, capacity_protection_released) = match kind {
            ...
            HlsMediaActivityCommitKind::LiveSegmentCompletion(token) => {
                ...
                (completion.is_some(), matches!(completion, Some(super::HlsPlaybackCompletionOutcome::Advanced)))
            }
        };
        if !current {
            return HlsMediaActivityCommitAttempt::Completed {
...
mod tests {
    ...
    async fn contiguous_live_completion_wakes_capacity_protection_waiters() {
        let (manager, session, proxy_session_id, lease_id, live_identity) =
            live_media_fixture("capacity-release").await;
