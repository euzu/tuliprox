...
| `runtime_config_report_format`  | String | `json`  | Output format for the runtime config report. Supported values: `json` and `yaml`. Both formats are emitted as pretty, multi-line output. Ignored unless `runtime_config_report_enabled` is `true`.                                     |
