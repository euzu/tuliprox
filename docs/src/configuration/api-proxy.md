...
> **Important:** When you define credentials for a `target`, ensure that this target has an output format of `xtream` or `m3u` configured in your `source.yml`.
...
* **`redirect`** *(Default)*: Tuliprox responds to the client with an HTTP 302 Redirect, pointing directly to the upstream provider's URL (or rotating through DNS failover IPs).
...
*(Note: Only connections with exactly one active listener are eligible for eviction — shared connections with multiple listeners are not interrupted. Internal FFprobe metadata probe tasks run by default at the absolute lowest priority (`127`) and are immediately preempted/killed if any real user needs the slot.)*
...
Tuliprox also offers **REST-friendly aliases** in case restrictive firewalls or ISP blocks target `.php` extensions. For the sake of simplicity, you can also use `token` in place of the `username` and `password` combination on these endpoints:
...
#### Example: Nginx
When using Nginx, ensure that buffering is disabled to prevent stream stuttering or high memory usage on the proxy.
...
You can use Traefik as a reverse proxy in front of your Tuliprox instance. This is especially useful for handling TLS termination (HTTPS).

When using subdirectories (paths) instead of subdomains, you must ensure that Traefik strips the prefix before forwarding the request. This allows Tuliprox to handle the request as if it were at the root level (`/`).

⚠️ Connection Note: Ensure the path defined in your config.yml (under server: external) matches the path you use in Traefik. If you set `path: tuliprox` in Tuliprox, your clients must connect via `my-external-domain.com/tuliprox/...`.
...
> **Pro-Tip:** Ensure your Traefik static configuration (`entryPoints`) includes your Docker network range in `trustedIPs`. Otherwise, Traefik might strip the forwarded headers before they reach Tuliprox.