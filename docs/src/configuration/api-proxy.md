...
| `max_connections`       | Int      |    No    | `0`        | Additional streams allowed *above* `max_connections`. The total concurrent allowance is `max_connections + soft_connections`, so `max_connections: 1` with `soft_connections: 1` permits two concurrent streams. Soft streams are preempted before normal ones. `0` = no extra allowance. **Requires** `user_access_control: true` in `config.yml` to be enforced.                                                                    |
