...
pub const fn is_zero_i16(v: &i16) -> bool {
    *v == 0
}
pub const fn is_zero_u32(v: &u32) -> bool {
    *v == 0
}
...
pub fn is_empty_optional_vec<T>(s: &Option<Vec<T>>) -> bool {
    s.as_ref().is_none_or(std::vec::Vec::is_empty)
}
...
pub fn is_cluster_optional(cf: &Option<ClusterFlags>) -> bool {
    cf.is_none_or(|c| c.is_all())
}
