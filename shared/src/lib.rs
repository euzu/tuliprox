...
//
// `clippy::all` + `clippy::pedantic` are switched on workspace-wide. Everything
// clippy can rewrite mechanically has been applied with `cargo clippy --fix`.
// The lints below are the residue that has no machine-applicable fix and would
// need hand edits to DTO, serde and parsing code that this policy is explicitly
// not allowed to change the behaviour of.
//
