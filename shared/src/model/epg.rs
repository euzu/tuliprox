...
/// Append-only compatibility of the persisted EPG records.
///
/// The B+Tree encodes these structs positionally, so the origin fields are appended at the end of
/// the field list and an EPG database written by an earlier version must still load.
#[cfg(test)]
mod persistence_compatibility {
    use super::{EpgCategory, EpgChannel, EpgProgramme};
    use crate::utils::Internable;
    use serde::Serialize;
    use std::sync::Arc;

    /// The `EpgChannel` field list without the appended origin field.
    #[derive(Serialize)]
    ...
    /// The `EpgProgramme` field list without the appended origin field.
    #[derive(Serialize)]
    struct LegacyProgramme {
        start: i64,
        stop: i64,
        title: Option<Arc<str>>,
        desc: Option<Arc<str>>,
    ...
    #[test]
    fn channel_written_before_origin_tracking_still_decodes() {
        let legacy = LegacyChannel {
            id: "epg.channel".intern(),
            title: Some("Channel".intern()),
            icon: Some("https://cdn.example.com/icon.png".intern()),
            programmes: Vec::new(),
        };
        let bytes = rmp_serde::to_vec(&legacy).expect("encode legacy channel");

        let decoded: EpgChannel = rmp_serde::from_slice(&bytes).expect("legacy channel still decodes");

        assert_eq!(decoded.icon.as_deref(), Some("https://cdn.example.com/icon.png"));
    }

    #[test]
    fn programme_written_before_origin_tracking_still_decodes() {
        let legacy = LegacyProgramme {
            start: 100,
            stop: 200,
            title: Some("Title".intern()),
            desc: None,
            catchup_id: None,
            categories: Vec::new(),
            is_live: false,
            is_new: false,
            previously_shown: false,
            icon: Some("https://cdn.example.com/programme.png".intern()),
        };
        let bytes = rmp_serde::to_vec(&legacy).expect("encode legacy programme");

        let decoded: EpgProgramme = rmp_serde::from_slice(&bytes).expect("legacy programme still decodes");

        assert_eq!(decoded.icon.as_deref(), Some("https://cdn.example.com/programme.png"));
    }
}
