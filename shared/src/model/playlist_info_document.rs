...
use crate::model::{
    PlaylistItemType, SeriesStreamDetailEpisodeProperties, SeriesStreamDetailSeasonProperties,
...
impl StreamProperties {
    pub fn to_info_document(
        ...
        match self {
            ...
            StreamProperties::Video(video) => XtreamInfoDocument::Video(
                self.video_to_info_document(options, video, item_type, virtual_id, category_id),
            ),
            StreamProperties::Series(series) => XtreamInfoDocument::Series(
    ...
    fn series_to_info_document(
        ...
        XtreamSeriesInfoDoc {
            ...
            info: XtreamSeriesInfoData {
                ...
                cover: InfoDocUtils::make_resource_url(
            ...
            episodes: if let Some(episodes) =
                series.details.as_ref().and_then(|d| d.episodes.as_ref())
            {
    ...
    fn video_to_info_document(
        ...
        let stream_icon = InfoDocUtils::make_resource_url(
            resource_url.as_deref(),
            &self.get_stream_icon(),
            "logo",
        )
        .intern();
        ...
        let info = if let Some(details) = video.details.as_ref() {
            XtreamVideoInfoData {
                ...
                duration_secs: details
                    .duration_secs
                    .as_ref()
                    .map_or_else(|| Arc::clone(&empty_str), Arc::clone),
                duration: details
                    .duration
                    .as_ref()
                    .map(Arc::clone)
