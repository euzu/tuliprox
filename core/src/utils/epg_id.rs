...
pub enum EpgIdOutputCase {
    Preserve,
    LowercaseAscii,
}

impl EpgIdOutputCase {
    pub const fn from_lowercase(lowercase: bool) -> Self {
        if lowercase {
            Self::LowercaseAscii
        } else {
            Self::Preserve
...
pub fn with_folded_epg_id<R>(id: &str, f: impl FnOnce(&str) -> R) -> R {
...
pub fn fold_epg_id_arc(id: &Arc<str>) -> Arc<str> {
    with_folded_epg_id(id, |folded| if folded == id.as_ref() { Arc::clone(id) } else { folded.intern() })
}

/// Folds an untrusted request ID without adding it to the global interner.
pub fn fold_untrusted_epg_id(id: &str) -> Arc<str> {
...
pub fn canonicalize_untrusted_epg_id(id: &str, output_case: EpgIdOutputCase) -> Arc<str> {
    match output_case {
...
pub fn canonicalize_output_epg_id(id: &Arc<str>, output_case: EpgIdOutputCase) -> Arc<str> {
    match output_case {
...
pub fn lowercase_xmltv_text(value: &str, enabled: bool) -> Cow<'_, str> {
    if !enabled {
...
mod tests {
    use super::{
        canonicalize_output_epg_id, canonicalize_untrusted_epg_id, fold_epg_id_arc, fold_untrusted_epg_id,
        lowercase_xmltv_text, EpgIdOutputCase,
    };
    use quick_xml::{
