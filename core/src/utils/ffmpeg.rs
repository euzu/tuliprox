...
impl FfmpegExecutor {
    ...
    pub async fn create_thumbnail(&self, input_path: &str, width: u32, height: u32) -> Result<Vec<u8>, String> {
        ...
        let output = self
            .run_ffmpeg_with_timeout(&build_thumbnail_args(input_path, &output_path, &scale_filter, 180))
            .await
        ...
        if !output.status.success() {
            ...
            if stderr.contains("Output file is empty") || stderr.contains("nothing was encoded") {
                ...
                let retry = self
                    .run_ffmpeg_with_timeout(&build_thumbnail_args(input_path, &output_path, &scale_filter, 0))
                    .await
        ...
        tokio::fs::read(&output_path).await.map_err(|e| format!("Failed to read thumbnail: {e}"))
    }

    pub async fn probe_url(&self, params: &ProbeParams<'_>, proxy_cfg: Option<&ProxyConfig>) -> ProbeUrlOutcome {
    ...
    pub async fn probe_remote_url(&self, client: &Client, params: &ProbeParams<'_>) -> ProbeUrlOutcome {
        let ProbeParams { url, user_agent, analyze_duration, probe_size, timeout_secs } = *params;
        let analyze_overhead = Duration::from_micros(analyze_duration) + Duration::from_secs(5);
        let config_timeout = Duration::from_secs(timeout_secs);
        let timeout_val = std::cmp::max(analyze_overhead, config_timeout);
        ...
        if let Ok(outcome) = probe_result {
            ...
            warn!("ffprobe remote stdin probe timed out after {:?} for {}", timeout_val, sanitize_sensitive_info(url));
            ProbeUrlOutcome::Failed(ProbeFailureKind::Other)
        }
    }

    pub async fn probe_remote_seekable_url(&self, client: &Client, params: &ProbeParams<'_>) -> ProbeUrlOutcome {
        let ProbeParams { url, user_agent, analyze_duration, probe_size, timeout_secs } = *params;
        let analyze_overhead = Duration::from_micros(analyze_duration) + Duration::from_secs(5);
        let config_timeout = Duration::from_secs(timeout_secs);
        let timeout_val = std::cmp::max(analyze_overhead, config_timeout);
    ...
    async fn probe_remote_url_inner(
        ...
        let fetch_result =
            stream_probe_bytes_to_stdin(&mut stdin, client, url, user_agent, probe_size, max_bytes).await;
        ...
        match child.wait_with_output().await {
            Ok(output) => {
                if let Some(kind) = stdin_err {
                    debug!(
                        "ffprobe remote stdin probe had fetch error, checking child output for {}",
                        sanitize_sensitive_info(url)
                    );
    ...
    async fn probe_remote_seekable_url_inner(
        ...
        if !stage.fully_materialized {
            debug!("Running seekable ffprobe probe with staged head/tail data for {}", sanitize_sensitive_info(url));
        }
...
fn format_ffmpeg_timeout_error(args: &[String]) -> String {
    ...
    format!("Timed out running ffmpeg after {}s: {summary}", FFMPEG_TIMEOUT.as_secs())
}
...
async fn create_seekable_probe_temp_file() -> Result<tempfile::NamedTempFile, ProbeFailureKind> {
    ...
    if let Err(err) = cleanup_stale_seekable_probe_files(&temp_dir).await {
        debug!("Failed to cleanup stale ffprobe temp files in {}: {}", temp_dir.display(), err);
    }

    tempfile::Builder::new().prefix("probe-").suffix(".bin").tempfile_in(&temp_dir).map_err(|err| {
...
async fn ensure_probe_file_length(path: &Path, content_length: Option<u64>) -> Result<(), ProbeFailureKind> {
    ...
    let file = OpenOptions::new().create(true).write(true).truncate(false).open(path).await.map_err(|err| {
        warn!("Failed to open seekable ffprobe temp file {}: {}", path.display(), err);
        ProbeFailureKind::Other
    })?;
    file.set_len(content_length).await.map_err(|err| {
        warn!("Failed to resize seekable ffprobe temp file {}: {}", path.display(), err);
        ProbeFailureKind::Other
    })
}
...
async fn flush_probe_file(file: &mut fs::File, path: &Path) -> Result<(), ProbeFailureKind> {
    file.flush().await.map_err(|err| {
        warn!("Failed to flush seekable ffprobe temp file {}: {}", path.display(), err);
        ProbeFailureKind::Other
    })
}
...
async fn probe_local_path_with_ffprobe(path: &Path, analyze_duration: u64, probe_size: u64) -> ProbeUrlOutcome {
    ...
    match command.output().await {
        ...
        Err(err) => {
            warn!("ffprobe execution failed for {}: {}", sanitize_sensitive_info(&display_path), err);
            ProbeUrlOutcome::Failed(ProbeFailureKind::Other)
        }
    }
}
...
async fn stage_seekable_probe_file(
    ...
    let mut stage = SeekableProbeStage {
        ...
        fully_materialized: head_result.status != StatusCode::PARTIAL_CONTENT
            ...
            && head_result.total_length.is_none_or(|content_length| head_result.bytes_written >= content_length),
...
async fn fetch_remote_span_to_file(req: &RemoteSpanRequest<'_>) -> Result<SpanWriteResult, ProbeFailureKind> {
    let RemoteSpanRequest { client, url, user_agent, range, path, offset, max_bytes, allow_full_body_write_on_200 } =
        *req;
    ...
    if !status.is_success() && status != StatusCode::PARTIAL_CONTENT {
        warn!("ffprobe fetch returned {} for {}", status, sanitize_sensitive_info(url));
        return Err(ProbeFailureKind::Other);
    }

    let total_length = parse_remote_content_length(&response);
    if status == StatusCode::OK && range.is_some() && !allow_full_body_write_on_200 {
        return Ok(SpanWriteResult { status, total_length, bytes_written: 0, response_exhausted: false });
    }

    let mut file = OpenOptions::new().create(true).write(true).truncate(false).open(path).await.map_err(|err| {
        warn!("Failed to open seekable ffprobe temp file {}: {}", path.display(), err);
        ProbeFailureKind::Other
    })?;
    file.seek(std::io::SeekFrom::Start(offset)).await.map_err(|err| {
        warn!("Failed to seek seekable ffprobe temp file {}: {}", path.display(), err);
        ProbeFailureKind::Other
    })?;
    ...
    while bytes_written < max_bytes {
        ...
        file.write_all(&chunk[..to_write]).await.map_err(|err| {
            warn!("Failed to write seekable ffprobe temp file {}: {}", path.display(), err);
            ProbeFailureKind::Other
        })?;
    ...
    W: tokio::io::AsyncWrite + Unpin,
{
    ...
    if !response.status().is_success() && response.status() != StatusCode::PARTIAL_CONTENT {
        warn!("ffprobe fetch returned {} for {}", response.status(), sanitize_sensitive_info(url));
        return Err(ProbeFailureKind::Other);
    }
    ...
    loop {
        ...
        let to_write = if chunk.len() <= remaining { chunk.len() } else { remaining };

        if to_write > 0 {
            match stdin.write_all(&chunk[..to_write]).await {
                Ok(()) => {}
                Err(err) if err.kind() == ErrorKind::BrokenPipe => {
                    debug!("ffprobe closed stdin early for {}, stopping fetch", sanitize_sensitive_info(url));
                    return Ok(());
                }
                Err(err) => {
    ...
    W: tokio::io::AsyncWrite + Unpin,
{
    match stdin.write_all(probe_bytes).await {
        Ok(()) => Ok(()),
        Err(err) if err.kind() == ErrorKind::BrokenPipe => {
            debug!("ffprobe closed stdin early for {}, continuing with child output", sanitize_sensitive_info(url));
            Ok(())
        }
        Err(err) => {
            warn!("ffprobe stdin write failed for {}: {}", sanitize_sensitive_info(url), err);
...
fn apply_proxy_to_ffprobe(command: &mut Command, proxy_cfg: Option<&ProxyConfig>) {
    ...
    let Some(proxy_url) = build_ffprobe_proxy_url(proxy_cfg) else {
        warn!("Ignoring invalid ffprobe proxy URL: {}", sanitize_sensitive_info(proxy_cfg.url.as_str()));
        return;
    };
...
fn is_not_found_probe_error(stderr: &str) -> bool {
    ...
    ["http error 404", "404 not found", "http/1.1 404", "http/2 404", "server returned 404"]
        .iter()
        .any(|needle| normalized.contains(needle))
}

fn extract_probe_stream_stats(
    json: &Value,
    video_stream: Option<&Value>,
    audio_stream: Option<&Value>,
) -> ProbeStreamStats {
    ...
    ProbeStreamStats {
        ...
        bitrate: format.and_then(|value| parse_u32_field(value.get("bit_rate"))).or_else(|| {
...
mod tests {
    use super::{
        build_ffprobe_proxy_url, build_thumbnail_args, build_thumbnail_scale_filter, ensure_probe_file_length,
        extract_probe_stream_stats, fetch_probe_bytes, format_ffmpeg_timeout_error, parse_content_range_total_length,
        run_ffprobe_with_stdin, seekable_probe_window_bytes, stage_seekable_probe_file, write_probe_bytes_to_stdin,
        ProbeFailureKind, ProbeStreamStats, ProbeUrlOutcome, FFMPEG_TIMEOUT,
    };
    use crate::model::ProxyConfig;
    use serde_json::json;
                ...
                tokio::spawn(async move {
                    ...
                    let (status_line, content_range, payload) = if let Some(range_header) = range_header {
                        ...
                        (
                            ...
                            Some(format!("Content-Range: bytes {}-{}/{}\r\n", bounded_start, bounded_end, body.len())),
    ...
    fn build_ffprobe_proxy_url_keeps_existing_inline_credentials() {
        let proxy_cfg =
            ProxyConfig { url: "socks5://bob:pass@proxy.local:1080".to_string(), username: None, password: None };
        let resolved = build_ffprobe_proxy_url(&proxy_cfg).expect("proxy url should parse");
    ...
    fn format_ffmpeg_timeout_error_includes_binary_timeout_and_args() {
        let msg = format_ffmpeg_timeout_error(&[
            "-ss".to_string(),
            "180".to_string(),
            "-i".to_string(),
            "/tmp/in.mkv".to_string(),
        ]);
    ...
    fn extract_probe_stream_stats_prefers_format_section() {
        ...
        assert_eq!(
            ...
            ProbeStreamStats { duration_secs: Some(1541), bitrate: Some(3_100_000) }
        );
    }
    ...
    fn extract_probe_stream_stats_falls_back_to_stream_values() {
        ...
        assert_eq!(
            ...
            ProbeStreamStats { duration_secs: Some(120), bitrate: Some(1500) }
        );
    }
    ...
    fn extract_probe_stream_stats_sums_video_and_audio_bitrates() {
        ...
        assert_eq!(extract_probe_stream_stats(&payload, Some(&video), Some(&audio)).bitrate, Some(2_692_000));
    }
    ...
    fn extract_probe_stream_stats_saturates_combined_stream_bitrate() {
        ...
        assert_eq!(extract_probe_stream_stats(&payload, Some(&video), Some(&audio)).bitrate, Some(u32::MAX));
    }
    ...
    async fn ensure_probe_file_length_preserves_remote_logical_length() {
        ...
        ensure_probe_file_length(&temp_path, Some(remote_logical_length)).await.expect("file length should be set");

        let metadata = tokio::fs::metadata(&temp_path).await.expect("probe file metadata should be readable");
        assert_eq!(metadata.len(), remote_logical_length);
    }

    #[test]
    fn parse_content_range_total_length_extracts_total() {
        assert_eq!(parse_content_range_total_length("bytes 10-20/999"), Some(999));
        assert_eq!(parse_content_range_total_length("bytes 0-1/*"), None);
    }

    #[tokio::test]
    async fn fetch_probe_bytes_caps_response_and_sets_headers() {
        let (url, request_capture, accepted, handle) =
            match start_test_http_server("HTTP/1.1 200 OK", b"abcdefghij").await {
                Ok(server) => server,
        ...
        let bytes = fetch_probe_bytes(&client, &url, Some("test-agent/1.0"), 5).await.expect("fetch should succeed");

        assert_eq!(bytes, b"abcde");
        assert_eq!(accepted.load(Ordering::SeqCst), 1);

        let request_text = String::from_utf8_lossy(&request_capture.lock().expect("request capture mutex poisoned"))
            .to_ascii_lowercase();
    ...
    async fn stage_seekable_probe_file_writes_head_and_tail_segments() {
        ...
        let (url, accepted, handle) =
            start_range_test_http_server(Arc::clone(&body)).await.expect("range server should start");
        let tempdir = tempfile::tempdir().expect("tempdir should succeed");
        let temp_path = tempdir.path().join("probe.bin");
        let client = reqwest::Client::builder().no_proxy().build().expect("http client should build");
