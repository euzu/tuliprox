...
use shared::{
    ...
    model::{HdHomeRunConfigDto, HdHomeRunDeviceConfigDto},
};
...
create_bitset!(u8, HdHomeRunFlags, Enabled, Auth, SsdpDiscovery, ProprietaryDiscovery);
...
impl From<&HdHomeRunConfigDto> for HdHomeRunConfig {
    fn from(dto: &HdHomeRunConfigDto) -> Self {
        ...
        Self { flags, devices: dto.devices.iter().map(Into::into).collect() }
    }
}
